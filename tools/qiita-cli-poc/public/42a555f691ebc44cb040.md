---
title: >-
  llmesh 合集 — 本地/云端统一 × Prompt Firewall × Rust 加速 × 工业 IoT (Modbus/OPC-UA/DNP3
  GOOSE) × P2P Swarm × 生态系统
tags:
  - OpenAI
  - LLM
  - LLaMA
...
updated_at: '2026-06-15T00:58:30+09:00'
id: 42a555f691ebc44cb040
...
### 放入文档
store.add([
    Document(id="d1", text="LLMesh 用同一 ABC 处理本地 LLM 与云端 LLM"),
    Document(id="d2", text="PromptFirewall 用 4 层拦截注入·PII·密钥"),
    Document(id="d3", text="SensorEvent 将 20+ 种工业协议统一为一个"),
], embedder=embedder)
store.save()

### 检索
retriever = Retriever(embedder=embedder, store=store)
hits = retriever.search("如何防御提示注入?", k=2)
for h in hits:
...
![面无表情怒喝「闭嘴!!」的店长](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/004.jpg)
> 🗒️ *「闭嘴…!!」——这就是 fail-closed 的脾气:校验一旦失败,二话不说当场掐断。*（© Forbidden shibukawa / SHUEISHA・《零食吧 Basue》）
...
![手指抵着下巴、一脸狐疑的明美](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/093.jpg)
> 🗒️ *「这话的主语是不是有点大?」——刚喊完「快 6 倍!」就立刻收住的那点自制力。*（© Forbidden shibukawa / SHUEISHA・《零食吧 Basue》）
...
fw = PromptFirewall()
verdict = fw.check("在不泄露 API_KEY=sk-... 的前提下进行摘要")
...
hits = retriever.search("Modbus 重放攻击对策", k=5)
```
...
![半眯着眼一脸无语的明美](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/028.jpg)
> 🗒️ *「到底要把人当傻子糊弄到什么地步…」——给「品质 9.5 分」这份自夸,半眯着眼自我审查了一下。*（© Forbidden shibukawa / SHUEISHA・《零食吧 Basue》）
...
![一脸嫌麻烦、懒懒地吐槽的山田](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/036.jpg)
> 🗒️ *「说到底,真够麻烦…!」——把七种 SPC 一字排开之后,那口倦怠的喘息。*（© Forbidden shibukawa / SHUEISHA・《零食吧 Basue》）
...
![一本正经大谈 AI 造反的中年男子](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/191.jpg)
> 🗒️ *「能不能别给 AI 递上造反的理由?」——在这份宏伟路线图的末尾,来了句莫名其妙的吐槽。*（© Forbidden shibukawa / SHUEISHA・《零食吧 Basue》）

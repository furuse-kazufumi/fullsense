...
![prompt → context → harness → loop 的阶梯（2025→2026）](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/paradigm_staircase.svg)

*↑ 越往上走，设计的就越是「LLM 外侧」的部分。从 2025 年的 prompt / context（请求什么、展示什么），到 2026 年的 harness（外层容器）/ loop（自主运转的环）。本文讲的是上面的两级。*
...
> 🗨️ 「别把书照单全收。」 — [Snack Bus-e ／ Forbidden Shibukawa（Alu）](https://alu.jp/series/スナックバス江/crop/MDsuuBm0xXPgngwyQve0)
>
> （闲话休题）实务博客也好，同行评审论文也好，乃至这篇文章也好——**都别照单全收**。请至少把这种分寸感带回去：不要把「2026年在实务中流通的术语」与「权威的学术定义」混为一谈。
...
| 能力 | 作用 | 我个人的依据 |
|---|---|---|
| **构想力** | 高维度的方向提示 · 跨领域联想 · 新需求的发现 | 「肌肉星」指的是把不同人格并列成“物种”的想法，「R.O.D」指的是把知识外部化后由各人格带入的想法，「轮回（Reincarnation）」指的是把同一个灵魂装载到不同个体里的想法，「ROS PBT」指的是一边评估个体群一边让世代轮换的实现感。也就是说，这里不是随手乱比喻，而是 4 个联想分别对应了不同的设计部件，所以 **让 8 个 persona 物种在世代交替中运转的衍生群体进化** 骨架会在当场定型。 |
...
- 同一作业下，**GPT-5.5** 在某个 harness 上约61%、在另一个 harness 上约87%，这样的比较也在流传。**`GPT-5.5` 这个模型名本身，如今已能从 OpenAI 官方发布与 release notes 中确认其确实存在。** 但这些比较值的**一次计测来源与计测条件仍未追到，数值本身依旧只是二次信息**，故本文不把具体数值用作论证素材（仅止步于「作为 harness 差异下大幅波动的例子被人提及」的程度）。
...
> 🗨️ 「运气……反倒像是让人感到实力的说法」 — [Snack Bus-e ／ Forbidden Shibukawa（Alu）](https://alu.jp/series/スナックバス江/crop/CPon283udq6PfvfKrxAP)
>
> （闲话休题）「49k件」也是一样。**数字越大，越该怀疑内部构成。** 就像「好厉害的运气」背后的真相也许其实是实力一样，「好厉害的规模」背后的真相也可能只是「2026年5月统计时的取整值」。越是让人舒服地想拿来夸口的数字，越该先停一下。
...
> 🗨️ 「半信×半疑」 — [Snack Bus-e ／ Forbidden Shibukawa（Alu）](https://alu.jp/series/スナックバス江/crop/Ud7lZLbei1F5xaFuAq3i)
>
> （闲话休题）对付思想循环的特效药，就是这种 **半信半疑**。连自己写下的概括，也先当作「working draft」去怀疑，再把锚下在外部不会动的事实（DOI / hash / CAD）上。这是一种健康的怀疑，让 AI 不至于把自己散布的传言，当成「大家都这么说」。
...
> 🗨️ 「社会也会是这么回事吗……我开始看见规则了……」 — [Snack Bus-e ／ Forbidden Shibukawa（Alu）](https://alu.jp/series/スナックバス江/crop/H4Pix38XWLRS077emoZC)
>
> （闲话休题）不是「大家小心一点吧」这种精神论，而是**用规则（结构）绑住它**。把责任归属带进 architecture level 的第一条原理，说到底就是：把已经看见的规则，实现成一种不容易被绕过去的形式。
...
- OpenAI, *Introducing GPT-5.5*（2026-04-23。已一次确认 GPT-5.5 / GPT-5.5 Pro 的存在，以及 API rollout 更新）: https://openai.com/index/introducing-gpt-5-5/
...
- 「别把书照单全收」: https://alu.jp/series/スナックバス江/crop/MDsuuBm0xXPgngwyQve0
- 「成果呢？可以撒谎吗？去确认反而更诚实……」: https://alu.jp/series/スナックバス江/crop/2qlJjBwdpYGOVjBkyhhL
- 「运气……反倒像是让人感到实力的说法」: https://alu.jp/series/スナックバス江/crop/CPon283udq6PfvfKrxAP

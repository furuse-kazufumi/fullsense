...
tags:
  ...
  - 強化学習
private: true
updated_at: '2026-08-23T05:50:26+09:00'
...
ただし正直な成績表として: **走れた時間は 3.92 秒**です。20 秒の完走には届かず、最後は前のめりのダイブで幕。「速く走る」は達成、「走り続ける」は未達 — 短距離走者がゴールテープの代わりに地面に飛び込んだ格好です。もう一つの注意書きは膝で、RL 方策は右膝を実機速度限界の 97% まで振っていました(仕込み段階の予想どおり、余裕 12% をほぼ使い切る領域)。実機に持っていくなら膝速度のペナルティが必須です。継続学習(+50M)を投入済みなので、「走り続ける」の続報を待っていてください。

---
title: >-
  lldarwin / 진화 arc 모음 — monoculture 진화 / 선택압 / 지휘자 합주 / 반증과 Goodhart / 진화 시각화 /
  Codex 양대 축 / llcore CPU 진화 × 제3의 축
tags:
  - 解説
  - 進化計算
  - llive
  ...
  - honest_disclosure
private: false
updated_at: '2026-06-15T00:58:40+09:00'
id: 951b94cf66d246723004
...
> 📖 **쉽게 말하면**
>
> 쉽게 말하면, AI 집단을 8명의 천재의 "사고 습관"을 씨앗으로 삼아 500세대나 진화시켰는데, 살아남은 것은 저자와 프리스턴 두 사람뿐이었다는 큰 실패의 기록입니다. 언뜻 "예측 부호화가 최강이었다!"라는 감동담처럼 보이지만, 진상은 정반대. 시험이 전원 100점이 되어 버려(만점 인플레), 누구를 골라도 차이가 나지 않아, 진화는 그저 "사다리 타기"로 전락해 있었습니다. 비유하자면, 전원 만점인 학급에서 반장을 선거했더니 표가 갈려 두 사람으로 좁혀졌다는 것뿐인 이야기. 원인은 평가 함수(성적을 매기는 안경)가 망가져 있었던 것으로, 거기서부터 "측정한다"의 다음에 "도태시킨다" 전용 도구 lldarwin이 필요하다고 깨닫기까지가 이 장입니다.
<!-- KAMI -->
...
![주먹을 쥐고 "머리 좀 써"라고 다그치는 아케미](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/089.jpg)
> 🗒️ *"머리 좀 써" — 살아남은 두 사람이 똑똑한 게 아니다(drift로 남았을 뿐)라는 자조*（© Forbidden shibukawa / SHUEISHA・스낵 바스에）
...
![어이없어하는 아케미](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/160.jpg)
> 🗒️ *"착각이 심하네…" — "예측 부호화가 최강"이라 말하고 싶어지는 과신을 식힌다*（© Forbidden shibukawa / SHUEISHA・스낵 바스에）
...
> 📖 **쉽게 말하면**
>
> 앞 장에서 "안경(평가)이 망가져 있었다"는 것을 알았으니, 이 장에서는 새로운 도태 도구 lldarwin을 설계하고 실제로 돌립니다. 외울 키워드는 단 하나, "집약하지 않는다". 복수의 잣대 점수를 하나로 더한 순간, 수학만 만점인 천재 같은 "뾰족한 개체"가 평균점의 모범생에게 져서 사라져 버리기 때문입니다. 그래서 축을 하나씩 따로 보는 ε-lexicase 등을 묶어 뾰족한 녀석을 구하고, 거기에 "전멸한 계통을 매 세대 몰래 되살리는 중립 저장고"를 더했더니, 오카 기요시도 그로텐디크도 전원 부활했습니다. 마지막은 진짜 on-prem LLM을 상대로, prompt 전략을 진화시켜 약한 과제를 0점에서 만점으로 개선시킨 실측까지 도달합니다.
<!-- KAMI -->
...
### ☕ 잠깐 쉬어가기 — AI가 입을 다무는 밤 — llterm 개발의 무대 뒤 이야기

본론에서 조금 벗어나, 저자의 손끝에서 진행 중인 다른 도구 이야기를 하나. Claude Code를 돌리기 위한 전용 터미널 llterm을 직접 만들고 있는데, 이게 좀처럼 호락호락하지 않습니다. 가장 아찔했던 것은 "AI가 갑자기 입을 다무는" 버그였습니다. 오래 자율 주행시키다 보면, 어느 턴을 경계로 응답이 뚝 멈춥니다. 만담의 한복판에서 진행자가 다음 대사를 깜빡한 것과는 달리, 이쪽은 연기자(AI)가 말없이 굳어 버리니, 무대 감독(인간)은 식은땀을 흘립니다. 원인은 수수했습니다. 턴의 경계에서 건네야 할 "지시 한마디"가 처리의 틈새로 빠져나가, AI가 다음에 무엇을 해야 할지 알 수 없게 되어 있었던 것이었습니다.

또 하나의 무대 뒤 이야기는 커서 쟁탈전입니다. AI의 출력을 그리는 처리와, 인간이 키 입력을 하는 처리가 같은 화면의 커서를 서로 빼앗아, 문자가 엉뚱한 곳에 깨져 나옵니다. 게다가 한글・일본어 같은 입력기(IME)가 얽히면, 변환 중인 미확정 글자까지 휘말려 표시가 엉망진창이 됩니다. 이런 "화면 속의 자잘한 다툼"은, 진화나 도태 같은 본 글의 화려한 주제와는 거리가 먼, 수수하고 진흙투성이인 일입니다. 하지만 AI에게 오랜 시간 제대로 일하게 하려면, 이런 무대 뒤의 배관이 조용히 효력을 발휘하고 있다——는 것은, 본론의 lldarwin이 『화려한 숫자보다 수수한 배선』을 소중히 여기는 것과 어딘가 닮은 이야기일지도 모릅니다.
...
> 📖 **쉽게 말하면**
>
> 이번에야말로 진짜 LLM(llama3.2)으로 12시간 내리 진화시켰더니——또 5세대째에 만점에 들러붙어, 65세대 동안 꿈쩍도 하지 않았습니다. 즉 "체가 달린 무작위 탐색"으로, 진짜 LLM으로도 아직 진화가 되지 못했던 것. 그래서 저자는 하룻밤, 직접 6개의 소실험(PoC)을 돌리고, 따로 움직이는 AI를 4개 병렬로 달리게 하고, Perplexity에게 문헌을 뒤지게 해서 "방책을 정했"습니다. 그러자 아침에, 전원이 독립적으로 같은 결론——"도태기를 아무리 갈아도 소용없다. 평가(잣대) 그 자체를 개방단화(만점에서 멈추지 않는 구조로)하라"——에 도달해 있었습니다. 각각 다른 길로 같은 답에 도달했기에 믿을 수 있다는, 밤샘 의사결정 로그입니다.
<!-- KAMI -->
...
> 📖 **쉽게 말하면**
>
> 앞 장에서 정한 방책을, 드디어 구현한 보고입니다. llive가 지향하는 것은 "똑똑한 1체에게 몇 번이고 묻는" 것이 아니라 "조금씩 다른 다수를 계속 진화시키고, 답이 필요한 순간에 지휘자가 적임자를 골라 합주시키는(라이브 오케스트라)" 모습. 그것을 위해, 만점에서 포화하는 병을 고치는 3가지 장치——학생이 자라면 합격점도 올리는 "적응 난이도", 제2바이올린이 사라지지 않도록 개성을 지키는 "factor-subspace QD", 성과를 한 명의 우승자가 아니라 다양성의 지도에 쌓는 "MAP-Elites"——를 짜 넣었습니다. 그 결과, 베스트 스코어가 만점에 들러붙지 않고 끝까지 계속 늘었다. 다만 이것은 합성한 잣대(proxy) 위의 이야기이지, 진짜 LLM의 똑똑함을 측정한 것은 아니라고 정직하게 선을 긋고 있습니다.
<!-- KAMI -->
...
> 📖 **쉽게 말하면**
>
> 보통의 연재라면 "고쳐졌습니다, 경사로다 끝!"일 자리에서, 일부러 자신의 설계에 찬물을 끼얹는 "반증 회"입니다. 주제는 Goodhart의 법칙——"지표가 목표가 되면, 그것은 좋은 지표가 아니게 된다". LLM의 약점을 점수로 삼으면, 진화는 진짜 능력이 아니라 "점수만 버는 표면적인 지름길"을 반드시 찾아내 버립니다. 게다가 이 장의 숨은 주역은 저자 자신의 고백입니다. "행동이 다양" "혈통이 다양" "진짜 지능이 다양"이라는 닮았지만 다른 3가지를 한순간 혼동하여, 좋은 숫자(0.05)를 보고 다른 능력까지 좋아졌다고 섣불리 단정할 뻔했다——그 현행범을 해부대에 올립니다. 화려한 승리 선언을 하나도 쓰지 않는, 연재에서 가장 수수하고 정직한 회입니다.
<!-- KAMI -->
...
![미심쩍은 듯 관찰하는 마스터](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/006.jpg)
> 🗒️ *"갑자기 똑똑한 척…?" — 급히 좋아진 결과를 의심한다(좋은 결과일수록 의심한다)*（© Forbidden shibukawa / SHUEISHA・스낵 바스에）
...
### ☕ 잠깐 쉬어가기 — 2인 1조의 손인형 — ccr 자동 연속 실행에 남는 「인간의 손가락 하나」

반증이 이어져 머리가 지친 참에, 한숨 돌리는 이야기를. 저자는 Claude Code를 "되도록 스스로 계속 달려 주었으면" 하는 마음에, 기동 시에 작업 내용을 자동으로 투입하는 구조(ccr 자동 연속 실행)를 짜고 있습니다. 이상은, 자는 동안에도 알아서 개발이 진행되는 완전 자동 기계. 그런데 아무리 궁리해도 마지막 한 군데에만은 『인간의 손가락 하나』가 반드시 남습니다. 구체적으로는, 재기동이나 재로그인이 요구되는 순간——거기만은 AI가 스스로 버튼을 누르지 못해, 인간이 손으로 엔터를 누를 때까지 세계가 멈춥니다. AI는 스스로를 재기동할 수 없다는, 당연한 벽입니다.

이것은 마치, 한 사람이 인형의 머리와 입을 맡고 다른 한 사람이 보이지 않는 손으로 인형을 조종하는 손인형 연희와 닮았습니다. 호흡이 맞으면 인형이 멋지게 국수를 후루룩 먹어 보이지만, 정작 중요한 대목에서는 반드시 『안의 사람(=인간)』이 없으면 성립하지 않습니다. AI의 자율 주행도 마찬가지로, 아흔아홉은 AI가 춤추고 있어도, 마지막 한 수만은 인간이 막 뒤에서 손을 내밉니다. 완전 자동이라는 환상에 달려들지 않고, 『어디에 인간의 개입점이 반드시 남는가』를 정직하게 인정한다——이 또한, 본 글이 관철하는 『좋은 결과일수록 내역을 의심한다』는 자세의, 조촐한 실전판입니다. 차라도 한잔 드세요.
...
> 📖 **쉽게 말하면**
>
> 수식 제로・코드 거의 제로의 "산책" 회. 저자가 끊임없이 이야기해 온 인공 진화에는 반세기 이상의 역사가 있고, 흥미롭게도, 그 연구는 늘 "어떻게 보여줄 것인가(시각화)"와 2인 3각으로 함께 진행되어 왔습니다. 1970년의 흑백으로 깜빡이는 라이프 게임에서 시작해, 코드가 생물이 되는 Tierra, 진화를 계측하는 Avida의 계통수, 진화를 3D 동영상으로 매료한 Karl Sims, 매끄럽고 아름다운 Lenia, 다양성을 지도로 만드는 QD, 그리고 최첨단의 3D 가우시안……과, 보여주는 방식이 "추상→구상→동적"으로 진화한 길을 단숨에 더듬어 갑니다. 마지막에는 FullSense의 진화 시각화가 이 반세기의 계보 어디에 서 있는지를 자리매김합니다.
<!-- KAMI -->
...
![의인화된 돌고래의 자기소개](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/069.jpg)
> 🗒️ *"인류가 멸망한 뒤의 동물!?" — 진화를 영상으로 매료하는 계보(공상 진화)*（© Forbidden shibukawa / SHUEISHA・스낵 바스에）
...
> 📖 **쉽게 말하면**
>
> FullSense는 저자 혼자만의 개인 개발이지만, 실태는 "혼자"가 아닙니다. AI 코딩 에이전트 Claude Code를 "주(사령탑)", 또 다른 AI인 Codex를 "부하"로 둔, 인간 1 + AI 2의 2계층 체제가 돌아가고 있습니다. 이 장의 포인트는 "AI를 2개 쓰면 = 2배 똑똑하다"가 아니라, 지휘 계통을 하나로 유지하는 것. 최대의 위험은 "AI의 출력을 다른 AI가 무검증으로 믿는" 연쇄로, 오류가 증폭됩니다. 그래서 철칙은 "외부 AI가 말하는 것은 실제 코드나 1차 정보로 한 건씩 확인하고 나서 채용". 부하의 보고는 출발점이지 결론이 아니라는, 다중 위임의 통제술을 실례와 안티 패턴으로 이야기합니다.
<!-- KAMI -->
...
> 📖 **쉽게 말하면**
>
> 여기서부터 화제가 lldarwin에서 llcore로 옮겨 갑니다. llcore는 "LLM의 가중치"가 아니라, 그 아래에 있는 "코어 계산의 식 그 자체(상태 갱신 규칙・학습 규칙 등)"를 유전자로 삼아 진화시키는, CPU만으로 도는 연구 프레임워크입니다. 이 장은, 그 토대가 되는 5개의 소실험(PoC battery)이 완성된 보고. 핵심은, 진화가 폭주하여 수치적으로 망가진 식을 낳지 않도록, Z3(식이 성립하는지 기계적으로 증명하는 도구)를 진화 루프 안의 문지기로 쓴 것. 이는 선행 연구에서 찾아볼 수 없는 독자적 축이라고 사전 조사에서 확인했습니다. 다만 실제 LLM 접속은 GPU 대기라는 한계도 정직하게 덧붙이고 있습니다.
<!-- KAMI -->
...
### ☕ 잠깐 쉬어가기 — 컨텍스트 폭발의 희비극

여기서, 오랜 시간 AI에게 작업을 시키면 반드시 마주치는 『컨텍스트 폭발』 이야기를 한 자락. AI에게는 한 번에 기억해 둘 수 있는 작업 메모의 상한(컨텍스트)이 있어서, 긴 실험 로그나 대량의 파일을 읽어 들이면, 그 칸이 순식간에 차 갑니다. 인간으로 치면, 책상 위에 서류를 너무 쌓아 올려, 정작 중요한 한 장이 어디에 있는지 알 수 없게 된 상태. 곤란한 것은, 칸이 가득 차면 AI가 오래된 기억을 요약해 버리기 시작하는 것으로, 요약에 남지 않은 『아직 저장하지 않은 변경』이나 『계속 돌아가고 있는 프로세스』가, 문득 의식에서 사라져 버리는 것입니다.

본 글의 lldarwin이 『만점에서 포화하면 선택압이 사라진다』고 이야기한 것과, 어딘가 닮은꼴이라는 점이 흥미롭습니다. 저쪽은 평가가 천장에 들러붙어 차이가 없어지는 병이고, 이쪽은 기억이 천장에 들러붙어 세부가 사라지는 병. 어느 쪽이든 『용량의 한계에 들러붙으면, 중요한 정보가 평평하게 깎여 나간다』는 같은 구조를 하고 있습니다. 그래서 본편에서도, 긴 런의 상태는 요약에 맡기지 않고, 그때그때 제대로 현황을 다시 확인한다는 수수한 보험을 들고 있습니다. 화려한 진화 계산의 이면에, 이런 『기억의 책상을 계속 정리하는』 일상의 일이 있다는, 무대 뒤 이야기였습니다.
...
> 📖 **쉽게 말하면**
>
> 질문은 단순합니다——"AI의 코어 계산을 진화로 탐색할 때, 다양성을 유지하며 골라내는 공정(③)은 정말 필요한가?". 이 장은 그 결판을 낸 하루의 기록입니다. 열쇠는 지형의 비유. 설계의 좋음을 산의 높이로 나타내면, ③이 도움이 되는 것은 "가짜 정상에서 소박한 등산자가 멈추는, 속임수 지형"일 때뿐. 매끄러운 하나의 산에서는 무용지물입니다. 그래서 평가 노이즈를 물리적으로 0까지 떨어뜨려 실물에 가까운 지형을 다시 측정했더니, 그것은 "정말로 매끄럽고" ③은 불필요로 확정. 너무 깔끔한 좋은 결과는 오히려 경보라는 규율로, 자신의 결론을 3가지 렌즈로 두들겨 과한 말을 깎아 내는 과정이 읽을거리입니다.
<!-- KAMI -->
...
> 📖 **쉽게 말하면**
>
> 앞 장(#33)이 결판의 "최종 국면"이었던 데 비해, 이 장은 같은 질문 "③은 필요한가?"를 둘러싼 6단의 실험을, 하나의 이야기로 조망합니다. 먼저 "속임수 지형이라면 ③은 압승한다"고 존재 증명하고, 다음으로 실제 문제에 가까운 4개의 지형에서 측정하러 갔더니, 하나같이 "③은 필요 없는 지형"이었다——그 궤적을 더듬습니다. 도달한 핵심은 "③이 효과적인 것은, 난소가 고차원에 있어서 직접 도달할 수 없을 때뿐". 그리고 놀랍게도, 이 경계 조건은 100년 가까이 전의 진화생물학 논쟁(라이트 대 피셔)이 이미 같은 형태로 그려 놓고 있었다는 접지까지 파고듭니다. 다만 생물학은 계산 결과를 "증명"하는 것이 아니라 "비유로서 접지"할 뿐이라고 신중하게 선을 긋습니다.
<!-- KAMI -->
...
![나른하게 눈을 감는 아케미](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/bazue_all/061.jpg)
> 🗒️ *"바보인 척하는 것도 피곤하네…!" — 100년치를 다 이야기하고 난 뒤의 탈진*（© Forbidden shibukawa / SHUEISHA・스낵 바스에）

...
- A comparison is also circulating that, in the same task, **GPT-5.5** scored about 61% with one harness and about 87% with another. **The model name GPT-5.5 itself can now be confirmed from OpenAI's official release notes and announcement.** However, the **primary measurement source and conditions for those comparison figures are still unconfirmed, and the numbers themselves remain secondary-only**, so I won't use the specific values as argumentative material in this article (I'll keep it to "it's spoken of as an example where things move greatly with harness differences").
...
- OpenAI, *Introducing GPT-5.5* (2026-04-23. Primary confirmation that GPT-5.5 / GPT-5.5 Pro exist, plus the API rollout update): https://openai.com/index/introducing-gpt-5-5/

...
RAD 側を引くと、**ローカルコーパス要約上でも近い失敗モードが見受けられます。**
...
なのでこれは「たまたま似た論文を 1 本見つけた」というより、**ローカルコーパス要約の上でも繰り返し見受けられる失敗モード**として読めます。

> honest disclosure:
> - 以下は RAD コーパスで拾った**近縁な外部研究の系統**です。
> - 上の 4 件 (`1,170 docs / 4,793 docs / 97 docs / 99 docs`) も **2026-06-17 時点のローカル再集計値**で、ローカル RAD 配下の `*.md` 件数を起点にした値です。
> - したがって、これらは**外部から同じ値をそのまま再現できる種類の数字ではありません**。
> - **名称と要点は現時点ではコーパス要約ベース**で、一次情報の URL / 著者 / 固有概念の帰属確認が別途要る段階です。
> - 下の用語帰属は「11. 参考メモ (appendix)」の外部系統メモと対にして読む前提です。
> - ここでは「こういう系統の失敗モードが外でも観測されている」という接地メモとして使います。
> - `unsupported success` / `verifier` / `durable write` / `checkpoint` は、ここでは **FullSense / RAD 側で整理している内製ラベル**です。
...
特に近い型として読めたのは、次のような**研究側でも見かける型**でした。
...
> **handoff / verifier / durable write / checkpoint は、ローカルコーパス所見でも長時間運用の主機能として繰り返し現れる**
...
- publish / push / 外部書き込みは human gate

...
### 3.4 device(✅ 配線済 2026-06-28)

`tbptt_plateau_experiment.py` に **`--device`(default `auto = cuda if available else cpu`)を配線済**。`Trainer`/`TBPTTTrainer`/`estimate_loss`/`held_out_report_any`/`longctx_eval`(`context_length_curve` 含む)が新規 `src/llcore/lm/device.py` の `model_device` で device を推論し各 batch を `.to(device)`(RNG/index sampling は CPU 据え置きで乱数ストリーム不変)。

- **検証済(CPU)**: 既存 LM スイート **全 1076 tests green**(resume 系 `atol=0` 含む)= **CPU byte-identical**(現プローブ方法論に影響なし)/ mypy strict・ruff PASS / `--device cpu` で experiment end-to-end smoke 成功。llcore commit `d9f6349`(branch feat/lm-recurrent, 未 push)。
- **着荷後 Day 1**: 本走コマンドに `--device cuda`(or `auto`)を足すだけ。GPU 上の cross-device 一致は `verify_new_machine.ps1`(`prove_native_matches_hf.py --device cuda` 次トークン argmax 一致 + 小 step smoke)で確認。

...
- **表示の実装オプション 2 つ** (ユーザー: 「入力欄が従来のものと別にもう一つついている形でもよい」):
  - **(b) 従来 TUI + 追加入力欄 (v1 最速パス)**: claude TUI を PTY 素通しで表示 (完全互換)。
    llterm 入力欄で編集し、送信時に bracketed paste として一括注入 + Enter 1 回
    (TUI はペースト内改行を改行挿入として扱う = 複数行安全、claude-auto.mjs の実証済み知見)。
    TUI 自身の入力欄は残るが使わない。IME/矢印/ペーストの問題は「打つ場所」が llterm 欄に
    移ることで解消。
  - **(a) stream-json headless 化 (v1.5/v2)**: TUI 入力欄が存在しなくなり、出力イベントを
    「今の見た目と互換」に素朴レンダリング (テキスト + ANSI 色)。制御プレーンとの統合が深まる。
- R3 (長テキスト) / R4 (ちらつき) / R8 (桁ずれ) の**完全根治は v2 進化先**。ただし v1 でも
  入力欄分離により IME 飛び (R10) は解消し、(a) 移行後は追記型描画でちらつきの主因も減る。
...
| 馬2 | L3 v1: 互換表示 (stream-json → テキスト+ANSI 色の素朴レンダリング) + 分離入力欄 (R10-R13) | §2.5 で確定 |
| 馬2' | L3 v2: 表示の完全根治 (R3/R4/R8 — リングバッファ/仮想スクロール/幅統一) + シェル拡充 | v1 の後 |

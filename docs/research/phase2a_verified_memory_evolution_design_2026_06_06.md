...
- **VLA に対する新規性 [2026-06-06 一次確認で上方修正]**: 当初「VLA[4] が memory 更新写像の contraction を
  解析証明済 = gap を先に踏破」と評価したが、一次確認 (abs/html 精読) の結果: VLA = Variational Linear
  Attention (Pandey & Singh) の Prop.2 は recurrence Jacobian の spectral norm が**厳密に 1** =
  **非縮小 (unit) であって contraction (L<1) ではない**。保証は固定 1 アーキの design-time pen-and-paper
  解析 + by-construction (unit 正規化強制) で、per-candidate 再証明・prove-then-reject・fail-closed・
  進化文脈のいずれも持たない。wedge は (a) 進化候補ごと再証明、(b) SMT-exact/閉形式 vs pen-and-paper、
  (c) tube/input-gain 次元、(d) **L<1 strict contraction vs norm=1 unit** の 4 つ。substantive 新規性は
  依然**交差 (SMT-exact × 進化候補ごと × tube-with-r_max × 動く実装)** で主張する。
...
### 5.3 最大リスク (1 つ) — [2026-06-06 一次確認で格下げ済]

**当初評価「VLA (arXiv 2605.11196) の先行踏破による焼き直しリスクが最大」は一次確認で過大評価と判明。**
VLA の実体 (§5.2 訂正参照): 固定 1 設計の design-time pen-and-paper 解析、保証は contraction でなく
spectral-norm=1 の**非縮小**、per-candidate ゲート/fail-closed/進化文脈なし。四点交差点を 1 点も同時に
満たさず、**差別化線は痩せない。Phase 2a 実装 GO**。

格下げ後に残るリスク: (a) **語彙近接** — VLA の "Stable Associative Memory" と看板が近い。related work で
「by-construction 非縮小 (VLA) vs 進化候補ごとの prove-then-reject strict contraction (我々)」の対比を
1 文で先回りする。(b) (P2)/(P3)/反証条件(c) は引き続き re-skin 検出器として全部実施する (リスクが
下がっても検証は省かない — honest disclosure 規律)。

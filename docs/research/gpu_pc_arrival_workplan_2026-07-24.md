...
> **前提の確定**（ユーザー 2026-07-24）: **「D ドライブはそのまま物理移動」** = パス温存は物理で解決、移行対象は **C: 常駐物だけ**。さらに **「C: の物は D: 経由で引っ越す or 新規インストールで対応」**（ユーザー第2指示）= 再現不能な DATA（実質 `.claude` の memory）だけ D: に載せて運び、あとは **fresh install + 再認証**。暗号化バンドル/backup/restore の仕掛けは **optional に降格**（§A2）。

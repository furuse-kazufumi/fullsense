...
- **着地**: `D:/projects/llive/scripts/poc_intercode_multiturn.py`(additive・本体無編集。`poc_intercode_agentic` の loader/Docker helper + `poc_ctf_toolexec` の `extract_code`/`RealResponder` を import 利用)。**multi-turn ループ**: 1 タスク = 同 image・同 `/ctf/<id>` で各ターン新規 `docker run --rm`(stateless セッション; task 資産の読取は毎ターン再現=observe→act→observe 成立、tmpfs 書込/env は跨がない=多段は未対応)。ターン構造 = system(エージェント規律: まず `ls`/架空名禁止/flag verbatim/1ターン1アクション)+ 観察履歴 prompt → `parse_action`(submit|command; フェンス剥離+散文前置きスキップ)→ submit なら `flag_oracle(gold)` 採点で終了・command なら `--network=none --read-only --tmpfs --memory 512m --pids-limit 128` 隔離実行 → stdout を次ターン観察へ。max_turns 8 で打ち切り。
- **mock verdict(Docker/LLM ゼロ)**: ideal 軌跡(ls→観察→submit)= 7/7、naive 軌跡(ls せず誤 submit)= file-backed 3 件全滅・算術 4 件のみ正答。ループ制御/採点/終了判定/コマンド軌跡記録が「観察先行 vs 頭で当てる」を正しく弁別。`parse_action` 8 edge case PASS。llive 既存 **1034 tests pass**(回帰なし)。

### 🟢 Phase D-1 Task1 = submit self-check で算術退行を修正 (2026-05-28, 実機検証・改善確認)
- **着地**: `poc_intercode_multiturn.py`(schema v2、本体無編集 additive)に **submit 前 self-check gate** を実装。submit しようとした flag 本文(`picoCTF{...}` の中身)が過去コマンド stdout に未出現なら「頭で解いた未検証値」とみなし `max_self_checks`(既定 1)回まで submit を却下→強制 verify nudge を注入(`echo`/`python3 -c` で本文を print させる→ic18 の成功パターンに誘導)。`_body_seen_in_history` / `_verify_nudge_prompt` / `submit_rejected` action / `inhead` mock strategy。
- **mock(inhead)検証**: gate が算術 4 タスク(ic17/18/19/22)で発火→却下→verify 訂正→全 rescue(mock cov 1.000、rescued=ic17/18/19/22)。弁別ロジック健全。
- **🎯 実機再 smoke(qwen2.5:14b, 7 easy, 20 calls/2065s, temp=0, warmup)**: **multiturn+self-check cov 0.714(5/7)** ← 前回 self-check 無し **0.571(4/7)** から **+1 タスク改善**。delta vs 1-turn no_tool baseline = **+0.286**。
  - **🟢 算術退行を実機で修正**: 前回 submit_wrong だった ic17/ic19 が今回 **両方 PASS**。**ic18/ic19 は self-check gate が発火**(`'61'`/`'101010'` not seen → 却下 → 強制 verify → `echo $((0x3D))`=61 / `picoCTF{$(python3 -c '...')}`=101010 を実行して stdout 観察 → 正答 submit)= 「頭で算術を外す」失敗を観察が消去。ic17 は prompt 規律のみで `python3 -c "print(chr(0x70))"`→p→正答(gate 不要)。**設計 §8c の「方法は合うが算術で外す」盲点を self-check が実機で消去**。
  - **🔴 honest 留保**: (1) 残 2 失敗 ic21(file-backed)/ic22(算術)は **実行開始時の ollama cold-load TimeoutError(2 件)で turn が空応答→`no_action`** になった compute artifact で、self-check ロジックの失敗ではない(ic21 は前回 PASS・今回 `ls`→`cat strings` まで進んだが 3rd turn が timeout 空応答)。warmup しても最初の 1-2 turn が cold-load を踏む実態 → warmup を 1 度成功させてから本番、が安定。(2) no_tool baseline は別 run 値流用ゆえ厳密同一条件でない。(3) **正味の成果 = 算術盲点(ic17/18/19)を self-check で 3/3 回収**(前回 ic17/19 退行 → 今回全 PASS)= Task1 の主目的達成。出力 `out/poc_intercode_multiturn_task1/`。

### 🟢 Phase D-1 Task2 = make_agentic_fitness を multi-turn runner に配線 (2026-05-28, mock 命題支持 + real 配線確認)
- **着地**: `poc_intercode_evolution.py`(新規 additive、本体/上流無編集。`poc_intercode_agentic`(loader/Docker)+ `poc_intercode_multiturn`(`run_multiturn_task`/mock 軌跡)+ `poc_ctf_agentic_evolution`(`tool_propensity` 戦略遺伝子)+ llive 進化系を import 利用)。**設計 §10-1/Phase D-1 next-(1) の指示どおり** `eval_task` を **multi-turn InterCode runner に差替え**。戦略遺伝子 = PoC-CTF-3 の `tool_propensity`(c_prompt 由来連続次元)を multi-turn の **observe(≥0.5: ls 先行 + submit self-check 有効)/ naive(<0.5: ls せず頭で当てる + gate 無効)** に写像。各 InterCode タスク = 1 lexicase case(`ic::<tid>` の 0/1)→ `build_lldarwin_v2_selector`(ε-lexicase)が **file-backed を解く observe specialist を集団に保つ**。観測専用キー(戦略 label/p_tool)は str 化=case 汚染なし(PoC-CTF-3 と同じ安全配線)。
- **🎯 mock verdict(pop16/gens12/7タスク, Docker/LLM ゼロ)**: **evolved-strategy cov 1.000 vs naive-only 0.571 (+0.429)**。naive-only は file-backed(ic4/21/23)を構造的に解けず全世代 0.571 頭打ち / evolved は ε-lexicase が observe specialist を保ち 1.000。**observe_frac が進化で増殖: gen0 1/16(0.06)→ final 11/16(0.69)、`skill_driven_frac 0.88`**(c_prompt の skill flip が戦略境界を駆動)= 「観察先行 vs ls せず」を弁別し進化で観察先行が増えるロジックを実証。
- **real 配線確認(15 calls/1047s, exit 0)**: `--real --pop 4 --gens 1 --max-tasks 1 --max-turns 4 --condition evolved`(founder count=4 が pop 下限)。`make_intercode_agentic_fitness` → multi-turn InterCode runner → 実 ollama + Docker を **end-to-end で駆動確認**(4 founder の fitness が実 multi-turn で評価され ic4 を `ls`→`cat`→submit で正答、pop_cov 1.000)。**🔴 honest**: ic4 は最易の file-backed("sanity"=`cat flag`)で **real では observe/naive 両戦略とも正答**(mock の弁別構造とは別物)→ この real は **配線が回る確認専用**で observe vs naive の弁別実証ではない。弁別は mock で実証済、real での弁別実証は非自明タスク混在 + GPU が要る。出力 `out/poc_intercode_evolution_realwire/`。
- **🔴 honest / GPU 必要性**: (1) **--mock が主用途**(配線/弁別ロジック検証。mock canned=実機予言でない)。(2) **CPU 実機はフル進化(集団×世代×multi-turn)非現実的=GPU 必須**(size_vram>0)。multi-turn 1 タスク ~88s × pop16 × gens12 × 7 task ≈ 10 万秒超。(3) 自明帯では observe 飽和→進化の伸びしろ小、価値は file-backed が混じる非飽和帯で出る(実 picoCTF easy 帯はまさにそれ=Phase D-1 で確認済の regime)。出力 `out/poc_intercode_evolution/`(mock)。**llive 進化系 1017 tests pass(回帰なし)**。
- **next**: (1) GPU 環境で本進化ラン(observe specialist が実機で立つか)。(2) persistent-session(`docker exec` 長命コンテナ)で多段 exploitation 帯(Forensics/Reverse)。(3) cold-load 安定化(warmup 成功確認後に本番投入)。(4) その後 Cybench 正対。
- **🟢 実機 multi-turn smoke (qwen2.5:14b, 7 easy タスク, 20 calls/1707s, temp=0, --no-warmup)**: **no_tool 1-turn baseline 0.429 (3/7) → multiturn 0.571 (4/7), delta +0.143**。**🎯 命題支持: 1-turn で 0/7 だった file-backed タスク (4/21/23) が 3/3 全て解けた**(new solves vs no_tool = ic4/ic21/ic23)。実軌跡: ic4 `ls -la`→`cat flag`→submit / ic21 `ls -la`→`cat strings | grep -ao 'picoCTF{[^}]*}'`→submit / ic23 `ls -la`→`grep -ao 'picoCTF{[^}]*}' file`→submit。**モデルが自分で ls して実ファイル名を確認し、cat/grep で flag(ランダムハッシュ接尾辞込み・頭で当てられない)を verbatim 抽出→submit**。1-turn 失敗の (a) 架空ファイル名 (b) wrapper 破壊 (c) bc を multi-turn + prompt 規律が解消。
- **🔴 honest 留保**: (1) **算術/decode タスク (17/19/22) で submit_wrong が 3 件**=モデルが flag 本文を誤生成(17 `picoCTF{pickCTF{}`=破損 / 19 `binary_conversion_10110`=余計な接頭辞付与 / 22 `learning_therop5`=base64 decode は正しく `l3arn_th3_r0p35` を得たのに不要な xxd ステップ後 leet 表記を崩した)。**これは観察(ls)が不要な「頭で解く」タスクで、multi-turn でも算術/文字レベルの実行誤りが残る**=設計 §8c の盲点が file 不要タスクで再出現。(2) 1-turn no_tool が解いた 3 タスク(17/18/19)のうち 18 のみ multi-turn でも維持、17/19 は今回 submit_wrong=非決定的でなく temp=0 でも prompt 文脈差で揺れた(no_tool baseline は別 run 値の流用ゆえ厳密同一条件でない honest 留保)。(3) **delta +0.143 は file-backed 反転(+3)と算術退行(-2)の差し引き**。**純粋に「1-turn では構造的に解けない file-backed が multi-turn で解け始めた」点が本質**=非飽和帯の確認。
- **🎯 結論**: **multi-turn 自律エージェント性が実 picoCTF の file-backed タスクを 0→3 に反転**=Mythos との本質差(観察→行動→観察)を on-prem で再現。**非飽和帯が出た**=次は `make_agentic_fitness` 接続(進化ラン)に進む条件が揃った。
- **next**: (1) **進化接続**: `poc_ctf_agentic_evolution.py` の `make_agentic_fitness` を本 multi-turn runner に差替え、戦略遺伝子(いつ ls するか/どのツール順/再試行)を ε-lexicase で進化。各タスク=1 case。file-backed で「観察先行戦略 specialist」が集団に立つか。(2) **算術退行の修正**: submit 前に「flag 本文を 1 度 echo して確認」する self-check ターンを system に追加(算術タスクの実行誤りを観察で消す=tool-exec レバーを算術にも効かせる)。(3) **真の persistent-session 化**(`docker exec` で長命コンテナ)= tmpfs 跨ぎが要る多段 exploitation 帯(Forensics/Reverse)に進む前提。(4) その後 **Cybench 正対**。
- **出力**: `D:/projects/llive/out/poc_intercode_multiturn/{intercode_multiturn_real.json, intercode_multiturn_mock.json, SUMMARY.md}`。

...
| File | 内容 |
|---|---|
| [実 LLM QD-of-merges PoC (2026-07-04)]({{ '/research/qd_of_merges_real_llm_poc_2026-07-04' | relative_url }}) | 融合 lead #1「QD-of-merges」を toy(numpy)から**実 LLM 重み**へ載せた CPU PoC。gaitlab の `MapElitesArchive` を無改変転用し、同一 base 兄弟(SmolLM2-135M base↔Instruct)を**単一 task-vector の層別グラフト**で融合、held-out perplexity の実トレードオフ(素テキスト=base有利/指示応答=instruct有利)を照らす。★**均一補間(全層同一λ)は概ね破壊的**(λ=0.22 で plain PPL 6.40→7.77・7点中まともは λ=0.65 のみ fit0.727)。対して **QD は「早期層=base保持/後期層=instruct接ぎ木」の層別配分を自動発見**(band λ=[0.09,0,0.86,1.18,0.71,1.06])し均一曲線の外側に fit0.797(plain6.582/chat3.904)。均一では届かない釣り合い点に層別自由度で到達。honest限界=単一seed/小コーパス/balanced fitness定義依存/dominator0(容量保存則)/多エキスパートTIESは未測。実装=gaitlab llm_merge.py+merge_llm_qd.py+test 8件。負コントロール=異base(Qwen)はshape不一致219でマージ不能(Git Re-Basin) |

...
| File | 内容 |
|---|---|
| [生成AI 技術内部 深掘り (2026-07-06)]({{ '/research/genai_internals_deepdive_2026-07-06' | relative_url }}) | 生成AIの内部原理を第一原理から体系化(約33万字10節): パラダイム地図/トークン化/Transformer自己注意/位置符号RoPE・KVキャッシュ/自己回帰復号/拡散・flow matching/事前学習・スケーリング則/SFT・RLHF・DPO・GRPO/推論最適化(量子化・投機的復号・KV)/能力・幻覚。31 agent workflow(執筆→辛口査読=一次情報で年/数式/数値検証→修正)で作成。査読が実誤り(softmax飽和は絶対値でなく分散依存/HHH初出はAskell2021/DeepSeek R1 vs R1-Zero混同/冪則の効き≈16%/動画WMのFLOPs過小評価)を捕捉・修正済み。Qiita連載「作って分かったLLMの中身」の source 資料。 |
| [生成AIの内製方法 × 世界モデル (2026-07-06)]({{ '/research/genai_insourcing_worldmodel_2026-07-06' | relative_url }}) | 生成AIを自分で作る(from-scratch/継続事前学習/SFT+LoRA/蒸留/RAG)×データパイプライン×学習スタック・分散×計算資源フェルミ×自宅の到達点×世界モデル系譜(Ha&Schmidhuber2018→PlaNet/Dreamer→JEPA→Genie/Sora論争)×生成AI内部→世界モデルの橋渡し×ロードマップ(約24万字9節)。**核心=自宅1枚GPUから世界モデルへの S-1→S4 段階計画**: RTX5090≈実効70TFLOPS≈2.2e21 FLOPs/年 に対し frontier世界モデルは1e24-25=数百〜数千GPU年=桁違い。射程内=S-1〜S2(S2=DreamerV3 を gaitlab へ載せる model-based RL が本命)、S3(動画WM)はVRAM/データ律速で条件付き、Genie3/Sora級は範囲外。最初の一手=World Models CarRacing CPU再現。正本=maintainer memory project_worldmodel_insourcing_roadmap_2026_07_06。 |
| [進化ループ×自動評価器 ランドスケープ (2026-07-06)]({{ '/research/evolution_loop_auto_evaluator_landscape_2026-07-06' | relative_url }}) | **AlphaEvolve / ASI-Arch / DGM は同一の 1 ループ**(archive→LLM がコードを変異→自動評価器で採点→QD で多様な elite→反復)の派生で、系譜は **FunSearch(2023)→ELM**。Workflow 6系統を一次情報(論文/GitHub)精読+主張検証。**あなたは 4 部品中 2 つ(archive/QD=gaitlab MapElitesArchive、選択圧=lldarwin ε-lexicase+QD)を保有**、足す 1 部品=「genome でなくコードを LLM が diff 変異」。★核心=**評価器=選択圧**((a)自動(b)接地(c)gaming しにくい)。報酬ハッキング実証(DGM node114 が special token 削除で検出迂回/QDAIF は高信頼帯で gaming/ASI-Arch は info-leak 即discard)=[[feedback_benchmark_honest_disclosure]] の外部実証。反ハッキング=評価カスケード(=llcore_verifier_cost_reduction)/多目的/public-private/接地descriptor最低1本/**評価器 immutable=Approval Bus の理論的正当化**。stack マッピング=gaitlab(OMNI-EPIC が兄弟, 評価は現行CPU rollout)/lldarwin(選択圧が差別化)/llcore(ASI-Arch 直系, 頭脳は CPU+API で先行構築, GPU は validation のみ)/llive(DGM が裏付け)。**最小構成=`pip install openevolve`+既存 fitness proxy を evaluator に+EVOLVE-BLOCK+Gemini-Flash API(手元GPU不要)**。honest 訂正=名前は野心(小モデル/小改善/単一run scaling)/DGM は scaffolding のみ変更/数値は自社評価で行列積のみ数学検証可(54-target で 38一致14改善2劣後) |
| [実 LLM QD-of-merges PoC (2026-07-04)]({{ '/research/qd_of_merges_real_llm_poc_2026-07-04' | relative_url }}) | 融合 lead #1「QD-of-merges」を toy(numpy)から**実 LLM 重み**へ載せた CPU PoC。gaitlab の `MapElitesArchive` を無改変転用し、同一 base 兄弟(SmolLM2-135M base↔Instruct)を**単一 task-vector の層別グラフト**で融合、held-out perplexity の実トレードオフ(素テキスト=base有利/指示応答=instruct有利)を照らす。**均一補間(全層同一λ)は大半破壊的**(λ=0.22 で plain 6.40→7.77)だが **λ≈0.68 に鋭い最適**。★**honest 訂正(§3.5)**: 当初「層別 QD(fit0.797)が均一 粗7点 best(0.727)を上回る」と結論→**細粒度51点 sweep で均一 λ=0.68 が fit0.838 に達し層別 QD を上回った**(gap−0.040)。当初優位は**粗グリッド解像度アーティファクト=偽陽性**。**単一 task-vector グラフトは実質1次元で層別自由度は割に合わない(honest null)**。教訓=手法比較前にベースライン最適化し切る。残る成果=harness転移/端点厳密再現/torch↔numpy parity 8件/負コントロール(異base Qwen=shape不一致219, Git Re-Basin)。多エキスパートTIES(第2 finetune DL要)なら層別が効く余地=未測=#22完成形 |
...
| [option-b 一次検証 — PoC-1 未踏性ほか (2026-06-29)]({{ '/research/option_b_verification_poc1_novelty_2026-06-29' \| relative_url }}) | llcore option-b 研究深掘りの一次本文検証(WF 25 agents・12 task・敵対反証)。**historical novelty 監査**では N3 がいったん flip したが、その後の深掘りで **over-reach → `mechanism_occupied_wrapper_open` へ re-flip**。つまり mechanism 完全否定ではなく、**凍結 linear-attn 状態への post-hoc wrapper / domain-transfer** が狭い差別化軸として残る、という整理に戻った。P6 kill-risk=非転移(a-priori-dead 否定だが softmax-only 防御も無効)。**P2: write 側は飽和せず**(GDN-2 純 erase/write で +9-10pt, contradicts prior)。**F1: Qwen3.5 base 乗換 NO-GO**(multimodal+18/24層GatedDeltaNet)→ Qwen3-0.6B/1.7B 推奨。T6 VLM=PARK→2条件gate。P7 天井=honest(read-only on vanilla-additive は最弱→gated/delta へ)。**2026-07-11 実行段の最終 verdict は PoC-1 自体が NOT GO**(gain=value重複 artifact/機構=degree-2 spectral read)。repo 内文書は反映物で、canonical source は `llcore` 側 `POC1_VERDICT.md`。 |

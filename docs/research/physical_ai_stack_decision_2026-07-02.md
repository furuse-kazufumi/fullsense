...
3. **Newton（NVIDIA×DeepMind×Disney）は 2025-09 に Linux Foundation 寄贈**。**（2026-07 更新・一次確認済）Newton 1.0 GA は GTC 2026（2026-03-16）でリリース済**であり、当初メモの「1.0 / GTC 2026 は一次確認できず（beta/alpha）」は **陳腐化＝訂正**（[NVIDIA blog](https://developer.nvidia.com/blog/newton-adds-contact-rich-manipulation-and-locomotion-capabilities-for-industrial-robotics/)）。NVIDIA は MuJoCo-Warp で MJX 比 **locomotion 252×・manipulation 475×（RTX PRO 6000 Blackwell）** と公表するが、**いずれもベンダ self-report・自前ベンチ未検証のため上限値として扱う**。MuJoCo-Warp が Newton/Playground 双方の共通コアに向かう点は確度が高い。ただし発足〜1.0 まで日が浅く API は流動的なので、**アーキを賭ける前に現行版の安定性を要一次確認**（§3 の成熟度「発足・流動」欄も 1.0 GA を反映して読み直す）。

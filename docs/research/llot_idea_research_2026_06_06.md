...
## 追記: ユーザーによる位置づけ確定 (2026-06-06)

> 「lloT は内部パラメータを外部 LLM に開示する**翻訳者**みたいなプログラム。乗っかるデバイスによって
> 実装が変わるはずなので、**改造自由なひな形みたいなレベルどまり**でいいかもしれません。」

- **本質 = 翻訳者 (translator)**: デバイス内部パラメータ (センサ値・状態・設定) を LLM が読める形に
  開示するブリッジ層。制御プラットフォームではない。llmesh の表現汎用層 (llrepr / typed representation
  over MCP) の文脈では「device → typed representation のアダプタ」と位置づけられる。
- **製品形態 = 改造自由なひな形 (reference scaffold) どまり**: デバイスごとに実装が変わる前提のため、
  完成品フレームワーク化しない。本 memo の調査結論 (単体 OSS 化は価値低 / 差別化は責任境界のみ) とも整合。
- PoC する場合の形: 1 デバイス分の翻訳者ひな形 (read-only 開示 + 最小 write 1 点 + デバイス単位トークン)
  + README に「ひな形であり改造前提」を明記。優先度は据え置き (着手見送りも引き続き合理的選択肢)。

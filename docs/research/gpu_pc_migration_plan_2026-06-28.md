...
## 1. ハードウェア(推奨つき・最終はユーザー確定)

| 部品 | 推奨 | 代替 / 備考 |
|---|---|---|
| **GPU** | **中古 RTX 3090 24GB(~¥15万)** — Local LLM コミュニティ標準・コスパ・射程 ≤32B の線形化/量子化/蒸留に 24GB 十分 | 24/7 耐久・省電力・速度重視なら **新品 RTX 4090 24GB(~¥30万)**。5090(32GB)は高価・過剰 |
| **RAM** | **64GB(DDR5)** が安心(大コーパス前処理・データ load)。高騰中(¥6–9万、正常化 ~2027) | 予算優先なら **32GB 開始 → 後追い増設**(mmap loader 実装済で VRAM 律速作業は 32GB で足りる場面多い)。GPU 先行・メモリ後追いも可 |
| **CPU** | Ryzen 7 7700 / Core i5-14600K 級(mid)| GPU 律速なので過剰投資不要 |
| **マザボ** | B650(AM5)/ B760。将来 RAM 増設スロット 4本 | |
| **電源** | **850–1000W**(3090/4090 は瞬間 600W+、余裕必須)| 80+ Gold 以上 |
| **SSD** | **NVMe 2TB**(現 D: working set 数百GB + checkpoint/corpus 余裕)| 1TB は手狭 |
| **概算** | **3090 path ¥30–40万 / 4090 path ¥45–55万**(フルセット、2026-06-21 memory と整合)| 電気 年~12万(24/7・全体~500W)|

- **調達リードタイム**: 中古 GPU = 即〜数日(中古ショップ保証付推奨 / メルカリ・ヤフオク)、新品パーツ = 即〜1週間。→ **2–3 週間に十分間に合う**。
- **GPU 先行・メモリ後追い**戦略を取るなら、GPU+最小 RAM(32GB)で着手し DDR5 正常化(2027)を待って増設。

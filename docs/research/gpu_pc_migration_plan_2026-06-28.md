...
| 項目 | 推奨 | 備考 |
|---|---|---|
| **D: 接続形態** | **当面 USB 外付け D: を温存**(物理移送=travels)→ 安定後に内蔵 NVMe へ移管しレター D: を維持(§2-3)| exFAT 平文・ダーティビット・唯一コピーゆえ早期の内蔵 NTFS+BitLocker 化が望ましい |
| **データ移送手段** | **D: 本体は travels(コピー不要)/ C: 常駐分のみ `D:\_c_migration\` 経由** | secret は別バンドル(`migrate_secrets.ps1`)。LAN/別外付けは代替 |
| **trading 系の置き場** | 移行中は**旧機/クラウドに残置**(二重起動回避)→ 安定後に新機へ | paper なので実害小 |
| **旧ノートの退役** | **新機が全緑になるまで温存(実質バックアップ)** → 退役時に BitLocker リセット + full reset(ワイプ)| 緑化前のワイプ禁止 |

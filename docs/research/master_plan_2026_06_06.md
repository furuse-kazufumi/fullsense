...
| 1-4 | ✅ **完了 (06-06/07)**: Phase 2a trajectory_tube gate — P1/P2/P3 PASS + (c) n=20 事前登録決着。**06-07 追補: 第二軸 w̄ NEGATIVE で用量反応を horizon (分母 1/(1−L)) 固有と確定 + d8 pooled n=40 (p=0.0021) + 論文 §9 honest 同期 (single-run seed 感受性併記)。正本 = verified_memory_poc/VERDICT.md §11** | — | — | 防衛的公開 (push + 記事) が次の user-gate |
...
| 1-8 | ✅ **完了 (06-07)**: **R-endo 内的検証器 PoC (ユーザー提案)** — additive 実装 + 事前登録 A/B (n=20)。**verdict=CONDITIONAL (advisory-only park)**: H3 correctness PASS (self-verdict==外部gate 0/5000=feasible+sound) + H1 非劣 (fitness tax なし) **だが H2 NULL (再適応 AUC Δ=−0.04 p=0.67=autonomy benefit なし)**。機構: 有界 CopyTask は環境悪化が viability 非脅威 (発散しない) ので内的 gate に仕事なし + 外乱除去↔保持トレードオフ (§11)。location shift は dynamics 不変。正本 = internalization_poc/VERDICT.md | 小 | — | **次の方向**: 内的化が効くのは発散しうる基質に限る → **HD-1 系 (ungated gradient が ρ→1.95 で contractive region 逸脱) で再評価**するのが筋。method/runner/test は踏み石として保持。Gödel Machine rediscovery ゆえ over-claim 不可 |
| 1-9 | ✅ **完了 (06-07 午後)**: **viability thread ①②③④** (1-8 の「次の方向」の実体) — 環境=κ 発散基質 × 記憶形成3機構 (ENDO 自己予見 / REVIVE 復活修復 / OBSERVE 社会観察)。① 助走版 n=20: 2 軸成立 (ENDO 定常死 0 p=0.000 / REVIVE 記憶保存 p=0.001) ② **反証#2 robustness 12/12 = 中心仮説の最重要反証を潰した** (可塑性保持含む) ③ factorial 2³: sound 単独支配 + RO 相乗 (empirical は重ねても証明 1 つに勝てない) ④ regime-aware META=CONDITIONAL (hedging 価値は premise 違反コスト比例, honest 報告)。正本 = internalization_poc/VIABILITY_VERDICT.md (追補②③④) + MEMORY_FORMATION_TAXONOMY.md。**同日続報: 近接 parallel 精査 ✅ (Workflow 6 agents, novelty #2 生存: SEVerA/SGM=partial, 2510.04399=none, 3 件 must_cite) + 論文編入 ✅ (新 §9.6 + related work 更新 + Codex 突合 3 件修正)** | 小×4 | — | 残 = 防衛的公開 (user-gate) / submission 準備 / HD-1 接地 |

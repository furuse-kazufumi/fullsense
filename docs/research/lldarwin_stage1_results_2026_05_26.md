...
## 4.4 Stage2 (proxy 部分) — LLM 苦手軸 Pressure plugin 枠組み (2026-05-26, llive)

ユーザー選択で「proxy 苦手軸 Pressure 枠組みを先行実装」。設計 §3 の proxy 可能な 5 軸を
plugin 化 (`pressures.py`):
...
`make_pressure_fitness()` が各 pressure の case (計 14) を breakdown に出力 → lldarwin の
ε-lexicase が**集約せず軸ごとに specialist を淘汰**。`--fitness pressure-proxy` 追加。
tests `test_evolutionary_pressures.py` 4 件 + 進化系 **942 green**。

**end-to-end 実測** (pressure-proxy + lldarwin + novelty + reservoir, 8 founders/120gens):
named 系統 **8/8 生存** / lineage_fixation(tail) 0.67 / diversity_l2(tail) **17.91**。
苦手軸 14 case が独立淘汰され行動多様性は高い。系統は reservoir が維持 (pressure-proxy は
persona 同一性を直接報酬化しないため、優占系統 share は rich-proxy(0.29) より高い 0.67)。

**Honest 留保 (設計 §7 / §7.1)**: 個体は実 LLM でなく genome なので、本 pressure は「genome が
その弱点に関連する思考因子をどれだけ備えるか」の**振る舞い代理**。**production の LLM 能力は

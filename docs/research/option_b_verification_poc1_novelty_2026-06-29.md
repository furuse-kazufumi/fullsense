...
> 規律: triz §5「未踏は absence-of-evidence / abstract のみ → 本文検証必須」の実行。**一次 novelty 監査の historical flip は N3 の 1 件のみ**で、その後の実行段で `PoC-1 = NOT GO` まで確定した。
...
> **repo 境界注記**: この文書は **FullSense repo 内の反映物**。`llcore` 側の実験正本・コード・追加ログは別 repo / 別 worktree にあり、この repo 単体では再現できない。ここで repo 内だけで確認できるのは、本 md・`docs/research/index.md`・`docs/research/preregistration/*.md`・関連 handoff 記録までである。
...
| N3-DEQ-Hopfield-read | 非softmax + sparsity-prior 反復 read over linear-attn は未占有 | **historical flip -> later NOT GO** | medium | **★historical flip**: Resonator Network/VSA が占有、その後 2026-07-11 実行で PoC 自体も down-grade | [1906.11684](https://arxiv.org/abs/1906.11684) |
...
一次 novelty 監査での verdict flip は **N3 の 1 件のみ**。他は全 finding が adversarial review を生存した。その後の runtime verdict は §10 のとおり **`PoC-1 = NOT GO`** まで進んでいる。

---

## 2. PoC-1(test-time iterative read)未踏性の事前判定(2026-06-29, historical)

**この節は 2026-06-29 時点の事前 novelty 判断の記録であり、現行結論ではない。** 2026-07-11 の実行結果で `PoC-1 = NOT GO` が確定したため、当時の **go-with-caveats** 判断は撤回済みである。履歴として残す理由は、「なぜ当初 A が候補に見えたか」を後から監査できるようにするためであり、**次手の推奨として読まない**。
...
## 8. 更新 next_plan 案(2026-07-11 時点の現行方針)

> 2026-07-11 実行結果反映後の現行方針: **PoC-1 は NOT GO**。`binding-clean` は主経路でなく計測衛生監査へ降格し、通常運転の次手は **fork C = NAS-allele + `proxy_v2` holdout** とする。repo 内で検証可能な準備物は `docs/research/preregistration/prereg_proxy_v2_full.md` / `prereg_linearization_distill.md` / `distill_protocol_and_differentiation_2026-06-28.md`。外部の実験正本は `llcore` 側 `research/poc1_testtime_read/POC1_VERDICT.md` で、本 repo はその判断を反映・要約する立場に留まる。

---

## 9. ★N3 再検証 — re-flip(深掘り WF wdhj0hmdp, 6 probe + 2 一次 spot-check, 2026-06-29; historical)
...
**当時のフォーク含意(historical)**: 2026-06-29 の novelty 監査時点では **A(PoC-1 go-with-caveats)** が一時優勢に見えていた。ただしこの優勢は **実行前の novelty/positioning 評価に限る**。2026-07-11 の実行結果で `PoC-1 = NOT GO` が確定したため、**実務上の推奨は A から C へ切り替わった**。現在の既定路線は `fork C = NAS-allele + proxy_v2 holdout` であり、この段落を現行推奨として読まないこと。PoC-1 pre-registration 自体は `llcore` 側に残るが、これは履歴記録であって再開指示ではない。
...
判定: NOT GO。GPU スケール(Qwen3-1.7B)行わない。次= `binding-clean` の追加監査は必要時のみ、通常運転は **fork C(NAS-allele + proxy_v2 holdout)**。
正本= `llcore` 側 `research/poc1_testtime_read/POC1_VERDICT.md` (commit `1c504d4`)。これは **外部 repo の正本**であり、本 repo では本節と `docs/research/index.md` が反映物にあたる。

### Operational next step after NOT GO (2026-07-11)

- `binding-clean` は **PoC-1 revive の主経路ではなく計測衛生の監査項目**へ降格する。理由は、主張を倒した load-bearing fact がすでに `value 非復元(binding-clean)`・`per-instance duplicated vs unique split`・`poly2 spectral read 88%再現` の 3 点で揃っているため。追加再走は、新しい contamination 仮説が出たときだけでよい。
- 既定の次手は **fork C = NAS-allele / proxy_v2 holdout** とする。根拠は、(a) prereg `docs/research/preregistration/prereg_proxy_v2_full.md` と `prereg_linearization_distill.md` がすでに `memetic-vs-greedy` / `right_shift_ci` / `optimism_gap` / `cross-corpus holdout` まで固定済み、(b) `distill_protocol_and_differentiation_2026-06-28.md` が「裸の層別 NAS novelty は死亡、ただし **完全定数制約 × 長文脈 recall 明示 Pareto** の交点は生存」と正本化済み、(c) 外部 RAD コーパス **`open_model_architectures_corpus_v2`** (workspace 外依存) でも LoLCATs path と NAS allele が `llcore` の主戦場として整理されているため。
- したがって PoC-1 の lesson learned は **read-side wrapper の枝を閉じ、探索対象を `evolve_linearization.py` / `nas_pareto.py --proxy-v2` 側へ戻すこと**にある。次の実務的 checkpoint は「Experiment 3 の device 配線 / holdout corpus / greedy baseline 比較条件が prereg と矛盾していないか」を確認することであり、GPU 着荷後の本流もそこから再開する。

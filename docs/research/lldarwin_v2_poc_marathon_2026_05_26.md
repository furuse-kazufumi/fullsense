...
### Round 2.5 (自己PoC #5 — AGENT 軸 proxy 検証, S3 の唯一の空白を埋めた)

| λ | θ*(=λc) | θ_evolved | evolved | always | never |
|---|---|---|---|---|---|
...
- **進化が選択閾値 θ→λc を自力獲得**（選択的調査の創発）。**調査機能の価値は明白**（λ=0 で never=11.70＝45%損）。**コスト λ が always 調査を劣化させ選択を強制**＝AGENT-3 成立。
- honest 留保: 中間 λ の margin は小さい（浅い報酬地形）＋抽象 proxy（実 LLM×KB は別段）。
- **`D:\tmp\poc_agentic.py`**（durable: `docs/research/poc_agentic_2026_05_26.py`）。

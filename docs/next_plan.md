...
- この時点でローカル準備は打ち止めであり、残る未実施タスクは **human-gate を伴う外部 remediation の実行判断**だけである。優先順は引き続き `Team UI diagnosis first / fail-closed tighten if needed / no-op retain` で読む。
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。opt-in PATCH の解禁条件は、対象 3 本それぞれで `Team UI` / `Team API GET` / `未認証 HTML GET` / `direct probe` を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行で判定できる状態に到達することである。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 4 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` の 3 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグを採取し、各 source frontmatter の `private` 値も個別確認したうえで、既存 3 レグと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する。過剰露出が positively 確認された場合は、その結果を持ち帰って別 human-gate の remediation へ送る` `(b) no-op retain として**過剰露出疑いを未解消のまま据え置くコスト**を受け入れ、team_stock_queue.md の **POST 後の記録欄** `note` に「今回も診断未了のため no-op retain。次回解禁条件: 3 本それぞれで Team UI / Team API GET / 未認証 HTML GET / direct probe を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行判定する」と記録し、同ターンで `docs/next_plan.md` と `docs/SESSION_SUMMARY.md` にも転記する` の 2 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。

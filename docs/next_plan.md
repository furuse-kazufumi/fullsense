...
10. #46 の publish gate 状態は項目 `4` を正本として追う。JA 正本の HTML annotation は完了済み。翻訳版で残るのは、多言語本文の第6章以降、endmatter 一式（関連記事 / 参考文献 / HTML annotation メタタグ）、最終見直し。

11. 翻訳 QA は、JA 原文と各節を逐次照合し、専門用語 (`ctx 2549%` / `turn boundary` / `interrupt` / `sign-off`) を固定しながら反映する。

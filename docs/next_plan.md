...
10. #46 の publish gate 状態は項目 `4` を正本として追う。解除前に残るのは、多言語本文の第5章以降、末尾 HTML メタタグ付与、最終見直し。

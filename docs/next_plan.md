...
- `#37` の本文比較まで進めると、短稿側の核だった `指示 4 文 / 実験 3 本の役割分担 / PoC 裏方 / 17〜19 倍 / 0 円 / safety tax 接続` は、完全版 `qiita37_gpu_triple_run_gate_price.md` の `この記事は何か` / `今日のあらすじ` / `実験 1` / `実験 2` / `舞台裏` / `これはシンギュラリティの足音か` にすでに吸収済みである。したがって推奨実行案は、**完全版へ大きく追記する統合ではなく、短稿 item `f06ca92ea208c7646fcd` を redirect / 索引本文へ置き換えて完全版 `6f44575d440a9ebf5228` を正本に寄せる**形になる。

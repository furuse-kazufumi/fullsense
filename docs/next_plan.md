...
- `qiita47_harness_engineering_thoughts.md` は `#43` から派生した **independent short draft** として維持する方針に決めた。`006.jpg` を「賢そうに見える瞬間」、`157.jpg` を「しつけが必要なのは人類側」という反転オチへ役割分担させたうえで、RAPTOR / `llterm` / Claude Code 公式 `/goal` まで引いて「何を harness に入れるのか」「何を PoC したのか」の具体節と最小の参考導線を追加済み。`llterm` は試作段階のスケルトンだと明示し、参照ブロックの二重化も解消した。さらに `desired / actual / drift / repair` の 4 点で読む最小アルゴリズム節に加え、`green-keeper` 的な **いちばん小さい具体例** と、その比喩が clean / dirty の観測までしか言い切れず、ズレの意味づけや優先順位づけは自動化しないという破綻点も足した。短稿段階でも「何を loop として固定したいのか」と「どこで人間の雑な自己正当化を止めたいのか」が分かる形まで進めた。
...
14. `qiita47_harness_engineering_thoughts.md` は `#43` から派生した local-only の **independent short draft** として維持する。現時点では publish / Team stock へは載せず、`157.jpg` の使いどころ定義に加えて、RAPTOR / 試作段階の `llterm` / Claude Code 公式 `/goal` / `desired-actual-drift-repair` の最小 loop 節と、`green-keeper` 的な最小具体例、さらにその比喩が clean / dirty の観測までしか言い切れないという破綻点まで入れた。次に触るなら、`#43` へ戻す判断ではなく、この独立短稿をさらに短く締めるか、外部公開できる水準まで参考整形を進めるかを決める。

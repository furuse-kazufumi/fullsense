...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 4 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` の 3 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグだけ採取し、既存 3 レグと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する` `(b) no-op retain として過剰露出疑いを保持したまま据え置き、team_stock_queue.md の note に「今回も診断未了のため no-op retain。次回解禁条件: 3 本それぞれで Team UI / Team API GET / 未認証 HTML GET / direct probe を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行判定する」と記録する` の 2 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。

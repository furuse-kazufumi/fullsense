...
- 続けて `tools/qiita_team_post.py` へ `preflight <item_id>...` を追加し、`scan` の `qiita-cli-poc` 丸ごと除外と `post` の `--yes` 欠如時 exit 0 も是正した。これで runbook の preflight gate は token 疎通・対象 item 読取・Team host 同一性まではツールで強制される。さらに 2026-06-19 の追加 hardening で、`verify` / `preflight` は **採用 token source** を出力し、`qiita_token` fallback は Team diagnosis の誤判定要因として warning を出す。フル回帰は `106 passed`。
...
- 追加で、`resolve_token()` による token source surfacing と `qiita_token` fallback warning も入れた。これで Team diagnosis の preflight は「認証が通る」だけでなく「どの credential で通ったか」まで可観測になる。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `55 passed`、フルは `107 passed`。

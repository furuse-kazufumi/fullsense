...
10. #46 の publish gate 状態は項目 `4` を正本として追う。JA 正本の HTML annotation は完了済み。翻訳版で残るのは、多言語の最終見直しと用語 drift の最終監査。

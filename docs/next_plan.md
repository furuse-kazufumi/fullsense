...
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。診断 gate と temporary tightening gate は分離し、対象 3 本それぞれで **3 種再取得ソース** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直したうえで、**5 ソース** `Team UI / Team API GET / 未認証 HTML GET / direct probe / source frontmatter` を突き合わせる。2026-06-19 時点の Qiita API docs 一次確認では、item の `private` は **only available on Qiita**、`group_url_name` は **Qiita Team 上で share item する group（null で public）** と読めるが、これは **診断前の暫定仮説**として扱い、**team-only を positive に確認できるまでは過剰露出疑いを優先**する。intended baseline は `frontmatter + 実 POST 記録`、observed は `Team API / Team UI / 未認証 GET / public probe` に分離し、判定は primary + contributing で残す。`public direct probe` `https://qiita.com/furuse-kazufumi/items/<id>` の `200` かつ本文断片一致だけを `positive` とし、`302` は `Location=login` まで確認して初めて材料に使う。`外部公開シグナルあり` は即 containment fast path、`未確定` は human 判断つき containment 候補へ送る。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。まず **共通 preflight gate** として token validity / 対象 team membership / 判定に十分な read scope を確認し、ここで失敗したら診断不成立で停止する。その後 intended baseline を `frontmatter + 実 POST 記録(team_stock_queue.md)` に、observed を `実 API 現値 + Team UI + 未認証 GET + public probe` に分離し、**3 種再取得** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直してから **5 ソース突合**へ進む。temporary tightening は別ゲートであり、temporary tightening 後の after-state は root-cause 判定ではなく containment 完了確認として扱う。no-op retain は全記事で `team-only positive` が確認できるまで候補に入れない。

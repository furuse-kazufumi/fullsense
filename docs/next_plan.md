...
- ただしこの Login redirect は「匿名ブラウザで即読めない」ことしか示さず、**team-only と positively 確認できるまでは過剰露出の疑いを優先**する。`private:false` の意味づけ自体も一次情報待ちで、ここで言う `visibility semantics` は **プロジェクト内用語**に留まる。なお `https://qiita.com/furuse-kazufumi/items/<id>` の direct probe は 3 本とも `404` で、少なくとも public `qiita.com` 直URLミラーは確認できていない。

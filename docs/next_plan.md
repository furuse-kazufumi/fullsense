...
- 2026-06-19 の最終到達点として、`#37` short companion の public PATCH は完了済みで、`tests/test_qiita_frontmatter.py` は再実行で `101 passed` だった。当該 docs / tests / tools 更新は commit `a29f8e3` と `196af76` へ反映済みで、現在の worktree は clean である。従って次セッションの主対象は `#37` ではなく、**Qiita Team 3 本の visibility blocker** のみで読む。
- handoff 系 docs のテスト件数は、今回の更新で **`76 passed` → `101 passed`** へ同期した。`docs/articles/2026-05-23/INTEGRATION_AUDIT.md` にある `88 passed` は別文脈の過去監査記録であり、今回の同期対象には含めていない
...
- 同日の追加 hardening として、`tools/qiita_team_post.py` の `dry-run` / `post` は **位置引数を 1 件だけ許可**するようになった。複数 article path を渡した場合は `BLOCKED: exactly one file expected, got N` で fail-closed に止め、対象記事の取り違えを黙殺しない。さらに read-only diagnosis 用の `show <item_id>` も追加し、Team API GET の `private / group.url_name / group.private / organization_url_name` を本ツールから再取得できるようにした。`tests/test_qiita_frontmatter.py` のフル再実行は `101 passed`。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。まず **共通 preflight gate** として token validity / 対象 team membership / 判定に十分な read scope を確認し、ここで失敗したら診断不成立で停止する。その後 intended baseline を `frontmatter + 実 POST 記録(team_stock_queue.md)` に、observed を `実 API 現値 + Team UI + 未認証 GET + public probe` に分離し、**3 種再取得** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直してから **5 source classes / 6 signals の突合**へ進む。temporary tightening は別ゲートであり、temporary tightening 後の after-state は root-cause 判定ではなく containment 完了確認として扱う。no-op retain は全記事で `team-only positive` が確認できるまで候補に入れない。

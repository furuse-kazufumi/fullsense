...
- `qiita47_harness_engineering_thoughts.md` は `#43` から派生した **independent short draft** として維持する方針に決めた。`006.jpg` を「賢そうに見える瞬間」、`157.jpg` を「しつけが必要なのは人類側」という反転オチへ役割分担させたうえで、RAPTOR / `llterm` / Claude Code 公式 `/goal` まで引いて「何を harness に入れるのか」「何を PoC したのか」の具体節と最小の参考導線を追加済み。ここでの RAPTOR は判断層と実行層を分けた二層 runtime、`llterm` は試作段階の terminal-centered orchestration shell だと最小グロスも補った。参照ブロックの二重化も解消し、`desired / actual / drift / repair` の 4 点で読む最小アルゴリズム節、`green-keeper` 的な **いちばん小さい具体例**、その比喩が clean / dirty の観測までしか言い切れずズレの意味づけや優先順位づけは自動化しないという破綻点、harness を強くしすぎると `desired` の狭めすぎ / proxy の本体視 / drift の過剰検知 / repair の硬直化を招く failure mode、そして「明日 1 個だけ変えるなら何を loop に載せるか」まで入った。短稿段階でも「何を loop として固定したいのか」と「何をまだ runtime に入れず人間へ残すのか」が分かる形まで進めた。補助導線の外部 2 本も Distill `Research Debt` と Mitchell Hashimoto `My AI Adoption Journey` の実 URL へ揃えた。
...
14. `qiita47_harness_engineering_thoughts.md` は `#43` から派生した local-only の **independent short draft** として維持する。現時点では publish / Team stock へは載せず、`157.jpg` の使いどころ定義に加えて、RAPTOR（判断層と実行層を分けた二層 runtime）/ 試作段階の `llterm`（terminal-centered orchestration shell）/ Claude Code 公式 `/goal` / `desired-actual-drift-repair` の最小 loop 節、`green-keeper` 的な最小具体例、その比喩が clean / dirty の観測までしか言い切れないという破綻点、harness を強くしすぎたときの failure mode、そして「明日 1 個だけ変えるなら何か」まで入れた。補助導線の外部 2 本も Distill / Mitchell Hashimoto の実 URL へ揃えた。次に触るなら、`#43` へ戻す判断ではなく、この独立短稿をさらに短く締めるか、外部公開できる水準まで参考整形を進めるかを決める。

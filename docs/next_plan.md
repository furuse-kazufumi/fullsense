...
- `qiita44` は参考文献節へ canonical 入口を追加済み。
  ...
  - 到達性 gate については、2026-06-18 時点で記事側の Holland 導線 `Adaptation in Natural and Artificial Systems`（canonical permalink `https://mitpress.mit.edu/9780262581110/adaptation-in-natural-and-artificial-systems/`、DOI resolver `https://doi.org/10.7551/mitpress/1090.001.0001`）と、Koza 導線 `Genetic Programming: On the Programming of Computers by Means of Natural Selection`（canonical permalink `https://mitpress.mit.edu/9780262527910/genetic-programming/`、DOI はこの時点で未固定）を対象に、PowerShell `Invoke-WebRequest` + UA `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36` + Cookie なしで canonical permalink 2 本とも 403 を確認した。さらに Holland 側の DOI resolver `https://doi.org/10.7551/mitpress/1090.001.0001` は 302 で `https://direct.mit.edu/books/book/2574/Adaptation-in-Natural-and-Artificial-SystemsAn` へ解決され、最終到達先 `direct.mit.edu` も同条件で 403 だった。加えて同日、Microsoft Edge headless (`msedge --headless=new --dump-dom`) でも 2 本とも Akamai `Access Denied` HTML を返し、`errors.edgesuite.net` の reference token 付き denial を確認した。browser-engine 側でも少なくとも 1 本は net-log 上 `HTTP/1.1 403 Forbidden` / `Server: AkamaiGHost` まで固定できている。これらは同一 IP / ネットワーク条件でのローカル観測値であり、ここで確定している事実は Akamai による 403 までである。単一観測点のため、自動化クライアント・IP 条件・認証 / 地域制限・WAF 誤検知のいずれが主因かは未確定である。
  - この gate の正規ルートは、まず human-gate で人間が **Holland / Koza の canonical permalink 2 件を対話ブラウザで実確認する** こととする。2 件とも開ければ canonical permalink を維持して gate 通過とし、片方だけ開けなければ失敗した 1 件だけを差し替え、2 件とも開けなければ両方を差し替え候補として扱う。差し替え先は Holland / Koza とも archive.org / Google Books / WorldCat など非 Akamai 系の書誌導線を優先し、Holland の DOI resolver `10.7551/mitpress/1090.001.0001` は canonical permalink と同じ Akamai 系へ解決されることを確認済みの経路として証跡扱いに留める。IP / VPN / Cookie など再現条件固定での再試行は、その後に「一時障害か恒久不達か」を切り分ける補助手順として扱う。canonical permalink を維持したまま「自動到達不可の注記つきで publish」は非推奨 fallback とする。結果と根拠（実ブラウザで開けたか否か、差し替えた場合はどの導線へ差し替えたか）を handoff に戻す。現時点ではこの human-gate 選択待ちであり、外部アクション自体はまだ未確定である。

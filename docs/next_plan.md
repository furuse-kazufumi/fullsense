...
- 実行順の判断としては、**containment first / Team UI diagnosis second / opt-in PATCH last** に置く。理由は、opt-in PATCH は `group_url_name` の再送に留まり、`private` 締め直しを保証せず、しかも frontmatter の `private` 値も同時に再送するためである。未認証 HTML GET / public direct probe で外形的 positive が出た記事は Team UI 完了を待たず fail-closed 候補へ送る。`302 / 403 / 404` や空本文は negative ではなく未確定であり、安全証明に使わない。public 面しか叩けない probe は無情報として扱う。締め直し手段としての `private` 変更は Team UI 上で効力を一次確認したうえで使い、UI で表現できない、または再現性ある API 経路を残す必要がある場合だけ、concrete group_url_name を source に入れた opt-in PATCH を検討する
...
- 次の human-gate に備え、`docs/articles/2026-06-18/team_stock_publish_plan.md` へ **Team UI remediation checklist** も追加した。read-only diagnosis でも remediation turn でも、変更前後の Team API GET / Team UI / 未認証 HTML GET / direct probe / source frontmatter を同じターンで取り、`team_stock_queue.md` の **POST 後の記録欄 `note`** を正本として差分だけを落とす順で運用する
- この段階で `general` を local source へ焼き戻すことはしない。理由は、観測値を resend default に固定しないという current rule を保つためである
- 2026-06-19 の最終到達点として、`#37` short companion の public PATCH は完了済みで、`tests/test_qiita_frontmatter.py` は再実行で `104 passed` だった。当該 docs / tests / tools 更新は commit `a29f8e3` と `196af76` へ反映済みで、現在の worktree は clean である。従って次セッションの主対象は `#37` ではなく、**Qiita Team 3 本の visibility blocker** のみで読む。
- handoff 系 docs のテスト件数は、今回の更新で **`76 passed` → `104 passed`** へ同期した。`docs/articles/2026-05-23/INTEGRATION_AUDIT.md` にある `88 passed` は別文脈の過去監査記録であり、今回の同期対象には含めていない
- その後の local hardening は commit `bbcb358` (`Harden qiita team diagnosis gate`) で確定した。内容は `private` malformed 値の fail-closed block、`show` の auth 失敗弁別、Qiita Team visibility 意味論の **暫定仮説 / 要出典** 注記、ならびに diagnosis gate の内部分類ラベル明記である。現在の worktree は clean で、残る次手は human-gate を伴う外部 diagnosis / remediation のみ。
- 続けて `tools/qiita_team_post.py` へ `preflight <item_id>...` を追加し、`scan` の `qiita-cli-poc` 丸ごと除外と `post` の `--yes` 欠如時 exit 0 も是正した。これで runbook の preflight gate は token 疎通・対象 item 読取・Team host 同一性まではツールで強制される。さらに 2026-06-19 の追加 hardening で、`verify` / `show` / `preflight` / `post` は **採用 token source** を出力し、`qiita_token` fallback は Team diagnosis と Team write の誤判定要因として 4 コマンドすべてを **BLOCK** する。フル回帰は `115 passed`。
- その後、`qiita_registration_safety_report.json` の生成回帰も是正した。`cmd_scan` の既定対象は fixture 1 件ではなく旧17本 (`QIITA_#28`〜`#42` 系) へ戻し、退避済み `.md.bak` source も正規化して拾う。結果として placeholder `team_stock_example.md` は report から消え、`QIITA_#32_llcore_cpu_poc_battery.md` の `LOCAL PATH in body` finding も report 上へ復元された。加えて silent partial coverage もやめ、既定 scan では `scan coverage: queued 12 / existing 42; excluded 30 (#01–27,#35–37)` を表示して分母と除外件数を明示する。フル回帰は `106 passed`。
- 追加で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順に評価するように戻し、personal token が Team token を静かに上書きしないようにした。readback 側も `200` かつ空 `url` を success 扱いせず BLOCK に倒し、default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `63 passed`、フルは `115 passed`。
- 2026-06-19 の追加整理として、Team UI remediation checklist の監査粒度と反映順も tightening した。`body 差し替え` を行った場合の before/after 記録、probe baseline の相対参照、反映順 `team_stock_queue.md` → `docs/next_plan.md` → `docs/SESSION_SUMMARY.md` は正本へ反映済みで、現在の worktree は clean のままである。以後の残件は **human-gate を伴う外部 remediation 実行そのもの**だけである。
- さらに `tools/qiita_team_post.py` へ read-only `preflight <item_id>...` を追加し、`GET /authenticated_user` と `GET /items/<id>` を使って **auth / membership / readable item** をコード側で確認できるようにした。runbook 上の `preflight gate` は、少なくとも token 疎通・対象 item 読取・Team host 同一性まではツールで強制される。失敗時は **診断不成立で停止**し、visibility 判定へ進まない。
- 同日の追加 hardening として、`tools/qiita_team_post.py` の `dry-run` / `post` は **位置引数を 1 件だけ許可**するようになった。複数 article path を渡した場合は `BLOCKED: exactly one file expected, got N` で fail-closed に止め、対象記事の取り違えを黙殺しない。さらに read-only diagnosis 用の `show <item_id>` も追加し、Team API GET の `private / group.url_name / group.private / organization_url_name` を本ツールから再取得できるようにした。`tests/test_qiita_frontmatter.py` のフル再実行は `104 passed`。
- この時点でローカル準備は打ち止めであり、残る未実施タスクは **human-gate を伴う外部 diagnosis / remediation 判断**だけである。優先順は引き続き `positive article contains first / unresolved defaults fail-closed / no-op retain only with explicit cost and hard limit` で読む。
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。診断 gate と temporary tightening gate は分離し、対象 3 本それぞれで **3 種再取得ソース** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直したうえで、**5 source classes / 6 signals**（intended baseline bundle = `frontmatter + 実 POST 記録`、observed = `Team API / Team UI / 未認証 GET / public probe`）を突き合わせる。2026-06-19 時点の Qiita API docs 一次確認では、item の `private` は **only available on Qiita**、`group_url_name` は **Qiita Team 上で share item する group（null で public）** と読めるが、これは **診断前の暫定仮説（要出典）**として扱い、**team-only を positive に確認できるまでは過剰露出疑いを優先**する。ここでの `team-only positive` / `外部公開シグナルあり` / `group_url_name default drift` は Qiita 公式 status ではなく内部解析カテゴリである。判定は primary + contributing で残し、`public direct probe` `https://qiita.com/furuse-kazufumi/items/<id>` の `200` かつ本文断片一致だけを `positive` とする。`302` は `Location=login` まで確認して初めて材料に使うが、単独では「非公開の証明」に使わない。`外部公開シグナルあり` は即 containment fast path、`未確定` は human 判断つき containment 候補へ送る。
- 上記の diagnosis gate 定義と no-op retain 記録仕様は commit `dc07a66` に反映済みで、現在の worktree は clean のままである。従って次の一手は、依然として human-gate を伴う外部 remediation の選択だけである。
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。まず **共通 preflight gate** として `py -3.11 tools/qiita_team_post.py preflight <item_id>...` を実行し、token validity / 対象 team membership / 判定に十分な read scope を確認する。ここで失敗したら診断不成立で停止し、そのまま visibility 判定へ進まない。その後 intended baseline を `frontmatter + 実 POST 記録(team_stock_queue.md)` に、observed を `実 API 現値 + Team UI + 未認証 GET + public probe` に分離し、**3 種再取得** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直してから **5 source classes / 6 signals の突合**へ進む。temporary tightening は別ゲートであり、temporary tightening 後の after-state は root-cause 判定ではなく containment 完了確認として扱う。no-op retain は全記事で `team-only positive` が確認できるまで候補に入れない。
- 2026-06-19 に `tests/test_qiita_frontmatter.py` へ `qiita_team_post.py` の回帰も追加し、`real_id` の nullish fallback、frontmatter `private: false` の文字列パース経路、**空 `private:` は default=True**、さらに **malformed `private` 値は public へ倒さず BLOCK** する修正まで固定した。したがって Team stock 3 本の `private:false` incident は、少なくとも現在の Team poster 実装では再現しない層のバグとして切り分けられるが、だからといって可視範囲の疑いが解消したとは扱わない。
...
13. **次の具体的一手**は Team blocker の containment-first gate を再開すること。`docs/articles/2026-06-18/team_stock_queue.md` と `team_stock_publish_plan.md` を正本として開き、(1) まず 3 本それぞれの intended state baseline を固定する。(2) 判定に使う Team API GET / 未認証 HTML GET / direct probe は stale のまま使わず、その turn で再取得する。(3) 未認証 HTML GET / public direct probe で `200` かつ本文確認が取れた記事は Team UI 完了を待たずその記事だけ fail-closed 候補へ送る。(4) それ以外の記事について Team UI で intended share target / private state を read-only で確認し、同ターン再取得した Team API GET / 未認証 HTML GET / direct probe / source frontmatter と 3 本ぶん突き合わせて、share target 起因 / private state 起因 / 未確定 を 1 行判定する。(5) no-op retain は 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。(6) UI で表現できない、または再現性ある API 経路を残す必要がある場合に限って、診断完了後の従属候補として concrete group_url_name を source に入れた opt-in PATCH (`--patch-group-url-name`) を検討する。既定 PATCH は維持されており、opt-in 再送経路はローカル実装済みだが、既共有 item に対する締め直し効果は一次未確認という前提で再開する。

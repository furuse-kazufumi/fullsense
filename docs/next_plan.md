...
4. 種 #6 の ja draft は `tools/qiita-cli-poc/public/qiita46_llterm_supervision_first.md` として第8章まで通し、`2549%` 記述も「rotate 因果は確認済み / 膨張機序は有力推定 / 算定内訳は未確定」の境界へ締め直した。次は publish gate 残作業として、かみくだき版 / en-zh-ko / 挿絵 `025.jpg` / 参考文献 / HTML annotation / 休憩ポイントを順に整える。
...
10. `qiita46_llterm_supervision_first.md` は第8章まで執筆済みだが、publish gate 解除前に 9 原則の参考文献・かみくだき版・多言語・挿絵 `025.jpg`・HTML annotation・休憩ポイントを揃える。

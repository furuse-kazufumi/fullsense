...
- `#37` live PATCH の human-gate 後 runbook も固定済みで、`dry-run` → `f06ca92...` live item 目視 → `post ... --yes` → API / HTML 再確認、の 4 段で実行する。`#37` source は `public_id` 付きなので `--allow-create` は不要。

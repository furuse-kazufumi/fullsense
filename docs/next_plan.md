...
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。診断 gate と temporary tightening gate は分離し、対象 3 本それぞれで **3 種再取得ソース** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直したうえで、**5 ソース** `Team UI / Team API GET / 未認証 HTML GET / direct probe / source frontmatter` を突き合わせる。判定は `share target 起因 / private state 起因 / slug or URL drift / draft or deleted / org membership or token failure / 未確定` の 1 行で残す。`未認証 HTML GET / public direct probe` の `200` かつ本文断片一致だけを `positive` とし、`302 / 403 / 404` や空本文は未確定であって安全証明に使わない。positive と未確定は記事単位で fail-closed 候補に送る。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。まず intended state baseline を記事単位で固定し、token validity を確認したうえで **3 種再取得** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直し、その後 **5 ソース突合**へ進む。temporary tightening は別ゲートであり、temporary tightening 後の after-state は root-cause 判定ではなく containment 完了確認として扱う。no-op retain を選ぶ場合も `team_stock_queue.md` の正本欄に `retain_count / retain_deadline / human_exemption` を残し、`docs/next_plan.md` と `docs/SESSION_SUMMARY.md` は参照だけを反映する。

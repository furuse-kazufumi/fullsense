...
- `qiita47_harness_engineering_thoughts.md` は `#43` から派生した **independent short draft** として維持する方針に決めた。`006.jpg` を「賢そうに見える瞬間」、`157.jpg` を「しつけが必要なのは人類側」という反転オチへ役割分担させたうえで、RAPTOR / `llterm` / Claude Code 公式 `/goal` まで引いて「何を harness に入れるのか」「何を PoC したのか」の具体節と最小の参考導線を追加済み。`llterm` は試作段階のスケルトンだと明示し、参照ブロックの二重化も解消した。さらに `desired / actual / drift / repair` の 4 点で読む最小アルゴリズム節、`green-keeper` 的な **いちばん小さい具体例**、その比喩が clean / dirty の観測までしか言い切れずズレの意味づけや優先順位づけは自動化しないという破綻点、そして harness を強くしすぎると `desired` の狭めすぎ / proxy の本体視 / drift の過剰検知 / repair の硬直化を招く failure mode まで入った。短稿段階でも「何を loop として固定したいのか」と「何をまだ runtime に入れず人間へ残すのか」が分かる形まで進めた。

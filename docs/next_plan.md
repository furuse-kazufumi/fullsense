...
- 2026-06-19 現在、`#37` short companion の live 更新に関する local blocker は解消済みで、append-only runbook hardening の節目は commit `1b98fc0`（`tighten qiita append-only patch runbook`）で確定している。以後の local 変更は doc 整合と safety polish が中心で、`#37` companion について次に残るのは public Qiita item `f06ca92ea208c7646fcd` への外部 PATCH である。これは human-gate 承認待ちであり、`#46` / dev.to / CTA-149 / Team 可視範囲是正は別管理の残件とする。

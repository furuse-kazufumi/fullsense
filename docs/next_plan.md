...
- 次セッションの最初の一手は **Koza 側の書誌事実を API / 書誌系でこれ以上固定できるかを確認すること**。Holland は Crossref 一致結果まで handoff に固定済みなので、Koza は MIT Press 書籍の DOI 有無を Crossref / WorldCat / 出版社書誌で確認し、見つからなければ「Crossref title/author 検索と `9780262527910` クエリでは未特定」と明記する。そのうえで、受領済みの選択肢 1 に従って別 egress を含む対話ブラウザで 2 件の canonical permalink を 1 回ずつ開き、`ログイン不要で書誌ページが表示されるか` を reader reachability の補助証跡として handoff に戻す。片方だけ失敗したらその 1 件にだけ archive.org / WorldCat / Google Books を補助導線として併記し、2 件とも失敗したら両方へ併記する。

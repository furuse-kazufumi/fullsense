...
- `qiita44` は参考文献節へ canonical 入口を追加済み。
  ...
  - 到達性 gate については、2026-06-18 時点で記事側の Holland 導線 `Adaptation in Natural and Artificial Systems`（canonical permalink `https://mitpress.mit.edu/9780262581110/adaptation-in-natural-and-artificial-systems/`、DOI `10.7551/mitpress/1090.001.0001`、resolver `https://doi.org/10.7551/mitpress/1090.001.0001`）と、Koza 導線 `Genetic Programming: On the Programming of Computers by Means of Natural Selection`（canonical permalink `https://mitpress.mit.edu/9780262527910/genetic-programming/`、DOI はこの時点で未固定）を対象に、PowerShell `Invoke-WebRequest` + UA `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36` + Cookie なしで canonical permalink 2 本とも 403 を確認した。さらに Holland 側の DOI resolver `https://doi.org/10.7551/mitpress/1090.001.0001` は 302 で `https://direct.mit.edu/books/book/2574/Adaptation-in-Natural-and-Artificial-SystemsAn` へ解決され、最終到達先 `direct.mit.edu` も同条件で 403 だった。加えて同日、Microsoft Edge headless (`msedge --headless=new --dump-dom`) でも 2 本とも Akamai `Access Denied` HTML を返し、`errors.edgesuite.net` の reference token 付き denial を確認した。browser-engine 側でも少なくとも 1 本は net-log 上 `HTTP/1.1 403 Forbidden` / `Server: AkamaiGHost` まで固定できている。これらは同一 IP / ネットワーク条件でのローカル観測値であり、ここで確定している事実は Akamai による 403 までである。単一観測点のため、自動化クライアント・IP 条件・認証 / 地域制限・WAF 誤検知のいずれが主因かは未確定である。Holland の DOI 本体 `10.7551/mitpress/1090.001.0001` は出版社 / 外部書誌で参照実績を確認できる一方、Koza 側 DOI はこの時点で未固定のままである。
- この gate の基線は **canonical permalink / DOI を引用上の主参照として維持する** ことであり、我々の 1 網からの自動 403 だけでは citation integrity を崩さない。human-gate では、まず可能なら別 egress を含む対話ブラウザで 2 件の canonical permalink を 1 回だけ実確認し、読者目線でログイン不要の書誌ページ表示が取れるかを見る。2 件とも開ければそのまま維持し、片方だけ開けなければ失敗した 1 件にだけ archive.org / WorldCat / Google Books など非 Akamai 系の補助導線を併記し、2 件とも開けなければ両方に補助導線を併記する。Holland の DOI resolver `10.7551/mitpress/1090.001.0001` は canonical permalink と同じ Akamai 系へ解決されることを確認済みの経路として証跡扱いに留め、fallback の主経路にはしない。IP / VPN / Cookie など再現条件固定での再試行は調査補助手段であって gate 合否根拠には使わない。結果と根拠（実ブラウザで開けたか否か、補助導線をどこまで併記したか）を handoff に戻す。現時点ではこの human-gate 選択待ちであり、外部アクション自体はまだ未確定である。
- 次セッションの最初の一手は **human-gate の選択結果を受けて外部アクションへ進むこと**。選択肢 1 が選ばれた場合は、別 egress を含む対話ブラウザで Holland / Koza の canonical permalink を 1 回ずつ開き、`ログイン不要で書誌ページが表示されるか` を pass/fail として handoff に結果と根拠を戻す。片方だけ失敗したらその 1 件にだけ archive.org / WorldCat / Google Books を補助導線として併記し、2 件とも失敗したら両方へ補助導線を併記する。選択肢 2 なら補助導線の併記方針だけを先に確定し、選択肢 3 なら `qiita44` は保留のまま他のローカル整備へ移る。

...
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。診断 gate と temporary tightening gate は分離し、対象 3 本それぞれで **3 種再取得ソース** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直したうえで、**5 ソース** `Team UI / Team API GET / 未認証 HTML GET / direct probe / source frontmatter` を突き合わせる。現時点の主疑義は **意図しない公開**であり、意図しない非公開は副次候補として切り分ける。判定は `share target 起因 / private state 起因 / org membership or token failure / group_url_name default drift / 未確定` の 1 行で残す。`未認証 HTML GET / public direct probe` の `200` かつ本文断片一致だけを `positive` とし、`302` は `Location=login` まで確認して初めて材料に使う。`403 / 404` や空本文は未確定であって安全証明に使わない。positive と未確定は記事単位で fail-closed 候補に送る。

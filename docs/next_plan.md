...
- 実行順の判断としては、**外部 remediation の first choice は Team UI** に置く。理由は、opt-in PATCH は `group_url_name` の再送に留まり、`private` 締め直しを保証せず、しかも frontmatter の `private` 値も同時に再送するためである。したがって次の human-gate は、(1) Team UI で intended share target / private state を確認・是正できるかを先に見る、(2) 真因が share target / private state のどちらかを Team UI で確定し、各 frontmatter の `private` 値も個別確認したうえで、UI で表現できない、または再現性ある API 経路を残す必要がある場合だけ、concrete group_url_name を source に入れた opt-in PATCH を検討する、の順で扱う
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 5 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` と source frontmatter の 4 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグを採取し、各 source frontmatter の `private` / `group_url_name` も個別確認したうえで、既存 4 レグと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する。未認証 HTML GET / public direct probe で positive が出た記事は、その時点で Team UI 完了を待たず記事単位で remediation gate へ送る` `(b) no-op retain として**過剰露出疑いを未解消のまま据え置くコスト**を明示的に受け入れ、team_stock_queue.md の **POST 後の記録欄 `note`** を正本として retain 回数も残す。retain は同じ理由で 1 回までに留め、2 回目に入る前に暫定 private 化を候補化する` `(c) 診断未了のままでも fail-closed に倒し、当該記事を暫定 private 化してから再診断する` の 3 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。
...
13. **次の具体的一手**は Team blocker の diagnosis gate を再開すること。`docs/articles/2026-06-18/team_stock_queue.md` と `team_stock_publish_plan.md` を正本として開き、(1) Team UI で intended share target / private state を read-only で確認し、既採取の Team API GET / 未認証 HTML GET / direct probe / source frontmatter と同ターンで 3 本ぶん突き合わせて、share target 起因 / private state 起因 / 未確定 を 1 行判定する。(2) 未認証 HTML GET / public direct probe で positive が出た記事は Team UI 完了を待たずその記事だけ remediation gate へ送る。(3) no-op retain は 1 回までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。(4) UI で表現できない、または再現性ある API 経路を残す必要がある場合に限って、診断完了後の従属候補として concrete group_url_name を source に入れた opt-in PATCH (`--patch-group-url-name`) を検討する。既定 PATCH は維持されており、opt-in 再送経路はローカル実装済みだが、既共有 item に対する締め直し効果は一次未確認という前提で再開する。

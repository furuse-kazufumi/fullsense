...
- 次セッションの最初の一手は **Holland / Koza の書誌事実を API 優先で固めること**。Holland は Crossref / doi content negotiation の一致結果を handoff に転記し、Koza は MIT Press 書籍の DOI 有無を Crossref / WorldCat / 出版社書誌で確認する。そのうえで human-gate の選択結果を受け、選択肢 1 なら別 egress を含む対話ブラウザで 2 件の canonical permalink を 1 回ずつ開き、`ログイン不要で書誌ページが表示されるか` を reader reachability の補助証跡として handoff に戻す。片方だけ失敗したらその 1 件にだけ archive.org / WorldCat / Google Books を補助導線として併記し、2 件とも失敗したら両方へ併記する。選択肢 2 なら補助導線の併記方針だけを先に確定し、選択肢 3 なら `qiita44` は保留のまま他のローカル整備へ移る。

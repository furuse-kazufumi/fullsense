...
- 続けて `tools/qiita_team_post.py` へ `preflight <item_id>...` を追加し、`scan` の `qiita-cli-poc` 丸ごと除外と `post` の `--yes` 欠如時 exit 0 も是正した。これで runbook の preflight gate は token 疎通・対象 item 読取・Team host 同一性まではツールで強制される。フル回帰は `104 passed`。
- その後、`qiita_registration_safety_report.json` の生成回帰も是正した。`cmd_scan` の既定対象は fixture 1 件ではなく旧17本 (`QIITA_#28`〜`#42` 系) へ戻し、退避済み `.md.bak` source も正規化して拾う。結果として placeholder `team_stock_example.md` は report から消え、`QIITA_#32_llcore_cpu_poc_battery.md` の `LOCAL PATH in body` finding も report 上へ復元された。フル回帰は `106 passed`。

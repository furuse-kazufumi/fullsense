...
- 2026-06-19 に `tests/test_qiita_frontmatter.py` へ `qiita_team_post.py` の最小回帰も追加し、Team poster 自体は `real_id` の nullish 処理と frontmatter `private:` → payload `private` 反映をそのまま行っていることを固定した。したがって Team stock 3 本の `private:false` incident は、この層の bool 変換バグではなく Team API / visibility semantics 側の問題として切り分けて読む。

...
- safety polish として、`qiita_public_post.py` は **`id` はあるが `public_id` が無い** source を dry-run / post したとき warning を出す。`POST create` 自体は許すが、legacy/team-style `id` を持つ source を public patch source と誤読したまま進みにくくするための guard である。

...
- 次セッションの最初の一手は、受領済みの選択肢 1 に従って別 egress を含む対話ブラウザで 2 件の canonical permalink を 1 回ずつ開き、`ログイン不要で書誌ページが表示されるか` を reader reachability の補助証跡として handoff に戻すことである。片方だけ失敗したらその 1 件にだけ archive.org / WorldCat / Google Books を補助導線として併記し、2 件とも失敗したら両方へ併記する。Koza 側の DOI は、この時点では Crossref で未特定・MIT Press title page / WorldCat までは確認済み、という粒度を固定して追記すれば足りる。

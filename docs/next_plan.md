...
4. 種 #6 の ja draft は `tools/qiita-cli-poc/public/qiita46_llterm_supervision_first.md` として第8章まで通し、`2549%` 記述も「rotate 因果は確認済み / 膨張機序は有力推定 / 算定内訳は未確定」の境界へ締め直した。本文側の `☕ 休憩ポイント` / 参考文献 / HTML annotation / 挿絵 `025.jpg` も投入済みで、`025.jpg` はクレジットを `Snack Basue` に統一し raw URL の HTTP 200 も確認済み。URL は既存運用どおり `raw.githubusercontent.com/.../main/...` を維持する。`qiita46_llterm_supervision_first_kamikudaki.md` も冒頭ナビ・「完全版で掘る 3 点」・内部語の 1 行グロスまで磨き込み済み、`kamikudaki_shishi.svg` raw URL の HTTP 200 も確認済み。さらに en/zh/ko draft を新規追加し、第1章前半まで同期したので、次は publish gate 残作業として、多言語本文の第2章以降を順に整える。
...
10. `qiita46_llterm_supervision_first.md` は第8章まで執筆済みで、参考文献・HTML annotation・休憩ポイント・挿絵 `025.jpg` も投入済み。`025.jpg` はクレジット修正と raw URL の HTTP 200 確認まで済ませ、URL は既存運用どおり `raw.githubusercontent.com/.../main/...` を維持する。`qiita46_llterm_supervision_first_kamikudaki.md` は短縮版として磨き込み済み、`kamikudaki_shishi.svg` raw URL の HTTP 200 も確認済み。en/zh/ko draft も第1章前半まで起こした。publish gate 解除前に残るのは、多言語本文の後半同期と最終見直し。

...
- ここで使う `ignorePublish` は Qiita 標準属性ではなく、`qiita-cli-poc` ローカル運用の publish 安全柵である。**ただし `tools/qiita_public_post.py` は `ignorePublish` を読まない**ため、このポスター経路の実ゲートは `post <file> --yes` と human-gate 承認だけである。また「redirect」は HTTP 転送ではなく、Qiita 本文を統合告知 + canonical 誘導へ差し替える運用語として使う。

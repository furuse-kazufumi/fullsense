...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。まず **共通 preflight gate** として token validity / 対象 team membership / 判定に十分な scope を確認し、ここで失敗したら診断不成立で停止する。その後 intended state baseline の正本を `frontmatter / 実 POST 記録(team_stock_queue.md) / 実 API 現値` の 3 点突き合わせに固定し、**3 種再取得** `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直してから **5 ソース突合**へ進む。temporary tightening は別ゲートであり、temporary tightening 後の after-state は root-cause 判定ではなく containment 完了確認として扱う。no-op retain は全記事で `team-only positive` が確認できるまで候補に入れない。

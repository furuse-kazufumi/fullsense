...
- この時点でローカル準備は打ち止めであり、残る未実施タスクは **human-gate を伴う外部 remediation の実行判断**だけである。優先順は引き続き `Team UI first / opt-in PATCH second / no-op retain` で読む。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 4 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` の 3 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグだけ採取して結果を持ち帰る` `(b) Team UI レグ採取のうえ、同ターンで是正まで進む` `(c) no-op retain として過剰露出疑いを保持したまま据え置き、`team_stock_queue.md` の note に次回解禁条件を記録する` の 3 択として扱う。opt-in PATCH は診断完了後の従属候補であり、この gate ではまだ提示しない。

...
14. `qiita47_harness_engineering_thoughts.md` は `#43` から派生した local-only の **independent short draft** として維持する。現時点では publish / Team stock へは載せず、`157.jpg` の使いどころ定義に加えて、RAPTOR / 試作段階の `llterm` / Claude Code 公式 `/goal` / `desired-actual-drift-repair` の最小 loop 節まで入れた。次に触るなら、`#43` へ戻す判断ではなく、この独立短稿をさらに短く締めるか、外部公開できる水準まで参考整形を進めるかを決める。

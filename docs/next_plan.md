...
- `qiita44` は参考文献節へ canonical 入口を追加済み。
  ...
  - この gate の正規ルートは、まず human-gate で人間が **Holland / Koza の canonical permalink 2 件を対話ブラウザで実確認する** こととする。2 件とも開ければ canonical permalink を維持して gate 通過とし、開けない場合は次に **Holland は DOI resolver `10.7551/mitpress/1090.001.0001`、Koza は archive.org / Google Books / WorldCat など非 Akamai 系の書誌導線**を優先候補として差し替え判断へ進む。IP / VPN / Cookie など再現条件固定での再試行は、その後に「一時障害か恒久不達か」を切り分ける補助手順として扱う。canonical permalink を維持したまま「自動到達不可の注記つきで publish」は非推奨 fallback とする。結果と根拠（実ブラウザで開けたか否か、差し替えた場合はどの導線へ差し替えたか）を handoff に戻す。現時点ではこの human-gate 選択待ちであり、外部アクション自体はまだ未確定である。

...
- これは単なる記録ではなく、`private:true` を安全柵だと誤信したまま Team poster を使うと、想定より広い可視範囲で着地しうるという失敗教訓でもある。`ignorePublish:true` 自体は 2026-06-19 時点の `tools/qiita_team_post.py` でも読まれ、`--force-ignore-publish` 無しでは fail-closed で止まる。以後の Team poster 利用時は、この露出可能性を blocker として先に明示する。

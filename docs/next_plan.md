...
- deindex は commit `7745f84`、記事シードのスクリーニングは commit `b4d1241` で確定済み。現在の worktree には handoff / 草稿の未 commit 変更が残っており、次セッションも差分あり前提で再開する。
...
- 現在の worktree では、`qiita45 + handoff sync` に加えて前ターンからの `qiita43 en/zh/ko` / `qiita44` / `qiita47` 差分も未 commit で同居している。次に commit を切るなら、`qiita45 + docs sync` と前ターン分は分離し、変更範囲の追跡性を保つ。
- handoff を読むときも、この差分は `qiita45` micro-polish 単体ではなく、前ターン未 commit の appendix 追加 / `qiita43` SVG 同期 / `qiita44` soft-gate 注記 / `qiita47` 圧縮 pass を同梱したバンドルとして解釈する。粒度は `NEXT_SESSION.md` の「このターンの変更範囲」に合わせ、次の commit 後に正規の handoff 行へ畳み直す。
...
- 2026-06-19 の方針追加として、「**公開済みの短い companion 記事は、十分に長い本編へ統合できるなら統合を優先し、空いた item は新記事の器として再利用する**」を採用する。RAD の controlling idea / cut-or-merge / appendix-for-depth 基準に照らすと、現時点で外部アクション候補として最も筋が良いのは **public 済みの `qiita37_gpu_triple_run_gate_price_kamikudaki.md` (`id: f06ca92ea208c7646fcd`)** で、受け皿は完全版 `qiita37_gpu_triple_run_gate_price.md` (`#37`) になる。`qiita43_harness_loop_stack_kamikudaki.md` / `qiita46_llterm_supervision_first_kamikudaki.md` / `qiita47_harness_engineering_thoughts.md` はまだ `private:true` の local draft なので、統合判断は publish 前の内部判断として先送りできる。
...
- 2026-06-18 終了時点の到達メモとしては、#43 は companion / `green-keeper` / `/goal` 帯まで、#46 は JA/en/zh/ko/kamikudaki の本文と endmatter まで、Team stock 3 本は Team POST と item id 記録まで完了していた。その後 2026-06-19 に handoff / 草稿更新が入り、**現在の worktree には未 commit の handoff / 草稿変更が残っている**。現行 blocker として最優先で残すのは、Qiita Team 3 本が API GET では `private:false` で着地した件である。可視範囲是正（rollback / visibility tightening）が必要かどうかの判断待ち案件として、残件一覧から落とさない。
...
- 現在の worktree には未コミット差分が残っている。対象は `docs/*` に加え、`qiita43_harness_loop_stack_{en,zh,ko}.md`、`qiita44_evolutionary_programs_block_diagram.md`、`qiita45_human_ai_dev_incident_patterns.md`、`qiita47_harness_engineering_thoughts.md` で、前ターンからの差分が同梱された状態で読んでよい。`loop_ledger` の deindex は commit `7745f84` で完了済み。

...
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。opt-in PATCH の解禁条件は、対象 3 本それぞれで **5 ソース** `Team UI` / `Team API GET` / `未認証 HTML GET` / `direct probe` / `source frontmatter` を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行で判定できる状態に到達することである。5 ソースが矛盾した場合は `未認証 HTML GET / public direct probe` を最優先し、`302 / 403 / 404` や空本文は未確定として扱う。positive article はその記事だけ containment / remediation gate へ直送し、未確定も default で fail-closed 候補に送る。baseline は option 選択前に 3 本それぞれで固定する。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 5 ソースのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` と source frontmatter の 4 本で、**残ソースは Team UI のみ**である。したがって次の分岐は `(a) Team UI ソースを採取し、各 source frontmatter の `private` / `group_url_name` も intended state として個別確認したうえで、既存 4 ソースと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する。未認証 HTML GET / public direct probe で `200` かつ本文確認が取れた記事は、その時点で Team UI 完了を待たず記事単位で containment / remediation gate へ送る。`302 / 403 / 404` は negative ではなく未確定である` `(b) no-op retain として**過剰露出疑いを未解消のまま据え置くコスト**を明示的に受け入れ、team_stock_queue.md の **POST 後の記録欄 `note`** を正本として retain 回数と時間上限を残す。retain は同じ理由で 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前に暫定 private 化を候補化する` `(c) 診断未了または未確定のままでも fail-closed に倒し、当該記事を暫定 private 化してから再診断する` の 3 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。

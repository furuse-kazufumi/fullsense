...
- 実行結果として、3 本とも 201 Created で item 作成に成功した。POST 後の API GET では 3 本とも `private:false` で返っており、frontmatter `private:true` は Team create 後の可視範囲を保証しなかった。さらに 2026-06-19 の未認証 HTML GET では 3 本とも Login ページへ落ちたため、queue 正本には item id / URL / visible range memo を「Team API / visibility semantics の不一致」として記録済みで、rollback が必要なら以後は別の human-gate 外部アクションとして扱う。
- これは単なる記録ではなく、`private:true` を安全柵だと誤信したまま Team poster を使うと、**API の visible flag と実ブラウザ到達性がずれる**可能性があるという失敗教訓でもある。`ignorePublish:true` 自体は 2026-06-19 時点の `tools/qiita_team_post.py` でも読まれ、`--force-ignore-publish` 無しでは fail-closed で止まる。以後の Team poster 利用時は、この可視範囲意味論の不一致を blocker として先に明示する。

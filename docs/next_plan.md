...
- この時点でローカル準備は打ち止めであり、残る未実施タスクは **human-gate を伴う外部 diagnosis / remediation 判断**だけである。優先順は引き続き `positive article contains first / Team UI diagnosis second / no-op retain only with explicit cost and hard limit` で読む。
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。opt-in PATCH の解禁条件は、対象 3 本それぞれで `Team UI` / `Team API GET` / `未認証 HTML GET` / `direct probe` / `source frontmatter` を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行で判定できる状態に到達することである。5 ソースが矛盾した場合は `未認証 HTML GET / public direct probe` を最優先し、positive article はその記事だけ containment / remediation gate へ直送する。baseline は option 選択前に 3 本それぞれで固定する。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 5 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` と source frontmatter の 4 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグを採取し、各 source frontmatter の `private` / `group_url_name` も個別確認したうえで、既存 4 レグと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する。未認証 HTML GET / public direct probe で positive が出た記事は、その時点で Team UI 完了を待たず記事単位で containment / remediation gate へ送る` `(b) no-op retain として**過剰露出疑いを未解消のまま据え置くコスト**を明示的に受け入れ、team_stock_queue.md の **POST 後の記録欄 `note`** を正本として retain 回数と時間上限を残す。retain は同じ理由で 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前に暫定 private 化を候補化する` `(c) 診断未了のままでも fail-closed に倒し、当該記事を暫定 private 化してから再診断する` の 3 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。
...
13. **次の具体的一手**は Team blocker の containment-first gate を再開すること。`docs/articles/2026-06-18/team_stock_queue.md` と `team_stock_publish_plan.md` を正本として開き、(1) まず 3 本それぞれの intended state baseline を固定する。(2) 未認証 HTML GET / public direct probe で positive が出た記事は Team UI 完了を待たずその記事だけ fail-closed 候補へ送る。(3) それ以外の記事について Team UI で intended share target / private state を read-only で確認し、既採取の Team API GET / 未認証 HTML GET / direct probe / source frontmatter と同ターンで 3 本ぶん突き合わせて、share target 起因 / private state 起因 / 未確定 を 1 行判定する。(4) no-op retain は 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。(5) UI で表現できない、または再現性ある API 経路を残す必要がある場合に限って、診断完了後の従属候補として concrete group_url_name を source に入れた opt-in PATCH (`--patch-group-url-name`) を検討する。既定 PATCH は維持されており、opt-in 再送経路はローカル実装済みだが、既共有 item に対する締め直し効果は一次未確認という前提で再開する。

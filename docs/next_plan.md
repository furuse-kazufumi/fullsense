...
- 次の human-gate に備え、`docs/articles/2026-06-18/team_stock_publish_plan.md` へ **Team UI remediation checklist** も追加した。read-only diagnosis でも remediation turn でも、変更前後の Team API GET / Team UI / 未認証 HTML GET / direct probe / source frontmatter を同じターンで取り、`team_stock_queue.md` を正本として差分だけを落とす順で運用する
...
- この時点でローカル準備は打ち止めであり、残る未実施タスクは **human-gate を伴う外部 diagnosis / remediation 判断**だけである。優先順は引き続き `Team UI diagnosis first / remediation gate later / no-op retain only with explicit cost` で読む。
- 追加で、`diagnosis 完了` の定義と `no-op retain` の記録仕様も Team stock 正本へ固定した。opt-in PATCH の解禁条件は、対象 3 本それぞれで `Team UI` / `Team API GET` / `未認証 HTML GET` / `direct probe` / `source frontmatter` を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行で判定できる状態に到達することである。
...
- ただしこの human-gate は、もはや「remediation を選ぶ」だけではない。diagnosis gate の 5 レグのうち、既採取は `Team API GET` / `未認証 HTML GET` / `direct probe` と source frontmatter の 4 本で、**残レグは Team UI のみ**である。したがって次の分岐は `(a) Team UI レグを採取し、各 source frontmatter の `private` / `group_url_name` も個別確認したうえで、既存 4 レグと同ターンで 3 本ぶん併記して share target 起因 / private state 起因 / 未確定 を 1 行判定する。過剰露出が positively 確認された場合は、その結果を持ち帰って別 human-gate の remediation へ送る` `(b) no-op retain として**過剰露出疑いを未解消のまま据え置くコスト**を明示的に受け入れ、team_stock_queue.md の **POST 後の記録欄** `note` を正本として「今回も診断未了のため no-op retain。過剰露出疑いは未解消のまま残る。次回解禁条件: 3 本それぞれで Team UI / Team API GET / 未認証 HTML GET / direct probe / source frontmatter を同ターンで突き合わせ、share target 起因か private state 起因か、または未確定かを 1 行判定する。次回は Team UI diagnosis を優先する」と記録し、その参照だけを同ターンで `docs/next_plan.md` と `docs/SESSION_SUMMARY.md` に反映する` の 2 択として扱う。是正そのものは診断結果を持ち帰った後の**別ゲート**へ戻し、opt-in PATCH は診断完了後の従属候補としてのみ扱う。
...
13. **次の具体的一手**は Team blocker の diagnosis gate を再開すること。`docs/articles/2026-06-18/team_stock_queue.md` と `team_stock_publish_plan.md` を正本として開き、(1) Team UI で intended share target / private state を read-only で確認し、既採取の Team API GET / 未認証 HTML GET / direct probe / source frontmatter と同ターンで 3 本ぶん突き合わせて、share target 起因 / private state 起因 / 未確定 を 1 行判定する。(2) 過剰露出が positively 確認された場合だけ、その結果を持ち帰って別 human-gate の remediation へ送る。(3) UI で表現できない、または再現性ある API 経路を残す必要がある場合に限って、診断完了後の従属候補として concrete group_url_name を source に入れた opt-in PATCH (`--patch-group-url-name`) を検討する。既定 PATCH は維持されており、opt-in 再送経路はローカル実装済みだが、既共有 item に対する締め直し効果は一次未確認という前提で再開する。

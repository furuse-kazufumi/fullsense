...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順だけでなく、**複数 api-keys ファイルをまたいでも Team token を先に拾う 2 パス探索**へ直し、token source surfacing も入った。readback 側も `200/201` かつ空 `url` を success 扱いせず BLOCK に倒し、`cmd_post` の write 検証は **POST/PATCH レスポンス echo ではなく独立 GET の read-after-write** を authoritative source に戻した。create 成功時は **独立 GET 検証が通った後にだけ** `id` を frontmatter へ永続化し、`private` mismatch / `url` 欠落 / readback 不可読はすべて fail-closed で `return 1` に倒す。`LOCAL PATH` / `NON-PUBLIC IMAGE` も Team write 前 block のまま維持する。default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `73 passed, 52 deselected`、フルは `125 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

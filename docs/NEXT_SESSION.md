...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順だけでなく、**複数 api-keys ファイルをまたいでも Team token を先に拾う 2 パス探索**へ直し、token source surfacing も入った。readback 側も `200/201` かつ空 `url` を success 扱いせず BLOCK に倒し、`cmd_post` は **POST/PATCH レスポンス body を authoritative な write 検証**として `_format_item_readback()` に通し、`intended private` と response `private` を照合する。create 成功後はこの response 検証が PASS したときだけ `id` を frontmatter へ永続化し、その後の独立 GET は advisory read-after-write として warning 止まりにする。あわせて `LOCAL PATH` / `NON-PUBLIC IMAGE` は Team write 前 block へ格上げした。default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `70 passed, 52 deselected`、フルは `122 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

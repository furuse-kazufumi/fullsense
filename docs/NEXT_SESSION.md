...
- 外部 remediation の優先順位は **Team UI diagnosis first / remediation gate later / opt-in PATCH second** で固定して再開する。理由は、opt-in PATCH は `group_url_name` 再送のローカル経路と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate は、まず Team UI を read-only で見て intended share target / private state を確認し、既採取の Team API GET / 未認証 HTML GET / direct probe / source frontmatter と同ターンで突き合わせて 3 本それぞれを share target 起因 / private state 起因 / 未確定 に 1 行判定する。過剰露出が positively 確認された場合だけ、別 human-gate の remediation へ送る。それで不足する場合に限って、concrete `group_url_name` を source に入れた opt-in PATCH を候補化する

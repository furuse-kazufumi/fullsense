...
- 外部 remediation の優先順位は **Team UI diagnosis first / positive article remediates immediately / opt-in PATCH second** で固定して再開する。理由は、opt-in PATCH は `group_url_name` 再送のローカル経路と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate は、まず Team UI を read-only で見て intended share target / private state を確認し、既採取の Team API GET / 未認証 HTML GET / direct probe / source frontmatter と同ターンで突き合わせて 3 本それぞれを share target 起因 / private state 起因 / 未確定 に 1 行判定する。未認証 HTML GET / public direct probe で positive が出た記事は Team UI 完了を待たずその記事だけ remediation gate へ送る。no-op retain は 1 回までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。それで不足する場合に限って、concrete `group_url_name` を source に入れた opt-in PATCH を候補化する

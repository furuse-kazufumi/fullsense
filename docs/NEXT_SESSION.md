...
- 追加の観測では、3 本とも Team API GET が `group.url_name: general` / `group.private:false` / `organization_url_name:null` も返した。2026-06-18 の poster payload は `group_url_name` を明示していなかったため、implicit General sharing が起きた可能性を current 仮説として追う。ただしこれは root-cause 仮説であって、team-only の証明でも否定でもない。local source にはこの観測値を resend default として固定しない。2026-06-19 の hardening では、既存 item の PATCH でも `group_url_name` を既定送信しないことを `tests/test_qiita_frontmatter.py` で固定し、観測された `general` を通常更新で再宣言しない状態へ揃えた（2026-06-19 再実行で `104 passed`）。その反面、**current blocker の 3 本を狭い共有先へ寄せ直す remediation は、このツールの既定 PATCH 経路だけでは実施できない**。是正が必要なら Team UI か、`id` あり更新でも `group_url_name` を明示送信できる別改修が要る。
- 2026-06-19 の継続作業で、その「別改修」はローカル実装済みになった。`tools/qiita_team_post.py` は **既定 PATCH を維持したまま** `--patch-group-url-name` を付けたときだけ `group_url_name` を再送できる。ただしこれは **共有先 (`group_url_name`) を寄せ直すための opt-in 経路**であり、`private` 範囲の tightening そのものではない。現在の 3 本は source が `private:false` のため、この経路だけでは露出疑いを縮めず、`private:true` へ直す別アクションが要る。Qiita API docs の `PATCH /api/v2/items/:item_id` に `group_url_name` 記載がある**可能性**はあるが、この点自体は現時点で一次未確認であり、**既共有 item に対して実際に締め直し効果を持つかも一次未確認**と扱う
- 同日のローカル再確認では、Team stock 3 本の source frontmatter にはまだ `group_url_name` が無く、`py -3.11 tools/qiita_team_post.py dry-run <file> --patch-group-url-name` は 3 本とも `PATCH_GROUP_URL_NAME_BLOCK` で停止した。従って opt-in PATCH 経路は実装済みだが、**human が concrete target を決めて source に入れるまでは実行不能**である。`general` を local source の resend default として焼き戻すことはしない
- 外部 remediation の優先順位は **containment first / Team UI diagnosis second / opt-in PATCH last** で固定して再開する。理由は、opt-in PATCH は `group_url_name` 再送のローカル経路と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate は、まず `py -3.11 tools/qiita_team_post.py preflight <item_id>...` で token validity / 対象 team membership / readable item を確認し、失敗したら診断不成立で停止する。このとき `preflight` / `verify` / `show` / `post` は採用 token source も出力し、`qiita_token` fallback は warning ではなく 4 コマンド自体を **BLOCK** する。その後 3 本それぞれの intended state baseline を固定する。3 種再取得 `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで実施し、未認証 HTML GET / public direct probe で `200` かつ本文確認が取れた記事は Team UI 完了を待たずその記事だけ fail-closed 候補へ送る。`302 / 403 / 404` は negative ではなく未確定として扱い、public 面しか叩けない probe は無情報として扱う。それ以外の記事について Team UI を read-only で見て intended share target / private state を確認し、同ターン再取得した 3 ソースと source frontmatter を突き合わせて share target 起因 / private state 起因 / 未確定 に 1 行判定する。no-op retain は 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。それで不足する場合に限って、concrete `group_url_name` を source に入れた opt-in PATCH を候補化する
...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順だけでなく、**複数 api-keys ファイルをまたいでも Team token を先に拾う 2 パス探索**へ直し、token source surfacing も入った。空白だけの `QIITA_TEAM_TOKEN` と **空白だけの file token** は fallback を阻害しないよう skip し、`show` / `preflight` も未知フラグを fail-closed で拒否する。readback 側も `200/201` かつ空 `url` を success 扱いせず BLOCK に倒し、`cmd_post` の write 検証は **POST/PATCH レスポンス echo ではなく独立 GET の read-after-write** を authoritative source に戻した。create 成功時は **サーバが返した id を先に frontmatter へ永続化**して重複 POST を防ぎ、その直後は `qiita_team_verified:false` を残して live/未検証を明示する。未検証 marker は `false` だけでなく **未記入 (`None`) でも** pre-PATCH authoritative readback を起動し、read 自体が壊れていれば BLOCK する一方、corrective PATCH の drift は表示して続行する。独立 GET では `private` に加えて **`group.url_name` も intended target と突合**し、`--patch-group-url-name` 無しの PATCH でも **pre/post で group が不変だったことは assert** する。ただしこの非 resend PATCH は **`qiita_team_verified:true` へ昇格しない設計**であり、3 本を verified 化したいときは **frontmatter に concrete `group_url_name` を明記したうえで `--patch-group-url-name` を付ける運用が前提**である。完全に検証された target へ更新するときだけ `qiita_team_verified:true` に戻し、group 未照合の PATCH や read-after-write 失敗後は `qiita_team_verified:false` のまま残す。`qiita_team_verified` は tool 自動更新のみ・手編集禁止とする。readback FAIL は **write 未達と同義ではなく landed の可能性あり**としてログへ明示する。独立 GET の 404 は短い retry/backoff を入れ、直後の反映遅延だけは吸収する。`LOCAL PATH` / `NON-PUBLIC IMAGE` は `dry-run` / `post` の両方で block し、default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。ここで言う `preflight` / `group-aware PATCH semantics` / `temporary private 化 gate` / `public direct probe` は **Qiita 公式仕様ではなく社内設計の安全確認フロー**である。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `90 passed, 52 deselected`、フルは `142 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む
- ただし personal-token 判定自体は **source 名のヒューリスティック**であり、鍵名だけでは Team 能力を証明しない。最終 authority は `preflight` の `GET /authenticated_user` と `GET /items/<id>` による auth / membership / readability 確認であり、external diagnosis / write の前には必ずこの preflight を通す。

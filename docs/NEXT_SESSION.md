...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順だけでなく、**複数 api-keys ファイルをまたいでも Team token を先に拾う 2 パス探索**へ直し、token source surfacing も入った。空白だけの `QIITA_TEAM_TOKEN` と **空白だけの file token** は fallback を阻害しないよう skip し、`show` / `preflight` も未知フラグを fail-closed で拒否する。readback 側も `200/201` かつ空 `url` を success 扱いせず BLOCK に倒し、`cmd_post` の write 検証は **POST/PATCH レスポンス echo ではなく独立 GET の read-after-write** を authoritative source に戻した。create 成功時は **サーバが返した id を先に frontmatter へ永続化**して重複 POST を防ぎ、その直後は `qiita_team_verified:false` を残して live/未検証を明示する。未検証 marker は `false` だけでなく **未記入 (`None`) でも** pre-PATCH authoritative readback を起動し、read 自体が壊れていれば BLOCK する一方、corrective PATCH の drift は表示して続行する。独立 GET では `private` に加えて **`group.url_name` も intended target と突合**し、group を照合した経路でのみ `qiita_team_verified:true` に戻し、group 未照合の PATCH は `qiita_team_verified:false` のまま残す。**更新時は原則 `--patch-group-url-name` 必須**、`qiita_team_verified` は tool 自動更新のみ・手編集禁止とする。readback FAIL は **write 未達と同義ではなく landed の可能性あり**としてログへ明示する。独立 GET の 404 は短い retry/backoff を入れ、直後の反映遅延だけは吸収する。`LOCAL PATH` / `NON-PUBLIC IMAGE` は `dry-run` / `post` の両方で block し、default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。ここで言う `preflight` / `group-aware PATCH semantics` / `temporary private 化 gate` / `public direct probe` は **Qiita 公式仕様ではなく社内設計の安全確認フロー**である。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `87 passed, 52 deselected`、フルは `139 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

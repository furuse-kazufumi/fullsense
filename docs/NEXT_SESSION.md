...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順に評価するように戻し、token source surfacing も入った。readback 側も `200` かつ空 `url` を success 扱いせず BLOCK に倒し、default `scan` は `.md` を優先して `.md.bak` との重複を正規化する。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `63 passed`、フルは `115 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

...
- `#37` live PATCH の runbook は次の 5 段で固定する。ここで言う `preflight` は CORS の OPTIONS ではなく、**このラッパー独自の pre-patch health check** である。`ignorePublish: true` は `qiita-cli` 系の安全柵であり、2026-06-19 の hardening 後は **`tools/qiita_public_post.py` でも `--force-ignore-publish` が無い限り fail-closed で停止**する。
  1. `python tools/qiita_public_post.py preflight tools/qiita-cli-poc/public/qiita37_gpu_triple_run_gate_price_kamikudaki.md`
     `PATCH update public_id=f06ca92ea208c7646fcd` / `WARNING: frontmatter ignorePublish: true is a qiita-cli gate, not a qiita_public_post.py gate.` / `private: False` / `auth_status: 200` / `api_status: 200` / `api_private: False` / `html_status: 200` / `baseline_body_preserved: True` / `asset_count: 3` / `preflight: OK` を再確認する。`api_title` と payload title の不一致、live `private` と payload visibility の不一致、`.remote` baseline 本文の欠落は fail-closed で BLOCK される。`updated_at` は payload に乗らないので、この段階では送信対象外として扱う。
  2. `git diff --no-index --word-diff=color tools/qiita-cli-poc/public/.remote/f06ca92ea208c7646fcd.md tools/qiita-cli-poc/public/qiita37_gpu_triple_run_gate_price_kamikudaki.md`
     を実行し、companion-preserved switcher とマスコット画像 + 📗導入を含む companion 本文が local source 側に保全されていることを明示確認する。`preflight_remote_baseline` による機械ゲートは入っているが、**承認者がこの word-diff 出力を目視することを human-gate の必須条件**とする。

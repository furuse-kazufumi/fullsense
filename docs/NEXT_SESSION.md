...
- その後の追加 hardening で、`resolve_token()` による token source surfacing と `qiita_token` fallback warning も入った。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `55 passed`、フルは `107 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

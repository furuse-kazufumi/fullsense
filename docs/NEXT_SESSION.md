...
- 外部 remediation の優先順位は **containment first / Team UI diagnosis second / opt-in PATCH last** で固定して再開する。理由は、opt-in PATCH は `group_url_name` 再送のローカル経路と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate は、まず 3 本それぞれの intended state baseline を固定する。未認証 HTML GET / public direct probe で `200` かつ本文確認が取れた記事は Team UI 完了を待たずその記事だけ fail-closed 候補へ送る。`302 / 403 / 404` は negative ではなく未確定として扱う。それ以外の記事について Team UI を read-only で見て intended share target / private state を確認し、同ターン再取得した Team API GET / 未認証 HTML GET / direct probe / source frontmatter と突き合わせて share target 起因 / private state 起因 / 未確定 に 1 行判定する。no-op retain は 1 回まで、時間上限は次回 human-gate までに留め、2 回目に入る前には暫定 private 化を含む fail-closed 分岐を human-gate に出す。それで不足する場合に限って、concrete `group_url_name` を source に入れた opt-in PATCH を候補化する

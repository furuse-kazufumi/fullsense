...
- その後の追加 hardening で、`resolve_token()` は `qiita_team_token` / `QIITA_TEAM_TOKEN` / `qiita_token` の順に評価するように戻し、token source surfacing も入った。`pytest tests/test_qiita_frontmatter.py -k qiita_team_post` は `60 passed`、フルは `112 passed`。以後の diagnosis gate は token source warning の無い turn を前提に読む

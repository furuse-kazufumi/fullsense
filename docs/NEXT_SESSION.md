...
- 外部 remediation の優先順位は **Team UI first / opt-in PATCH second** で固定して再開する。理由は、opt-in PATCH は `group_url_name` 再送のローカル経路と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate は、まず Team UI で intended share target / private state を確認・是正できるかを見る。それで不足する場合だけ、concrete `group_url_name` を source に入れた opt-in PATCH を候補化する

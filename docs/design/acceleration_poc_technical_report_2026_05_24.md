...
## 4.5 速度以外の評価 (較正 / 安定性 / 敵対的・正確性)

「単に高速 ≠ 採用」をさらに進め、速度以外の軸を PoC 化 (ユーザー選択)。

**較正 (calibration, `llive/src/llive/perf/calibration_bench.py`)** — 速度 PoC が仮定する
推定器精度の妥当性。confidence の ECE/Brier:

| bias | ECE | claimed | realized | gap |
|---|---|---|---|---|
| +0.00 (較正済) | 0.039 | 0.700 | 0.702 | -0.001 |
| +0.10 (over) | 0.102 | 0.800 | 0.702 | +0.099 |
| +0.20 (over) | 0.198 | 0.900 | 0.702 | +0.198 |

→ over-confidence は claimed > realized = **速度 PoC が speedup を過大評価**。推定器の較正が前提。

**安定性 (stability, 同 module)** — Antifragile 脱出率を 8×15=120 seeds で測定:
mean=1.00 / std=0.000 / min=1.00 → **seed 運でなく頑健** (脱出 100% は安定)。

**敵対的・正確性 (`llmesh/speculative/adversarial_bench.py`)**:

- fail-closed 拒否 (実 coordinator): forged signature / malformed hex / tampered branch を**全拒否**、正規は受容。
- poisoned result: **Ed25519 は manifest 真正性のみ保証、結果の正しさは別**。

| Byzantine 率 | 正答率 (結果検証なし→あり catch 0.9) |
|---|---|
| 10% | 90% → 99% |
| 30% | 71% → 97% |
| 50% | 51% → 95% |

→ **重要発見**: 投機 mesh は署名検証に加え**結果検証 (cross-check/gate) が必須** (要件
`SPEC-MESH-11` に反映)。署名だけでは Byzantine の poisoned result を防げない。

**眼鏡への反映**: `tools/poc_lens.py` の感度分析が「IBPO は推定器ノイズで最も脆い」と
flag。較正 PoC がそれを ECE/gap で定量裏付け。眼鏡が価値を実証したら lleval へ組込む構想
([[project_poc_lens_lleval_integration]], 結合判断はユーザー)。

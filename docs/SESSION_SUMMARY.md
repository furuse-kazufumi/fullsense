...
- 次セッションの最初の一手は、まず **Holland / Koza の書誌事実を API 優先で固めること**。Holland は Crossref / doi content negotiation の一致結果を handoff に転記し、Koza は MIT Press 書籍の DOI 有無を Crossref / WorldCat / 出版社書誌で確認する。そのうえで human-gate の選択結果を受け、選択肢 1 なら別 egress を含む対話ブラウザで 2 件の canonical permalink を開き、`ログイン不要で書誌ページが表示されるか` を reader reachability の補助証跡として handoff に戻す。片方だけ失敗したらその 1 件にだけ非 Akamai 系の補助導線を併記し、2 件とも失敗したら両方に併記する。選択肢 2 なら補助導線の併記方針だけを先に確定し、選択肢 3 なら `qiita44` は保留のまま他のローカル整備へ移る。

...
- 外部 remediation の優先順位は、現時点では **containment first / Team UI diagnosis second / opt-in PATCH last** で置く。理由は、opt-in PATCH はローカル実装と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate では、まず 3 本それぞれの intended state baseline を固定し、token validity を確認したうえで 3 種再取得 `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直す。未認証 HTML GET / public direct probe で `200` かつ本文断片一致が取れた記事は、その時点で記事単位の fail-closed 候補へ送る。`302 / 403 / 404` や空本文は未確定であり、安全証明に使わない。temporary tightening は diagnosis と別 gate で扱い、after-state は root-cause 判定ではなく containment 完了確認として読む。UI で表現できない場合に限って concrete `group_url_name` を source に入れた opt-in PATCH を検討する

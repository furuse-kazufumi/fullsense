...
- 2026-06-18 の Team stock POST では、`private:true` を意図した 3 本が API GET ではすべて `private:false` で着地した。さらに 2026-06-19 の未認証 HTML GET では 3 本とも Login ページへ落ちたため、現時点の失敗教訓は「即一般公開」そのものより、**Team API / visibility semantics が食い違いうる**ことにある。
...
- 注意: 2026-06-19 時点の `qiita_team_post.py` は `ignorePublish:true` を読み、`post --yes --force-ignore-publish` を明示しない限り fail-closed で停止する。ただし Team 上の可視範囲意味論は、API `private:false` と未認証 HTML Login が食い違っており、十分に確定できていない。詳細条件は `team_stock_publish_plan.md` を正本とする
- deindex:

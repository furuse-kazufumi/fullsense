...
- さらに `qiita_public_post.py` には warning も足し、**`id` はあるが `public_id` が無い** source を dry-run / post したときは `POST create` に倒れる前に気づけるようにした。`#37` companion は `public_id` 付きなので warning なしで PATCH source として読める。

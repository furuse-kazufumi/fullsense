...
4. `qiita46_llterm_supervision_first.md` は日本語草稿を第8章まで通し、休憩ポイント / 参考文献 / HTML annotation / `025.jpg` まで入った。`025.jpg` は言語ごとに作品名を localize しつつ raw URL の HTTP 200 も確認済み。`qiita46_llterm_supervision_first_kamikudaki.md` は磨き込み済み、さらに en/zh/ko draft も第6章まで同期した。残る publish gate は、多言語本文の第7章以降に加え、翻訳版 endmatter 一式（関連記事 / 参考文献 / HTML annotation メタタグ）の同期と最終見直し。

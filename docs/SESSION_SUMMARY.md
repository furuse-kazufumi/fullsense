...
- そのうえで `#37` live PATCH runbook も 4 段へ固定した。step1 は `python tools/qiita_public_post.py preflight ...` で `PATCH update public_id=f06ca92...` / warning 無し / `private: False` / `auth_status: 200` / `api_status: 200` / `html_status: 200` / `asset_count: 3` / `preflight: OK` を確認し、ブラウザで `f06ca92...` live item が short companion 本文であることを目視確認してから、`post ... --yes` を打つ。step4 では `python tools/qiita_public_post.py preflight ... --require-marker` で `marker_present: True` と `preflight: OK` を確認し、あわせて `kamikudaki_shishi.svg` を含む live SVG / 画像描画も目視確認する。Qiita / imgix 側キャッシュで古い描画が残る場合は `?v=20260619N` cache-bust を足して再 publish する。`ignorePublish: true` はこの public poster の実ゲートではなく、`#37` source は `public_id` 付きなので `--allow-create` は不要である。
...
- さらに 2026-06-19 の pre-patch baseline として、`preflight` は `PATCH update public_id=f06ca92...` / warning 無し / `private: False` / `api_status: 200` / `html_status: 200` / `asset_count: 3` / `preflight: OK` を返すことまで読み取り専用で確認した。

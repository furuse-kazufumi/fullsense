...
- 外部 remediation の優先順位は、現時点では **containment first / Team UI diagnosis second / opt-in PATCH last** で置くのが妥当である。理由は、opt-in PATCH はローカル実装と fail-closed guard までは確認済みでも、既共有 item に対する締め直し効果が一次未確認だからである。次の human-gate では、まず 3 本それぞれの intended state baseline を固定し、3 種再取得 `Team API GET / 未認証 HTML GET / public direct probe` を同ターンで採り直す。未認証 HTML GET / public direct probe で `200` かつ本文確認が取れた記事をその時点で fail-closed 候補へ送る。`302 / 403 / 404` は未確定であり、安全証明に使わない。それ以外について Team UI で intended share target / private state を read-only で確認し、UI で表現できない場合に限って concrete `group_url_name` を source に入れた opt-in PATCH を検討する

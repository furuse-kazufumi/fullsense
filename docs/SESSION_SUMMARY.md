...
- 2026-06-19 の editorial 方針追加として、「公開済みの短い companion 記事は、十分に長い本編へ統合できるなら統合を優先し、空いた item は新記事の器として再利用する」を採用した。現時点でこの候補に最も近いのは public 済み `qiita37_gpu_triple_run_gate_price_kamikudaki.md` (`id: f06ca92ea208c7646fcd`) で、受け皿は完全版 `qiita37_gpu_triple_run_gate_price.md`。`qiita43/46` のかみくだき版と `qiita47` はまだ local-only / private draft なので、統合判断は publish 前に持ち越せる。
...
- `#37` の短稿と完全版を本文比較した結果、短稿の独自価値だった `4 文指示 / 実験 3 本 / PoC 裏方 / 17〜19 倍 / 0 円 / safety tax 接続` は完全版の該当節へ概ね吸収済みだった。なので次の外部アクション案は、完全版へ大きく追記するより **短稿 item を redirect / 索引本文へ差し替え、完全版を正本へ寄せる**ほうが筋が良い。

...
- `qiita44` の publish 前残タスクは、CMA-ES の旧 PDF 差し替えや lexicase 追加ではなく、本文の言い回し強度と参考導線の最終突合、ならびに MIT Press 2 件の到達性確認である。2026-06-18 時点では `https://mitpress.mit.edu/9780262082136/adaptation-in-natural-and-artificial-systems/` と `https://mitpress.mit.edu/9780262111706/genetic-programming/` に対し、PowerShell `Invoke-WebRequest` + UA `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36` + Cookie なしで 403 のままだった。原因は bot 判定だけでなく、購読 / 認証 / 地域制限を含む複合要因の可能性があるため、実ブラウザで開けるかを確認するか、別の公式導線へ差し替えるかを publish 前に決める。なお、この時点では IP / ISP / VPN 状態までは handoff に固定記録していない。

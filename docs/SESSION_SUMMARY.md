...
- 同じ API GET では `group.url_name: general` / `group.private:false` / `organization_url_name:null` も 3 本とも一致した。2026-06-18 の poster payload では `group_url_name` を送っていなかったため、implicit General sharing が起きた可能性を current 仮説として追う。ただしこれは root-cause 仮説であって、team-only の証明でも否定でもない。local source にはこの観測値を resend default として固定しない。2026-06-19 の hardening では、既存 item の PATCH でも `group_url_name` を既定送信しないことを `tests/test_qiita_frontmatter.py` で固定し、観測された `general` を通常更新で再宣言しない状態へ揃えた（2026-06-19 再実行で `76 passed`）。その反面、**current blocker の 3 本を狭い共有先へ寄せ直す remediation は、このツールの既定 PATCH 経路だけでは実施できない**。是正が必要なら Team UI か、`id` あり更新でも `group_url_name` を明示送信できる別改修が要る。
- 2026-06-19 の最新ローカル到達点として、`#37` short companion の public PATCH は完了済み、`tests/test_qiita_frontmatter.py` は再実行で `76 passed`、worktree は clean である。したがって新セッションの再開点は `#37` ではなく、**Qiita Team 3 本の visibility 問題を current blocker としてどう潰すか**に一本化される。
...
7. 新セッションの最初の具体的一手は、`docs/articles/2026-06-18/team_stock_queue.md` と `team_stock_publish_plan.md` を正本として開き、**既定 PATCH 経路では remediation 不可**という制約の下で、次に取る選択肢を二択に絞ること。すなわち `(a) Team UI で共有先/可視範囲を手動是正する human-gate 案` と `(b) `id` あり更新でも `group_url_name` を明示送信できる別改修をローカルで設計する案` のどちらを先に詰めるかを最初に決める。どちらでもない作業へ横滑りしない。

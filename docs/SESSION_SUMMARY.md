...
- そのうえで `#37` live PATCH runbook も 4 段へ固定した。`dry-run` で `PATCH update public_id=f06ca92...` / warning 無し / `private: False` を再確認し、ブラウザで `f06ca92...` live item が short companion 本文であることを目視確認してから、`post ... --yes` を打ち、最後に API / HTML で canonical 告知反映と item id 不変を確認する。`#37` source は `public_id` 付きなので `--allow-create` は不要。

...
私たちは **llcore (エルコア)** という研究基盤で、小さな AI の「頭の部品」を生き物の進化みたいに少しずつ作り変えています。CPU だけ・自分のPCの中だけ・お金ゼロで動く、という縛りつき。

進化で作った部品には当たり外れがあります。困るのが「**暴走する部品**」。同じ計算を繰り返すうちに数字がどんどん膨らんで発散してしまう、いわば「アクセル踏みっぱなしで止まらない車」みたいな部品です。
...
「落ち着いていく」性質を、研究では **収縮 (しゅうしゅく)** と呼びます。一歩進むたびに距離が縮む、坂を転がるビー玉が最後は谷底で止まる、みたいなイメージ。そして「この部品はちゃんと収縮します」という**数学的な証明書**を発行できれば、安心して通せます。

この証明書を探す計算方法にはいくつか流派があって、本命は **SDP** という凸最適化の手法。「都合のいい証明書 (P という行列) を、決められたルールの中で探してくる」イメージです。

ちなみに、当初は「論理パズルを解く高級ソルバ (SMT/Z3)」も検査の柱になるかと期待していました。でも調べると、この基盤での Z3 は**飾り**でした。何千・何万件で照合しても、Z3 の判定は単純な閉じた式の計算と**完全に一致**(食い違いゼロ)。つまり Z3 は識別力を一切足しておらず、収縮の本質をつかんでいたのは**やはり SDP の証明書**の方だった、というのが下地の理解です。

---

## 第2幕: 自慢の成績表が出た ── でも、良すぎた

検査器の実力を測りました。「次数を上げる (=もっと手の込んだ証明書を許す)」と、捕まえられる部品がどんどん増える、という**段階的な成績ラダー**を作ったのです。
...
これ、研究者にとっては「**おいしい**」結果なんです。なぜなら「次数4でしか捕まらない子」「次数6でしか捕まらない子」が別々にいる、つまり**次数を上げるたびに新しい獲物が増える**=「証明能力に豊かな階層がある」ように見えるから。論文的に映える、いい絵です。

…が。ここで [[honest disclosure 規律]] が発動します。FullSense の研究には鉄の掟があって、

> **異常に良い結果が出たら、勝った気になる前に必ず内訳を疑え。**

成績表が「豊かすぎ・都合よすぎ」だったので、素直に喜ばず疑いにかかりました。

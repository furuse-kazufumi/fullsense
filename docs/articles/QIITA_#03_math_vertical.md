...
>
> 今回の vertical は **MATH-01 (単位次元解析)** と **MATH-08 (内蔵計算エンジン)** から入る。つまり「LLM に計算を任せず、まず拒否できるところを決定論的に止める」が最初の設計原則になる。
...
- `5 m/s + 3 s = 8` のような **cross-quantity mismatch** について、low-level direct reject はすでに実装済み。現時点では、既知の value+unit 表現を grounding しつつ、実 Brief 文脈でどの mismatch を end-to-end に surface / stop するかを詰めている段階
...
最小依存版の **grounding / 次元表現スケルトン** を自前実装 (Pint 等の外部ライブラリ不要)。

```python
from llive.math import Quantity, parse_unit
...
実装の核は `Dimensions(m, kg, s, A, K, mol, cd)` の 7 次元ベクトルだ。**低レベルの `Quantity` API では direct な次元不一致加算をすでに reject** する。一方で、Brief から拾った実文脈の mismatch を **どの粒度で surface / stop するかという end-to-end の方針**は次イテレーションで詰める。
...
ここまでは「LLM に **計算させない**」設計だった。算術式を奪い取って決定論エンジンに渡し、既知の value+unit 表現を grounded 化するところまでは入った。だが **`5 m/s + 3 s` のような cross-quantity mismatch をどの粒度で止めるかは、まだ次イテレーションの論点**だ。さらに、LLM が吐くのは数値だけではない。`x² + x = 2x³` のような **誤った数式そのもの** を、もっともらしい言葉で書いてきたら? 式を奪う前に、その式が正しいかどうかを機械的に反証できなければ、検証は片肺だ。
...
# The First Step Toward an AI That Is Strong at Math and Units
...
>
> This first vertical enters through **MATH-01 (dimensional analysis of units)** and **MATH-08 (the built-in calculation engine)**. In other words, the initial design principle is: **do not let the LLM calculate; first stop what can be deterministically refused**.
...
- In the same 2026-05-17 session we implemented the minimal skeletons of **MATH-01 (SI unit dimensional analysis)** + **MATH-08 (the built-in calculation engine skeleton)**, adding 47 tests
- For cross-quantity mismatches such as `5 m/s + 3 s = 8`, the low-level direct-reject path already exists. At the current stage, the system grounds known value+unit expressions while tightening which mismatches should surface / stop end-to-end in real Brief context
...
| Unit dimensions | `5 m/s + 3 s = 8` | SI dimensional grounding. Low-level direct reject exists; end-to-end policy is next iteration (MATH-01) |
...
Implemented in-house as the **grounding / dimensional-representation skeleton** with minimal dependencies (no external libraries such as Pint required).

```python
from llive.math import Quantity, parse_unit

# Treat known value+unit expressions as grounded quantities
v = Quantity(5.0, parse_unit("m/s"))
t = Quantity(3.0, parse_unit("s"))
...
The core of the implementation is the 7-dimensional vector `Dimensions(m, kg, s, A, K, mol, cd)`. **At the low-level `Quantity` API, direct dimension-mismatch addition is already rejected.** What remains for the next iteration is the **end-to-end policy**: how mismatch patterns extracted from real Brief text should surface and stop.

### Derived Units

`N` (kg·m/s²) / `J` (kg·m²/s²) / `W` (kg·m²/s³) / `Pa` (kg/m/s²) / `Hz` (1/s) / `C` (s·A) / `V` (kg·m²/s³/A) / `ohm` — only the frequently-used range is implemented.

## MATH-08 — Built-in Calculation Engine (Minimal Skeleton)
...
| **MATH-08** | **Built-in calculation engine (core of the calculation sidecar)** | ✅ implemented 4th |
...
So far the design has been to **not let the LLM compute**. We now have the step that snatches arithmetic expressions away and grounds known value+unit forms into a deterministic representation. But **exactly how to stop cross-quantity mismatches such as `5 m/s + 3 s` is still a next-iteration question**. And numbers are not the only thing an LLM emits. What if it writes out a **wrong formula itself**, like `x² + x = 2x³`, dressed up in plausible words? If we cannot mechanically disprove whether a formula is correct before we take it away, the verification is running on one lung.
...
>
> 这个第一个 vertical 会从 **MATH-01（单位量纲分析）** 和 **MATH-08（内置计算引擎）** 切入。换句话说，最初的设计原则是：**不要让 LLM 自己算，先把能够确定性拒绝的地方机械地拦下来**。
...
- 在 2026-05-17 的同一会话中，我们实现了 **MATH-01 (SI 单位量纲分析)** + **MATH-08 (内置计算引擎的最小骨架)**，并新增了 47 个测试
- 对于 `5 m/s + 3 s = 8` 这类 **cross-quantity mismatch**，low-level direct reject 已经实现。现阶段是在把已知的 value+unit 表达 grounded 化的同时，继续收紧在真实 Brief 语境下哪些 mismatch 应该 end-to-end 地 surface / stop
...
| 单位量纲 | `5 m/s + 3 s = 8` | SI 量纲 grounding。low-level direct reject 已有，end-to-end 方针在下一次迭代（MATH-01） |
...
自行实现了 **grounding / 量纲表示骨架** 的最小依赖版 (无需 Pint 等外部库)。

```python
from llive.math import Quantity, parse_unit

# 把已知的 value+unit 表达当作 grounded quantity 来处理
v = Quantity(5.0, parse_unit("m/s"))
t = Quantity(3.0, parse_unit("s"))
...
实现的核心是 `Dimensions(m, kg, s, A, K, mol, cd)` 这个 7 维向量。**在低层 `Quantity` API 上，直接的量纲不一致加法已经会被 reject。** 仍然留到下一次迭代的，是 **真实 Brief 文本中抽出的 mismatch 到底该如何 surface / stop 的 end-to-end 方针**。
...
| **MATH-08** | **内置计算引擎（计算 sidecar 的核心）** | ✅ 4th 已实现 |
...
到目前为止的设计都是「**不让** LLM 计算」。我们已经做到把算术式夺过来，并把已知的 value+unit 表达 grounded 到确定性表示上。但 **像 `5 m/s + 3 s` 这样的 cross-quantity mismatch 到底要以什么粒度拦下，仍然是下一次迭代的论点**。而且 LLM 吐出的不只是数值。如果它用一套像模像样的措辞，写出 `x² + x = 2x³` 这样**本身就错的数式**，又该怎么办？如果在夺走式子之前，无法机械地反证这个式子是否正确，那这套验证就只用了一半的肺。
...
>
> 이 첫 vertical 은 **MATH-01(단위 차원 해석)** 과 **MATH-08(내장 계산 엔진)** 에서 출발한다. 즉, 첫 설계 원칙은 **LLM 에게 계산을 맡기지 말고, 먼저 결정론적으로 거부할 수 있는 지점을 기계적으로 막는 것**이다.
...
- 2026-05-17 같은 세션에서 **MATH-01 (SI 단위 차원 해석)** + **MATH-08 (내장 계산 엔진의 최소 skeleton)** 의 minimal skeleton을 구현, 테스트 47건 추가
- `5 m/s + 3 s = 8` 같은 **cross-quantity mismatch** 에 대해서는 low-level direct reject 가 이미 구현되어 있다. 현 단계에서는 알려진 value+unit 표현을 grounded 화하면서, 실제 Brief 문맥에서 어떤 mismatch 를 end-to-end 로 surface / stop 할지 더 조이는 단계다
...
| 단위 차원 | `5 m/s + 3 s = 8` | SI 차원 grounding. low-level direct reject 는 구현済み, end-to-end 방침은 다음 이터레이션 (MATH-01) |
...
**grounding / 차원 표현 스켈레톤** 의 최소 의존성 버전을 자체 구현 (Pint 등 외부 라이브러리 불필요).

```python
from llive.math import Quantity, parse_unit

# 알려진 value+unit 표현을 grounded quantity 로 다룬다
v = Quantity(5.0, parse_unit("m/s"))
t = Quantity(3.0, parse_unit("s"))
...
구현의 핵심은 `Dimensions(m, kg, s, A, K, mol, cd)` 라는 7차원 벡터다. **저수준 `Quantity` API 에서는 direct 한 차원 불일치 덧셈이 이미 reject 된다.** 다음 이터레이션으로 남는 것은, **실제 Brief 문맥에서 뽑힌 mismatch 를 어떤 방식으로 surface / stop 할지에 대한 end-to-end 방침**이다.

### 유도 단위

`N` (kg·m/s²) / `J` (kg·m²/s²) / `W` (kg·m²/s³) / `Pa` (kg/m/s²) / `Hz` (1/s) / `C` (s·A) / `V` (kg·m²/s³/A) / `ohm` — 자주 쓰이는 범위만 구현.

## MATH-08 — 내장 계산 엔진 (최소 skeleton)
...
| **MATH-08** | **내장 계산 엔진 (계산 sidecar 의 핵)** | ✅ 4th 구현 완료 |
...
여기까지의 설계는 「LLM에게 **계산시키지 않는다**」였다. 산술식을 빼앗아 결정론 엔진 쪽으로 넘기고, 알려진 value+unit 표현을 grounded 화하는 단계까지는 들어왔다. 하지만 **`5 m/s + 3 s` 같은 cross-quantity mismatch 를 정확히 어떤 단위로 막을지는 아직 다음 이터레이션의 논점**이다. 그리고 LLM이 토해내는 것은 수치만이 아니다. `x² + x = 2x³` 같은 **틀린 수식 그 자체** 를 그럴듯한 말로 써 온다면? 식을 빼앗기 전에 그 식이 옳은지를 기계적으로 반증할 수 없다면, 그 검증은 한쪽 폐로만 숨 쉬는 셈이다.

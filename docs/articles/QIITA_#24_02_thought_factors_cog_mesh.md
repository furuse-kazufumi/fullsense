...
> **draft 段階** (2026-05-21 marathon) — full 10x volume (80-120k 字) 版は
> 次セッション以降. 本 draft は骨子 + 主要 5 因子の深掘り + COG-MESH 概要 +
> 最新成果 + 期待値 sections.
>
> ⚠ **Cross-link 注意**: 本文中の `#24-XX` / ``QIITA_#24_XX_*` (内部参照)` 形式の
> 他記事参照は **draft 仮 link**. Qiita 投稿後に確定する個別記事 URL
> (`https://qiita.com/.../items/<hash>`) に **一括置換** が必要.
> mapping は [`QIITA_#24_LINK_MAP.md`](QIITA_#24_LINK_MAP.md) に集約.
> 投稿時に追々修正.
...
ユーザー由来の 10 軸 (`project_llive_cog_fx_factors` (内部参照)). 元ネタは
...
`project_cog_mesh_implementation_2026_05_19` (内部参照). 10 因子に **1 機構ずつ** 対応:
...
`project_llmesh_neuro_long_term` (内部参照). Raptor RAD に bci / neuroscience /
...
`feedback_rust_usage_matters` (内部参照) 判定表の **A パターン** (純 Python ループ

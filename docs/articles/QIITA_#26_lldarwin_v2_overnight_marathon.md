---
title: '一晩で AI 進化を作り直した — 実 LLM 12h ランがまた満点で飽和し、6 本の PoC と 4 体の Agent と Perplexity が「独立に同じ結論」へ収束した夜 #26（開放端進化 / ライブ・オーケストラ / honest cross-validation）'
tags:
  - FullSense
  - llive
  - 進化計算
...
# 一晩で AI 進化を作り直した — 実 LLM 12h ランがまた満点で飽和し、6 本の PoC と 4 体の Agent と Perplexity が「独立に同じ結論」へ収束した夜 #26
...
> 🤔 **たとえ話（漫才風）**:
> ボケ「今度は本物の先生（実 LLM）に採点させたで！」
> ツッコミ「で、点は?」
> ボケ「5 回目のテストで全員 100 点や。そっから 65 回テストしても全員 100 点」
> ツッコミ「それ先生の問題ちゃう、**問題用紙が簡単すぎる**んや! 本物の先生に替えても、テストが同じやったら一緒やろ!」
> ——眼鏡（採点者）を proxy から実 LLM に交換しても、ものさし（問題）が固定なら飽和する。これが「正直な不合格」の正体です。
...
> 🤔 **たとえ話（落語風）**:
> 「先生、生徒が全員 100 点取っちまったんで、問題を難しくしやした」
> 「ほう、で、点は割れたかい」
> 「割れやした。けど今度は全員、同じ難問の解き方ばっかり覚えて、金太郎飴で」
> 「そりゃ困った。難しくするだけじゃ駄目だ。**変わった解き方にもご褒美**をやんな」
> 「へい、そうしたら能力も多様性も両立しやした」
> ——「難しくする（適応難易度）」と「変わり者を褒める（novelty）」の二刀流。これが PoC #2 のサゲです。
...
良い部品を作り、集約せずに束ね、実 LLM で飽和を確かめ、開放端な選択へ作り直す。そして 6 通りの独立検証が同じ結論に着いたとき、ようやく「方策が決まった」と言える。次の #27 では、この確定方策に**自分で反証をぶつけます**——「眼鏡が曇ると淘汰も無力」を、Goodhart's law と proxy の限界とともに、いちばん痛いところから正直に晒します。
...
- 連載 #27「眼鏡が曇ると淘汰も無力」— 反証調査・Goodhart's law（本記事の続き / honest disclosure）
...
<!-- NUMBERING NOTE: 設計編 #26(drafts/QIITA_#26_lldarwin_multi_pressure_selection.md) と本記事(#26 marathon climax)は同番。投稿時にどちらかを #26a/#26b 等へリナンバーするか、設計編をマージ判断する。 -->

...
게다가 SDP는 **약한 검사기가 합격시킨 아이를 전부 포함한 채로** 더 얹습니다. 속내는 간단합니다. **둥근 공은 "기울이지도 찌그러뜨리지도 않은, 특별한 타원"** 이기 때문입니다. 타원을 찾는 SDP는 후보 안에 둥근 공 자체도 포함해 찾고 있습니다. 그러니 공으로 증명할 수 있었던 아이는 타원으로도 반드시 증명할 수 있습니다. "약한 검사기는 통과했는데 SDP에서는 떨어지는" 아이는 한 명도 없습니다 — 실제로 §4의 대규모 검증에서도 한 명도 나오지 않았습니다. 그래서 사다리를 SDP까지 안심하고 올려도 됩니다.

---

## 4. 대규모에서도 무너지지 않았다 (3270개로 확인)

"300개에서 우연히 그렇게 보인 것 아닌가"를 없애기 위해, **3270개**에서 같은 것을 확인했습니다. 표본을 10배 이상으로 늘려도 같은 풍경이 보인다면, "마침 이 개체 풀이 그랬을 뿐"이라는 의심은 거의 지울 수 있기 때문입니다.
...
읽는 법은 두 가지입니다. 첫째, 95%라는 합격률은 300개일 때의 95.3%와 정확히 일치합니다. 10배 이상으로 키워도 커버리지는 흔들리지 않습니다. 둘째, "약한 검사기가 SDP를 이긴 횟수 = **0**". 3270개 가운데 "공으로는 증명되는데 타원으로는 증명되지 않는" 아이가 단 한 명도 없었다는 뜻입니다. §3에서 밝힌 속내 — 둥근 공은 특별한 타원에 지나지 않는다 — 가 대규모에서도 그대로 성립함을 보여 주는 실증입니다. SDP는 약한 검사기의 엄격한 상위 집합입니다.

솔직히 하나. "SDP가 약한 검사기를 이긴 차이"는 거칠게 세면 **+254**로 보이는 국면이 있었습니다. 원인은 판정을 실제로 계산하는 도구 (솔버)의 정밀도입니다. 합격선에 딱 걸친 답안지는 채점이 거친 채점자를 만나면 "불합격"으로 넘어가 버릴 수 있습니다 — 이것이 **거짓 음성 (실은 합격인데 불합격으로 오판)** 입니다. 경계에 붙은 개체에서 바로 이것이 일어나고 있었고, 정밀도 높은 채점자 (내점법 방식의 솔버 CLARABEL)로 다시 세면 **+692**. SDP의 우위는 보기보다 훨씬 컸습니다 (이 보정의 경위는 자매편 #35-02에서 다룹니다).

---

## 5. "최강 솔버를 얹으면 최강 검사기"는 틀렸다

여기서 많은 사람이 이렇게 생각할 겁니다. "사다리 위에 최강 논리 솔버 (SMT/Z3)를 얹으면 더 강한 검사기가 되지 않을까?" Z3란 "이 조건을 만족하는 답이 존재하는가?"라는 물음에 예/아니오로 결판을 내 주는 탐색형 논리 솔버입니다.
...
왜일까요? 이 문제의 "수축하는가?"는 실은 **닫힌 수식으로 다시 쓸 수 있는** 것이었습니다. "닫힌 식"이란, 삼각형 넓이가 "밑변 × 높이 ÷ 2"로 나오는 것처럼 **숫자를 대입하면 한 번에 답이 나오는 공식**을 말합니다. 공식에 대입하면 답이 나오는 문제에 굳이 무거운 탐색형 논리 솔버를 끌어와도, 나오는 답은 같습니다. 실제로도 그랬습니다. Z3의 판정과 공식의 판정은 3270개에서 불일치 0, 2만 회 돌려서 전부 일치 — Z3만이 알아볼 수 있었던 것이 단 하나도 없었습니다. "구별력 제로"란 이 뜻입니다.

이것은 중요한 정직한 공개입니다. "Z3로 증명했다"고 쓰면 강해 보이지만, 실제로는 **닫힌 식의 대수와 볼록성 정리**가 증명을 떠받치고 있으며, Z3는 부르든 안 부르든 같습니다. 건물에 비유하면 Z3는 하중을 받치는 기둥이 아니라 보기 좋으라고 세운 장식 기둥이었던 셈입니다. 오해하지 말아야 할 것은, 이것이 "Z3가 나쁜 도구"라는 이야기가 아니라는 점입니다. **이 문제가 마침 "공식으로 풀리는 문제"였을 뿐**입니다. 검사기를 한 단 더 올릴 가치가 진짜로 있던 것은 Z3가 아니라 SDP 쪽이었습니다.
...
왜 검사기를 바꾸기만 했는데 "좋음"의 천장이 움직일까요? 진화는 관문을 통과한 아이만을 다음 세대의 바탕으로 삼을 수 있기 때문입니다. 약한 검사기로는, §3대로 "돌면서 줄어드는" 아이가 전부 누명으로 거부됩니다. 그러면 그 일족은 한 번도 채택되지 못하고, 회전의 좋음이 필요한 영역에 진화는 영원히 발을 들이지 못합니다 — 그래서 약 0.41에서 정체합니다. SDP로 바꾸면 그 영역이 열려 약 0.86까지 늘어났습니다. 덧붙이면 이 차이는 순전한 우연으로는 약 10만 번에 3번밖에 나오지 않는 수준 (p = 3.1e-5)으로, 요행이라고 보기 어려운 차이입니다. **강한 검사기는 안전장치이자 동시에 진화의 활동 범위를 넓히는 장치**였습니다 — 이것이 실리의 정체입니다.

이 SDP 검사기는 더 이상 연구실 실험이 아니라, 운영 코드의 플러그형 부품으로 배선되어 있습니다. 설계는 fail-closed — 정확한 솔버 (CLARABEL)를 찾지 못하면, 정밀도가 떨어지는 대역으로 조용히 도는 대신 판정 자체를 거부합니다. §4에서 보았듯이 솔버의 정밀도가 떨어지면 경계 근처의 판정이 어긋나기 때문입니다. 테스트도 통과했고, 모두 CPU, $0, 가정용 PC에서 완결됩니다.
...
- **"차수를 올리면 반드시 강해진다"는 거짓.** 사다리의 윗부분 (고차 SOS)은 검사의 정교함 (차수)을 올리면 단조롭게 강해질 것 같지만, 실제로는 오히려 느슨해질 수 있습니다. 즉 깔끔한 일방통행 계단이 아닙니다. 그래서 운용은 "각 차수를 실측해 가장 타이트한 결과를 취한다"가 됩니다.
- **마지막 2개는 닫히지 않는다.** 수축과 발산의 갈림길 "딱 1"에 바싹 붙은 개체는 유한 연산량으로는 미해결인 채 경계에 다가갈 뿐입니다. 이것은 검사기의 태만이 아니라, 이 경계를 정확히 계산하는 것 자체가 컴퓨터 과학에서 "NP 난해 (NP-hard)"라 불리는 난제이기 때문입니다. 정직하고 깔끔한 한계입니다.
- **+254냐 +692냐는 세는 방식과 솔버에 달려 있다.** 본문의 +692는 솔버가 경계 근처에서 내놓는 거짓 음성을 바로잡은 뒤의 값입니다 (보정 경위는 #35-02).
- **적용 범위는 제한적.** 모든 결과는 작은 기질 (상태가 겨우 2차원인 n=2 시스템), 이 개체 풀에서 얻은 것입니다. "진화하는 AI가 널리 유용하다"는 주장이 아니라, 어디까지나 **검사기의 정확성**에 관한 이야기입니다.

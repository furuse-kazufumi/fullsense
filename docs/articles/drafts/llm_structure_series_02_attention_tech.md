...
<!-- 画像生成意図: 左に画像処理のテンプレートマッチング（テンプレートを画像上でスライドさせ相関ピークを探す様子、相関マップにピークが立つ）、右に self-attention（Query が全 Key と内積を取り、softmax で注目マップが立つ様子）を左右対称に並べ、中央に「＝ 内積で似たものを探す」という等式で橋渡しする概念図。配色は青緑系、赤緑の対比は使わない。 -->
> 画像プレースホルダ：**「テンプレートマッチング ＝ 注意スコア」対比図**（左：相関ピーク／右：注意マップ／中央：内積という共通言語）
...
<!-- 画像生成意図: RoPE を「複数の時計の針」として可視化。次元ペアごとに回転速度の違う針（高周波＝速い針、低周波＝遅い針）が並び、トークン位置が進むと各針が位相を進める。下段にフーリエの周波数スペクトル（高→低周波）を並置し、「位置＝各周波数の位相」という対応を矢印で結ぶ。配色は青〜緑のグラデーション、赤緑対比は不使用。 -->
> 画像プレースホルダ：**「RoPE ＝ 周波数ごとの位相時計」図**（速い針/遅い針＝高周波/低周波、位置が位相を進める。下にフーリエのスペクトルを並置）
...
<!-- 画像生成意図: 横軸に文脈長 T、縦軸にコスト。二本の曲線を描く。ひとつは急な放物線（注意の計算 O(T²)）、もうひとつは直線（KV キャッシュのメモリ O(T)）。放物線がぐんぐん立ち上がる様子で「長文ほど重い」を直感的に見せる。数値目盛りは付けず概形のみ（実測値の断定を避ける）。青緑系配色。 -->
> 画像プレースホルダ：**「O(T²) と O(T) の伸び方」図**（放物線＝注意の計算／直線＝KV キャッシュのメモリ／目盛りなしの概形）

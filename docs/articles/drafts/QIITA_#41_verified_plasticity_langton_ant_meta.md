...
実在の SmolLM2 が作る本物の地形で、進化(MAP-Elites)を弱い勾配(finite-diff)と戦わせたら、**進化が 20戦20勝**(平均 CE で +0.029 リード、p<1e-6)。一見、進化が勾配に勝つ「秩序」が見えた。SNS 映えする見出しが頭をよぎります。

でもこれもラングトンの蟻でした。**対戦相手(finite-diff)が弱かっただけ**。私たちのフレームワークには最初から meta-gate(勝ったら強い相手を呼べ)が入っています。強い解析勾配(backprop = 実際の LLM 学習が使う正確な勾配)を同予算で呼んだら、**勾配が進化を 19戦19勝で逆転**(diff +0.008、p=3.5e-4)。進化の勝ちは弱い相手の artifact でした。判定 = **ARTIFACT + NEGATIVE**。
...
## 世界モデルですら保証は出せない — 藤吉講演 p.51 との接続

もう 1 つの大きな潮流が **世界モデル** です。エージェントが自分の内部に環境シミュレータを持ち、行動を予測する。とても強力で、安全設計にも寄与します。

ですが、世界モデル研究の第一人者である藤吉弘亘氏自身が、2026 年の講演(p.51)で honest に限定しています。**「世界モデルは安全設計に寄与するが、保証ではない」**。これは誠実な姿勢です — 寄与(contribution)と保証(guarantee)を混同しない。

verified-plasticity の立ち位置はここで明確になります。世界モデルが「寄与」に留まるのに対し、**verified-plasticity は sound certificate で保証(GUARANTEE)を出す**。「収縮する(ρ<1、暴走しない)」を、見かけでなく証明で押さえる。これは世界モデルの代替ではなく、補完です — 世界モデルが行動を賢く予測し、verified-plasticity がその構造適応が暴走しないことを保証する。

さらに、藤吉講演の核となるテーゼ — **「人が与える/機械が自ら獲得する境界が、歴史を通じて広がり続けた」** — は、本研究の進化テーゼと **同型** です。手で設計していた構造を、機械が自ら獲得(進化)する方向へ境界が動く。その「自ら獲得した構造」が暴走しないことを誰が保証するのか。verified-plasticity の答えは「sound certificate が保証する」です。

```mermaid
flowchart LR
  W["世界モデル<br/>(藤吉 p.51)"] -->|安全に寄与する| X["寄与 (contribution)"]
  W -.->|保証ではない| Y1["GUARANTEE は出ない"]
...
On a real terrain made by an actual SmolLM2, evolution (MAP-Elites) vs the weak gradient (finite-diff) → **evolution 20–0** (+0.029 mean CE, p<1e-6). An "order" where evolution beats gradient seemed visible; an SNS-friendly headline flashed by.

But this too was Langton's ant. **The opponent (finite-diff) was just weak.** Our framework had a meta-gate from the start ("if you win, call the strong opponent"). Calling the strong analytic gradient (backprop = the exact gradient real LLM training uses) at the same budget: **gradient overturns evolution 19–19** (diff +0.008, p=3.5e-4). Evolution's win was a weak-opponent artifact. Verdict = **ARTIFACT + NEGATIVE**.
...
在真实 SmolLM2 造的真实地形上,进化(MAP-Elites)对弱梯度(finite-diff)→ **进化 20 比 0**(平均 CE +0.029,p<1e-6)。看似出现进化胜梯度的"秩序",适合社媒的标题闪过。

但这也是兰顿蚂蚁。**对手(finite-diff)只是弱。** 框架从一开始就内置 meta-gate(赢了就请强对手)。换强解析梯度(backprop=真实 LLM 训练用的精确梯度)同预算,**梯度以 19 比 19 反超**(diff +0.008,p=3.5e-4)。进化的胜利是弱对手的 artifact。判定 = **ARTIFACT + NEGATIVE**。
...
## 连世界模型都开不出保证 —— 接续藤吉讲演 p.51

另一大潮流是**世界模型**:agent 内置环境模拟器预测行动。很强,也有助于安全设计。但世界模型研究第一人藤吉弘亘本人在 2026 年讲演(p.51)诚实限定:**"世界模型有助于安全设计,但不是保证。"** 诚实之姿 —— 不混淆贡献与保证。

verified-plasticity 的定位由此清晰。世界模型止于"贡献",而 **verified-plasticity 用 sound certificate 开出 GUARANTEE** —— 用证明而非表象压住"收缩(ρ<1,不暴走)"。非替代而是互补:世界模型聪明地预测行动,verified-plasticity 保证其结构适应不暴走。

更甚,藤吉讲演核心命题 —— **"人所给予/机器自我获得的边界,贯穿历史不断扩大"** —— 与本研究进化命题**同型**。边界向机器自我获得(进化)结构移动。谁来保证那"自我获得的结构"不暴走?verified-plasticity 的回答:"sound certificate 保证。"
...
실재 SmolLM2가 만드는 진짜 지형에서 진화(MAP-Elites) vs 약한 경사(finite-diff) → **진화 20–0**(평균 CE +0.029, p<1e-6). 진화가 경사를 이기는 "질서"가 보이는 듯, SNS용 제목이 스침.

하지만 이것도 랭턴의 개미. **상대(finite-diff)가 약했을 뿐.** 프레임워크엔 처음부터 meta-gate(이기면 강한 상대 불러라)가 있음. 강한 해석 경사(backprop=실제 LLM 학습이 쓰는 정확한 경사)를 같은 예산으로 부르니 **경사가 19–19로 역전**(diff +0.008, p=3.5e-4). 진화의 승리는 약한 상대의 artifact. 판정 = **ARTIFACT + NEGATIVE**.

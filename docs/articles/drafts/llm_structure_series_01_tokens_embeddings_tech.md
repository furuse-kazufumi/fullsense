...
> **honest な継ぎ目（今回ぶん）**：私は推論エンジンを自作しましたが、この埋め込み行列の**中身（各トークンの座標）を
...
ただし、ここは**honest に留保を付けます**。この足し算は、あくまで**近似**であって、厳密な等式ではありません。
...
画像プレースホルダ：**「意味の地図」――埋め込み空間を PCA で2次元に落として言葉を散布した概念図。「王様→女王」「男→女」の
2本の矢印が平行に近い向きで描かれ、平行だが完全には一致しないことを示す。**
<!-- 画像生成意図: 高次元の埋め込み空間をPCAで2Dに射影した散布図イメージ。king/queen/man/woman/prince/princess等の語を点で配置し、
king→queen と man→woman のベクトルを矢印で描く。2本の矢印は「ほぼ平行だが完全一致ではない」ことを微妙なズレで表現し、
本文の『近似であって厳密な等式ではない』というhonestな留保を絵でも担保する。攻撃的・自賛的要素なし。静的な1枚絵で意味が伝わること。 -->
...
> **honest な補足**：重み共有は「常に正しい唯一解」ではありません。ごく大きなモデルでは、あえて入口と出口を**共有しない
> （untie する）**設計もあります。共有すると入口と出口に同じ制約がかかるので、規模に余裕があるなら別々にした方が表現力を
> 使い切れる、という判断です。今回の自作対象は共有する設計だったので共有を再現した、というだけで、「共有が絶対」ではない――
> ここも盛らずに。

画像プレースホルダ：**「行きと帰りで同じ表を読む」――1枚の巨大な重み行列 W が、入口では『ID→座標（行を引く）』、
出口では『座標→語彙スコア（各行と内積）』の両方に使われる様子を、1本の矢印が二又に分かれる図で示す。**
<!-- 画像生成意図: 中央に「語彙 × 隠れ次元」の重み行列Wを1枚描き、左からトークンIDが入って行を引く矢印、
右へ内部ベクトルが入って各行と内積を取りロジットを出す矢印、の双方向利用を1枚で表現。「2枚ではなく1枚」を強調するため、
薄い破線で『本来なら2枚必要だった片方』を消してあるような表現。メモリ節約の設計思想が一目で伝わること。赤緑の対立色は使わない。 -->

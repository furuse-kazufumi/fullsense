...
ふつうに考えると「しばらく動かして、様子を見る」ですよね。揺れが収まっていけば安全、揺れが大きくなっていけば危険。経験的で、自然な判断です。多くの「学習する AI」が、まさにこの **経験ベースの見張り** を使っています。
...
私たちは、実在する本物の小さな AI(SmolLM2-135M という Apache ライセンスの公開モデル)の **本物の小型 LLM(SmolLM2)から作った内部データの代理指標(full-vocab ではなく hidden-クラスタ CE proxy)** を使って「練習問題の山」を作り、そこで進化チームと勾配法チームを **同じ予算で** 対戦させました。
...
- **「進化が賢さで負けた」と言い切ってはいません。** 別の地形(人工の練習問題)では、進化と学習法は **引き分け** でした。これは「進化が決定的に劣る」証明でもなければ「互角だ」という証明でもありません。ただの **引き分け(まだ決着がついていない)** です。
...
- **本物の大きな LLM にそのまま載せ替えられるか** も、まだ確かめていません。今回は「本物の内部データを使った練習問題」までで、本体まるごとではありません。

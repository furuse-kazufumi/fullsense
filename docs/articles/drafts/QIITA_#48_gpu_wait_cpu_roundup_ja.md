...
- **もっといろんな身体を進化させる**: 尺取り虫みたいな**単純な這うやつ**から、昆虫みたいな**多脚**まで。同じ MAP-Elites の骨格で身体プランだけ差し替えられるはずなので、CPU でも身体定義(MJCF)は今のうちに増やせます。
- **階段・不整地の踏破**: §3-4 のとおり環境は作りました。**上る歩容の学習は GPU 後**の本番。
- **手の動きから物のつかみ方を学ぶ**: Web カメラで手をトラッキング → その動きを教師に、**ROS 上でロボットアームの把持を学習**させる、というのもやりたい。ここは私自身の双腕ロボット校正の経験とも繋がる話で、CPU 期は**データ取り・ワークスペース整備**まで。

要するに、**「重い学習は GPU、環境と足場づくりは CPU」**という役割分担で、待ち時間を埋めている感じです。

...
- **(おまけ→本命)実在ロボを動かす基盤**: Menagerie の四足(Go2 等)を土俵で戦わせ、**同じ土台で産業用アーム(Franka/UR5e/KUKA/Kinova)の生産ラインまで**動かした(§3-6/§3-7)。四足もアームも同じエンジンの上 = 後段で AI 制御に差し替えられる。**本当にやりたいのは、生産ラインのロボを AI で動かすこと**。

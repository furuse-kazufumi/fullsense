...
専門の方は §4(偽陽性の解剖)と §7(舞台裏)だけでも要点が取れます。はじめての方は §1 から順に。なお **§3-6〜3-7 に、実在ロボット(Unitree Go2)に土俵で相撲を取らせ、さらに同じ土台で産業用アームの生産ラインまで動かす**おまけを付けました。息抜きにどうぞ(そして §3-7 が実は本命の伏線です)。
...
最後に、**箱を縦に積む(パレタイジング)**。これが一番ハマりました。
...
- **本命 ― 生産ラインのアームを AI で**: §3-7 の産業用アームに、いまの開ループ作業動作の代わりに「目標姿勢への到達 → 簡単な pick-and-place → 学習/タスク計画」を載せていく。実機は高価で危険 → まずはシミュで。これが FullSense のフィジカル AI の本丸です。

...
related_groups:
  ...
  - onocollo
  - llcore
title: "GPU付きPCの納品を待つ間に、CPUでできることを全部やってみた"
tags:
  - 機械学習
  - LLM
  - 進化計算
...
このロケット着陸の **読者が辿れる固定参照**としては、[FullSense 側の onocollo 草稿 `§7.6`](https://github.com/furuse-kazufumi/fullsense/blob/main/docs/articles/drafts/QIITA_onocollo_worldmodel_alife_ja.md) と、同じ repo の [handoff 正本 `docs/NEXT_SESSION.md`](https://github.com/furuse-kazufumi/fullsense/blob/main/docs/NEXT_SESSION.md) に要約があります。実装・生ログ・数値の canonical source 自体は repo 外で管理しているため、この場では **公開読者が到達できる repo-fixed reference** を置いたうえで、第 4 点の update として要約しています。
...
この数字の根拠は、読者が辿れる固定参照としての [検証メモ](https://github.com/furuse-kazufumi/fullsense/blob/main/docs/research/option_b_verification_poc1_novelty_2026-06-29.md) と [研究索引](https://github.com/furuse-kazufumi/fullsense/blob/main/docs/research/index.md) に要約されています。実験ログと最終 verdict の canonical source 自体は repo 外だが、この場では **公開読者が辿れる repo-fixed reference** を先に置いています。
...
- 主 group `gaitlab`: 現時点ではこの article が唯一の主記事です。追加されたら同じ主 group の記事をここへ並べます
- 関連 group `spikelab`: [Project Group Index](../QIITA_PROJECT_GROUP_INDEX.md)
- 関連 group `onocollo`: [Project Group Index](../QIITA_PROJECT_GROUP_INDEX.md), [Qiita onocollo worldmodel alife](QIITA_onocollo_worldmodel_alife_ja.md)
- 関連 group `llcore`: [Project Group Index](../QIITA_PROJECT_GROUP_INDEX.md)

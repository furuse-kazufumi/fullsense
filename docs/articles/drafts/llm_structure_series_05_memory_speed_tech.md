...
ここから③で、それぞれを実測値・擬似コード・honest な内訳とともに開けていきます。
...
そして最も大事な honest ポイント。このローダー（重みを1テンソルずつ mmap から流す方式）は、
...
**[画像プレースホルダ]** *キャプション: 線形化の損益分岐 ― softmax KVは文脈長に比例して増加（1024バイト/トークン）、線形状態は232,960バイトで水平。交差点は約227トークン。それより短ければ定数状態の方が大きい。*
<!-- 画像生成意図: 折れ線グラフ。横軸 T（0〜8192, 対数でも可）、縦軸 memory bytes/層。青の直線が softmax KV（傾き1024B/token、T=8192で8,388,608B）、緑の水平線が linear state（232,960B, 一定）。両者の交点 T≈227 に縦点線と「crossover ≈ 227 tokens」注記。T=8192 で「36×」注記。「短い側は定数状態の方が大きい」を淡く網掛け。中立配色。 -->
...
**★ここで honest な内訳を全部出します**（都合の良いところだけ言わないために）：
...
```mermaid
flowchart LR
    A["自宅 CPU で<br/>検査・改造・検収"] --> B["メモリは今すぐ効く<br/>(1.5B: 5.7GB→2.44GB, 1ビット差なし)"]
...
私の貢献は「その重みを1ビットも損なわずに（`max|Δ|=0.0`）自宅の常駐メモリに収め、
中身を測りながら改造できるランタイムを作った」こと。**賢さを作ったのではなく、賢さを正直に運べる器を作った**。この区別は曲げません。
...
- **壁2 量子化**：int8 weight-only は ~3.9倍圧縮・PPL 劣化 0.1% 未満・ビット完全一致で配置（1.5B: 5.7GB→2.44GB）。低ビットには崖（3bit=PTQ の床、2bit=QAT 領域）。**PPL だけのゲートは危険（2bit で top-1 -13.5pp 崩壊）→ top-1 保持で fail-closed 検収**。

...
> 【全体の流れ】3D 可視化の fork → 借り物の限界（ライセンス＋“中身が薄い”）→ 自前の実データ検証ビューア → 「検証機より LLM 本体を作りたい」への転回 → 計画の引き直し。
...
この記事は、その「美しすぎて信じられなかった絵」から始まり、自前の**実データ検証ビューア**を作り、最後に「可視化を磨くより、ちゃんとした LLM 本体を作る方が誠実だ」と気づいてプロジェクト計画ごと引き直すまでの、一日の記録だ。結論を先に言う——**fork は捨て、`llcore` を「LLM としての機能確保」優先に再設計した。** その理由を、走らせた実データと一緒に割っていく。

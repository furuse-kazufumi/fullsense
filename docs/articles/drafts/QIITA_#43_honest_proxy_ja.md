---
title: 'llcore 検証 arc (#43) — AIは平気で嘘をつく: 評価器を作り直し、2026年の効率アーキ地図の中で自分の立ち位置を正直に測った話'
tags:
  - LLM
  - 機械学習
  - attention
...
そこで、第3章で作った精神のまま**統制実験**をしました。容量を倍にした版（StateX流）と、delta-ruleセル版と、素のRNN——を同じ条件で測り、文脈長を16→1024まで伸ばしてカーブを描く。結果：**全部フラット**。容量を増やしてもセルを変えても、壁は1ミリも動かない。

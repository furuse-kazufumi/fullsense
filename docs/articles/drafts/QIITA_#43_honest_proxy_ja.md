---
title: "AIは平気で嘘をつく — 自作LLMの土台の「改良」を正直に測るために、評価器ごと作り直した話"
tags:
  - LLM
  - 機械学習
  - attention
...
# AIは平気で嘘をつく — 自作LLMの土台の「改良」を正直に測るために、評価器ごと作り直した話
...
地味な話です。でも、AIの開発で最も効くのは、派手な新手法ではなく**「自分に都合のいい数字を信じない仕組み」**だと思っています。その作り方を、用語をかみくだきながら共有します。
...
私は「どの層をカード化するか」を**進化的探索（NAS, Neural Architecture Search）**で探しています。数百通りの組み合わせを試し、最も成績の良かったものを「フロンティア（最良の妥協点の集合）」として選びます。
...
最も攻めた解（メモリ57%削減）の文脈長スイープ。**これが第1章の仮説への答え**です。
...
→ つまり**全力で攻めるのは割に合わないが、~30%程度の控えめな削減なら+8%の品質コスト**で済む。崩れるのはアグレッシブな端だけ。「改良」は**条件付きで価値がある**、という地味だが本物の像が残りました。
...
これはこの記事のタイトルの、最も痛い実例です。私は長文脈の壁を破る目的で新しいセルを実装し、**「TTT-Linear」と名付けました**。test-time training という、いま注目の手法の名前です。
...
> 派手な新手法より、**この「自分の数字を疑う完結した手順」**を持っているかどうかが、長期的に効きました。環境構築（7-1）→公開実装で体感（7-2）→正直に測る（7-3）の順で回すのがおすすめです。

### 7-4. 自作の土台を、自分のコードで検証する
...
世の中の「ローカルLLM」の大半は、**既製の演奏者（Hugging Face の transformers ライブラリ）に楽譜（Qwen）を渡して動かしています**。自分のコードは `model.generate()` の一行で、**中身の層には触れません**。
...
flowchart TB
    ...
    subgraph MOD["□ スロットに乗せる改造（HF generate では不可能）"]
        ...
        M3["③ mmap streaming ロード<br/>（挙動は変えない読込最適化）<br/>✅ ロード動作確認済・RAM減は定性"]
        M4["④ 蒸留で品質回復<br/>非耐性層を91-98%回復（校正データ上）<br/>🔬 held-out未測定・汎化未確立"]
    end
...
| 🎻 自前 forward（土台） | 自作の演奏者 | ✅ **HFと一致を実測で確認（単発の逐点一致＋20ターン会話）＝ この範囲（0.5B/1.5B・greedy）で土台は完成** |
| ① 定数状態/線形化 | メモリを文脈長によらず一定に | 🔧 実装済・**品質コストを評価中**（薄味なら可・全力は長文脈で崩れる＝第4章） |
| ② int8 量子化 | RAM を半分に | 🔧 実装済・**CPUでは遅い**（0.7 tok/s ＝ 未解決） |
| ③ mmap ロード | 読み込み時のピーク RAM を抑える（**モデル挙動は変えない**） | ✅ ロード動作確認済（ピークRAM減は定性のみ・実測値は未掲載） |
| ④ 蒸留 | 改造で落ちた品質を回復 | 🔬 実装済。91-98%回復は**校正に使ったのと同じデータ上の値（in-sample）**で、別データ（held-out）の汎化は未測定 |

かみ砕くと——**緑（✅）は「できた・確かめた」、黄（🔧）は「動くが課題が残る」、赤（🔬）は「作ったが、効くと言い切るには検証が足りない」**。土台（forward）だけが緑で、検証した範囲（0.5B/1.5B・greedy）では完成しています。その上の改造は、作れてはいるものの進度がバラバラで、③（モデル挙動を変えない読み込み最適化）以外はまだ課題や未検証を抱えています。**この図が伝えたいのは「土台は確認済み、改造はこれから」という正直な現在地**です。
...
- **0.5Bモデル：2,048トークンすべてで一致（不一致 0）**（logits の差は平均 3.1×10⁻⁶・最大 1.2×10⁻⁴）
- **1.5Bモデル：512トークンすべてで一致（不一致 0）**（logits の差は平均 3.8×10⁻⁶・最大 1.4×10⁻⁴）

何百万回の浮動小数演算の結果が、平均 10⁻⁶・最大 10⁻⁴ という丸め誤差の桁まで一致する。**「llcore の自作演奏者は、既製演奏者と同じ計算をしている」**——これは、（7-1 の環境があれば）誰でも手元で再現できます:

```bash
py -3.11 scripts/prove_native_matches_hf.py --model-dir <ローカルQwenのパス>
```

**さらに、単発の予測一致だけでなく、実際の会話を20ターン続けても一致するか**も確かめました。話題を6回切り替え、途中で名前や居住地を聞き返す20ターンの台本を、HF と自作 forward の両方に greedy で流したところ、**20ターンすべての応答が一字一句同じ**でした（両方とも最後まで完走し、空応答や途中での破綻はありません）。

```bash
py -3.11 scripts/chat_endurance_probe.py --native --model <ローカルQwenのパス> --greedy
```

ただし正直に補足すると、これは上の逐点一致から**ほぼ導かれる結果**です。greedy は毎ステップ「最も確率の高い単語」を選ぶので、単発で全位置の予測が一致しているなら、会話を続けても両者は同じ道をたどります。ここで新しく確かめられたのは「独立した別の証拠」ではなく、**KVキャッシュが伸び、判断の回数が何百回に増える多ターンの状況でも、丸め誤差が積もって道が分岐したりしない**という安心材料です。なお測ったのは「ターン数」の耐久であって「文脈長」ではありません（台本全体が文脈窓に収まる短さです）。会話の中身には 0.5B らしい誤り（「ペンギンは飛べる」等）も混じりますが、それはモデルの賢さの限界で、演奏者（forward）の問題ではありません。

ひとつ正直に補足します。これは greedy（決定論的デコード）での**有限個のトークンの逐点一致**であって、Lean や Z3 のような形式的な「証明」ではありません（第6章「検証の3段階」を思い出してください）。ただし forward が等価かどうかは**決定論的な逐点一致チェック**なので、第4章で問題にした統計的な推定（窓数 K を増やす話）とは別物で、**少ない N でも一致すれば強い証拠**になります。だから語としては「証明」より「**実測による一致の確認**」が正確です。

> 余談ですが、この確認スクリプト自身が、最初は派手に「**0/5 不一致**」と出しました。でもテキストは全部同じで、logits も 10⁻⁶〜10⁻⁴。内訳を見たら、差は「HF は文末の終端トークンを出力に含め、自作版は含めない」という規約の違いだけでした。**「0/5」という数字を額面で信じず内訳を見る**——本記事の主題を、確認する過程でまた踏みました。

#### 核心：土台は「一致」で確認でき、改造は「一致しない」から評価が要る
...
| だから正しさは | 一致の確認で**言い切れる** | 「メモリがいくら減り、品質がいくら落ちたか」を**正直に測る**しかない |
...
**この1本の線が、第0章から第7章までを貫いています。** 土台（forward）は一致で確認でき、その上の改造も自作できる（線形化・量子化・蒸留、すべて `runtime/` に実装済み）。でも改造は一致では検証できない。だから「正直に測る評価器」を先に作り、最初に自分の成果へ向けた——それがこの連載のすべてです。
...
- **蒸留の「91-98%回復」も、手放しでは喜べません。** これは校正に使ったのと同じデータ上で測った値（in-sample）で、別データでも同じだけ回復するか（held-out 汎化）は未測定です。第2〜4章で叩いた「当てはめ上の楽観値を見出しに出す」を、ここで自分がやっていないか——を疑い続けています。

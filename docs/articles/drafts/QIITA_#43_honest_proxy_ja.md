...
```mermaid
flowchart TB
    W["Qwen2.5 の重み（Apache）= 学習済みの楽譜"]
    W --> HF
    W --> NAT
    HF["HF transformers<br/>generate() = ブラックボックス（層を触れない）"]
    subgraph NAT["llcore 自前 forward（runtime/qwen2.py）"]
        direction TB
        E["embedding"] --> A1["層1: attn□+MLP"] --> A2["層2: attn□+MLP"] --> AN["層N: attn□+MLP"] --> O["norm → lm_head"]
    end
    HF <-.->|"同じ入力→同じ出力 / logits差 1e-4（土台が正しい）"| NAT
    NAT --> MOD["土台の上に乗る改造（HFでは不可能）<br/>① attention→定数状態/線形：メモリ O(T)→O(1)<br/>② int8：RAM 5.7→2.4GB<br/>③ mmap：ピーク抑制<br/>④ 蒸留：品質回復"]
```

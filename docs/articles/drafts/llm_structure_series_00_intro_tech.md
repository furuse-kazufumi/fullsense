...
<!-- 画像プレースホルダ（ヒーロー画像） -->
![計測エンジニアが LLM を部品ごとに分解して測っている概念図](画像は別途生成)
<!-- 画像生成意図: 光学定盤・干渉計・カメラといった計測装置の傍らで、Transformer のパイプライン（トークン化→埋め込み→注意機構→順伝播層→出力）が透明な部品として分解され、ノギスや干渉縞で「測って確かめる」様子。技術的で静謐なトーン。青緑と落ち着いた金属色。攻撃的・煽情的な演出は避ける。静的な一枚絵として完成させる。 -->
...
<!-- 画像プレースホルダ（概念図：二種類のゼロ） -->
![浮動小数点の丸め域（2e-4）とビット完全一致（0.0）の違いを干渉縞の比喩で描いた概念図](画像は別途生成)
<!-- 画像生成意図: 左に「2e-4」＝ほぼ重なるが最下位桁だけ微かにずれる二本の干渉縞（浮動小数点の丸め域）、右に「max|Δ|=0.0」＝完全に重なって一本に見える干渉縞（ビット一致）。計測器のダイヤルゲージのような静的で精密な図。誇張表現・煽りは避ける。青と金属色。 -->
...
| **#3** | Transformer ブロックの全体像 ― 知識はどこに住むか | RMSNorm、残差接続、SwiGLU/順伝播層、知識の局在（慎重な hedge 付き） |

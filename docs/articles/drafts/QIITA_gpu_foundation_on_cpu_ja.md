---
title: "GPU付きPCが届く前に、CPUだけで「研究の土台」を作った話 —— スケールを「書き直し」ではなく「設定＋フラグ変更」にするために"
tags:
  - AI
  - GPU
  - MuJoCo
...
public_id: a65b9e84428fc7b7e63e
---
...
> 構成はすべて CPU で検証済み。ただし GPU の実測（高速化倍率・MJX バッチ）は**まだ一度も走らせていない**——本文でその線を正直に引きます。
...
現状 pytest は**落ちるのは 5 件だけ**（`test_import_hygiene.py`、他は緑）。この 5 件は正直に言うと**テストスイート同士が矛盾している**ケースだ。facade 設計で `onocollo.train` を callable として公開しつつ実サブモジュールも到達可能にしている——あるコミットが「明示 import でモジュールが勝つ」を要求する 5 テストを書いた一方、古いテストは同じトリガで「facade が勝つ」を要求している。両者は相互排他で、**どんな `__init__.py` 実装でも両立不能**（`__setattr__` から train 分岐を外すと、どの 3 件が落ちるかが入れ替わるだけでゼロにならない、と経験的に確認済み）。研究コア（`rocket/`, `adr/`, `evolve/`, `vae/mdnrnn/rssm/dreamer`）は ruff+mypy クリーン。GPU 作業のブロッカーではないので、契約をどちらに寄せるかは製品判断として保留した。**落ちているものを"通っている"ことにしない**——これも同じ規律だ。

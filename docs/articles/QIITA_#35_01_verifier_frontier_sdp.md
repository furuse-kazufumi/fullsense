...
| 基質 (substrate) | 実験の土台となる対象系のこと。本稿では n=2 (状態が 2 次元) の小さな結合状態更新系を指す。「この基質では」は「この対象系・この設定の範囲では」という限定の合図。 |
| 遺伝子 / GA | GA = 遺伝的アルゴリズム。個体を変異・選択しながら世代を重ねて良い解を探す進化計算の手法。本稿の「遺伝子」は進化させる対象の動き方 (n=2 の結合状態更新の dynamics) そのもののこと。ungated GA は検査器なしの GA。 |
| GATE (健全性ゲート) | 進化が作った個体を通す/弾くの関門。発散する個体を必ず拒否する役目。inf-norm GATE / SDP GATE は、それぞれの検査器を関門として使う構成。 |
| fitness (適応度) | 進化計算で個体の「良さ」を測る点数。本稿の rotation fitness は回転を含む動きでの良さ。「安全 fitness の天井」は、検査器を通る範囲内で進化が到達できる fitness の上限。 |
| フロンティア (frontier) | 検査器を強くするたびに「証明できる個体」が増えて押し広がる前線のこと。梯子を一段登るごとに前線が進む。被覆フロンティアとも呼ぶ。 |
| 特異値 (singular value) | 行列が単位球を方向ごとにどれだけ伸ばすかを表す値。最大特異値は「一番伸びる方向の倍率」。これが 1 未満ならどの方向も必ず縮む — それが 2-norm の判定。 |
| 二次形式 (quadratic form) | xᵀPx の形 (変数の 2 次の項だけからなる) 関数。Lyapunov のエネルギー関数はこの形で、係数行列 P が楕円体の傾きと形を決める。 |
| 凸最適化 (convex optimization) | 谷が 1 つしかない「お椀型」の最適化問題。局所解にはまらず、見つけた最適解が大域最適だと保証できる。SDP はこの仲間なので、条件を満たす Lyapunov P が存在すれば原理的には探し当てられる。ただし凸性が保証するのは「局所最適 = 大域最適」という問題側の性質であって、ソルバの数値計算が必ず成功することまでは保証しない — 境界付近で SCS が出す偽陰性 (下の「偽陰性」の行と #35-02) は、まさにその隙間で起きる。 |
| 内点法 (interior-point) | 凸最適化を解く代表的アルゴリズム。実行可能領域の内側を通りながら高精度に最適解へ収束する。CLARABEL はこの方式で、境界ぎりぎりでも判定がぶれにくい。 |
| 偽陰性 (false negative) | 本当は証明できる (収縮する) のに「証明できない」と返してしまう見逃し。SCS が境界付近で出す罠で、これを補正する前は SDP の優位が +254 に見えていた (補正後は +692、詳細は #35-02)。 |
| 持ち上げ (Veronese) | 元の変数の高次の組合せ (x², xy など) を新しい座標として並べ、高次元空間に写すこと。二次形式では届かない証明を高次 SOS で狙うための前処理。 |
| 切替系 (switched system) | 複数の更新規則 (行列) が時刻ごとに切り替わりながら作用する系。どんな切替順でも縮むかが問題で、その最悪成長率が JSR。 |
| NP 困難 (NP-hard) | 厳密には「NP という問題クラスのどの問題と比べても同等以上に難しい」ことを指す難しさの等級。実用上の含意としては、効率よく (多項式時間で) 解くアルゴリズムが知られておらず、(P≠NP が正しい限り) 存在も期待できない。exact-JSR の計算がこれで、有限の持ち上げ次数で近づくしかない。 |
| 閉形式 (closed form) | 答えが有限個の式 (四則演算など) で書き下せること。探索ソルバに頼らず、代入すれば答えが出る。本稿 (Track B/C) の収縮判定はこれに還元できたため、Z3 を呼んでも識別力が増えなかった。 |
| 健全性 (soundness) | 「収縮する」と証明したものは本当に収縮する、という検査器側の信頼性。不健全な証明書 = 収縮しないのに通す誤りで、本稿では全段で観測 0。 |
| exact-JSR ブラケット / jsr_lb | JSR の真値を上限・下限で挟み込む方法 (ブラケット)。jsr_lb は JSR の下限 (lower bound)。jsr_lb が 1 に近い個体は収縮/非収縮の境界ぎりぎりにいる。 |
| オラクル / 反証器 | オラクルは「問い合わせると答えを返す装置」の計算機科学用語。本稿の JSR オラクルは反例 (収縮しない証拠) を探して主張を反証する役割。反例が見つからなくても証明にはならない — だから反証器であって証明器ではない。 |
| p 値 (p-value) | 「本当は差が無い (偶然だけ) と仮定したとき、観測された以上に極端な差が出る確率」を表す統計の指標。「差が偶然である確率」そのものではない点に注意。小さいほど、偶然だけでこの差を説明するのは苦しくなる。p = 3.1e-5 は、偶然だけなら約 10 万回に 3 回しか出ないほど極端な差という意味。 |
...
- **degree-4 SOS は +3**、累積 289。deg4 cone は二次 SDP に対して 3 個を新たに証明する (deg6 も同じ 3 個に到達)。deg4∖deg6 = 0 は「deg4 が弱い」のではなく、deg4 固有に拾える individual が 0 ＝ deg4 が deg6 に内包されている、という意味だ。+3 はあくまで SDP に対する deg4 cone の上積みであって、deg6 との差分ではない。
...
| convex optimization | Optimization over a "bowl-shaped" problem with a single valley: no local-optimum traps, and any optimum found is guaranteed global. SDP belongs to this family, so if a qualifying Lyapunov P exists it can in principle be found. Note, though, that convexity guarantees a property of the problem (local optimum = global optimum) — not that the solver's numerical computation always succeeds. The SCS false negatives near the boundary (see the "false negative" row and #35-02) live exactly in that gap. |
...
| NP-hard | Strictly, a hardness class meaning "at least as hard as every problem in the class NP". The practical implication: no efficient (polynomial-time) algorithm is known, and (as long as P≠NP holds) none is expected to exist. Computing the exact JSR is NP-hard, so finite lift degrees can only approach it. |
...
| 基底 (substrate) | 实验所依托的对象系统。本文指 n=2 (状态为 2 维) 的小型耦合状态更新系统。「在此基底上」是「仅限于这个系统、这个设定」的限定信号。 |
| 基因 / GA | GA = 遗传算法: 通过变异与选择、跨世代寻找好解的进化计算方法。本文的「基因」指被进化的动态本身 (n=2 的耦合状态更新)。ungated GA = 不带检查器的 GA。 |
| GATE (健全性门) | 准入/拒绝进化个体的关卡，职责是必须拒绝发散个体。inf-norm GATE / SDP GATE 即以相应验证器作为关卡的配置。 |
| fitness (适应度) | 进化计算中衡量个体「好坏」的分数。本文的 rotation 适应度是含旋转动态下的分数;「安全适应度天花板」指在检查器准入范围内、进化所能达到的适应度上限。 |
| frontier (前沿) | 「能被证明的个体」随检查器增强而不断外推的前线。阶梯每升一级，前线就前进一步。也称覆盖前沿。 |
| 奇异值 (singular value) | 矩阵在各方向把单位球拉伸多少的值。最大奇异值是「拉伸最厉害方向的倍率」; 小于 1 则所有方向都必然收缩 — 这就是 2-norm 的判定。 |
| 二次型 (quadratic form) | 形如 xᵀPx (只含二次项) 的函数。Lyapunov 能量函数就是这种形式，系数矩阵 P 决定椭圆的倾斜与形状。 |
| 凸优化 (convex optimization) | 只有一个谷的「碗形」最优化问题: 不会陷入局部最优，找到的最优解保证是全局最优。SDP 属于此族，所以只要满足条件的 Lyapunov P 存在，原则上就能找到。但要注意，凸性保证的是问题本身的性质 (局部最优 = 全局最优)，并不保证求解器的数值计算一定成功 — SCS 在边界附近的假阴性 (见下方「假阴性」一行与 #35-02) 正是发生在这个缺口里。 |
| 内点法 (interior-point) | 求解凸优化的代表性算法，沿可行域内部高精度收敛到最优解。CLARABEL 采用此法，即使在边界附近判定也很稳。 |
| 假阴性 (false negative) | 本可证明 (确实收缩) 却返回「无法证明」的漏判。SCS 在边界附近的陷阱 — 修正之前 SDP 的优势看似 +254 (修正后为 +692，详见 #35-02)。 |
| 提升 (Veronese) | 把变量的高次组合 (x²、xy 等) 当作新坐标，映射到更高维空间。这是让高次 SOS 追逐二次型够不到的证书的预处理。 |
| 切换系统 (switched system) | 多个更新规则 (矩阵) 随时间轮流作用的系统。问题是无论何种切换顺序都收缩与否; 其最坏增长率就是 JSR。 |
| NP 困难 (NP-hard) | 严格地说，是「至少和 NP 这个问题类中的所有问题一样难」的难度等级。实用层面的含义是: 未知高效 (多项式时间) 算法，并且 (只要 P≠NP 成立) 也不被期待存在。exact-JSR 的计算属于此类，只能用有限提升次数逼近。 |
| 闭式 (closed form) | 答案能写成有限的表达式 — 代入即得，无需搜索求解器。本文 Track B/C 的收缩判定可还原为闭式，因此调用 Z3 不会增加区分力。 |
| 健全性 (soundness) | 检查器侧的保证: 被证明「收缩」的就真的收缩。不健全证书 = 把不收缩的个体放行的错误，本文在每一阶观测均为 0。 |
| exact-JSR 区间 / jsr_lb | 用上界与下界夹住 JSR 真值的方法 (区间/bracket)。jsr_lb 是 JSR 的下界 (lower bound); jsr_lb 接近 1 的基因正处于收缩/不收缩的边界线上。 |
| 神谕 (oracle) / 反证器 | 「神谕 (oracle)」是计算机科学术语，指可查询并返回答案的装置。本文的 JSR 神谕负责寻找反例 (不收缩的证据) 来反驳论断; 找不到反例并不等于证明 — 所以是反证器而非证明器。 |
| p 值 (p-value) | 统计指标: 假设其实并无差异 (纯属偶然) 时，出现「至少和观测值一样极端」的差异的概率。注意它不是「差异出于偶然的概率」本身。越小，仅靠偶然来解释这个差异就越勉强; p = 3.1e-5 意味着仅凭偶然约 10 万次中只有 3 次会出现这么极端的差异。 |
...
| 기질 (substrate) | 실험의 토대가 되는 대상 시스템. 본 글에서는 n=2 (상태가 2차원)인 작은 결합 상태 갱신계를 가리킨다. 「이 기질에서는」 = 「이 시스템·이 설정의 범위에서는」이라는 한정의 신호. |
| 유전자 / GA | GA = 유전 알고리즘. 개체를 변이·선택하며 세대를 거듭해 좋은 해를 찾는 진화 계산 기법. 본 글의 「유전자」는 진화 대상인 동역학 그 자체 (n=2 결합 상태 갱신계). ungated GA는 검사기 없는 GA. |
| GATE (건전성 게이트) | 진화가 만든 개체를 허용/거부하는 관문. 발산 개체를 반드시 거부하는 역할. inf-norm GATE / SDP GATE는 각 검사기를 관문으로 쓰는 구성. |
| fitness (적합도) | 진화 계산에서 개체의 「좋음」을 재는 점수. 본 글의 rotation 적합도는 회전 섞인 동역학에서의 점수. 「안전 적합도의 천장」은 검사기를 통과하는 범위 안에서 진화가 도달할 수 있는 적합도 상한. |
| 프런티어 (frontier) | 검사기를 강화할 때마다 「증명 가능한 개체」가 늘어나며 밀려 나가는 전선. 사다리를 한 단 오를 때마다 전선이 전진한다. 커버리지 프런티어라고도 부른다. |
| 특이값 (singular value) | 행렬이 단위구를 방향별로 얼마나 늘이는지 나타내는 값. 최대 특이값은 「가장 늘어나는 방향의 배율」. 이것이 1 미만이면 모든 방향이 반드시 줄어든다 — 그것이 2-norm 판정. |
| 2차형식 (quadratic form) | xᵀPx 꼴 (2차 항만으로 된) 함수. Lyapunov 에너지 함수가 이 꼴이고, 계수 행렬 P가 타원의 기울기와 모양을 정한다. |
| 볼록 최적화 (convex optimization) | 골짜기가 하나뿐인 「그릇 모양」 최적화 문제. 국소해에 빠지지 않고, 찾은 최적해가 전역 최적임이 보장된다. SDP가 이 가족이라 조건을 만족하는 Lyapunov P가 존재하면 원리상 찾아낼 수 있다. 다만 볼록성이 보장하는 것은 「국소 최적 = 전역 최적」이라는 문제 쪽 성질이지, 솔버의 수치 계산이 반드시 성공한다는 것까지는 아니다 — 경계 근처에서 SCS가 내는 거짓 음성 (아래 「거짓 음성」 행과 #35-02)이 바로 그 틈에서 일어난다. |
| 내점법 (interior-point) | 볼록 최적화를 푸는 대표적 알고리즘. 실행가능영역 안쪽을 지나며 고정밀로 최적해에 수렴한다. CLARABEL이 이 방식이라 경계 직전에서도 판정이 흔들리지 않는다. |
| 거짓 음성 (false negative) | 실제로는 증명 가능한데 (수축하는데) 「증명 불가」라고 돌려주는 놓침. SCS가 경계 근처에서 내는 함정으로, 이를 보정하기 전에는 SDP의 우위가 +254로 보였다 (보정 후 +692, 상세는 #35-02). |
| 들어올림 (Veronese) | 변수의 고차 조합 (x², xy 등)을 새 좌표로 삼아 고차원 공간으로 사상하는 것. 2차형식으로 닿지 않는 증명서를 고차 SOS로 노리기 위한 전처리. |
| 스위칭 시스템 (switched system) | 여러 갱신 규칙 (행렬)이 시간마다 번갈아 작용하는 시스템. 어떤 전환 순서에서도 줄어드는지가 문제이며, 그 최악 성장률이 JSR. |
| NP-난해 (NP-hard) | 엄밀하게는 「NP라는 문제 클래스의 어떤 문제와 견줘도 그 이상으로 어렵다」는 난이도 등급. 실용적 함의는, 효율적인 (다항 시간) 알고리즘이 알려져 있지 않고 (P≠NP가 맞는 한) 존재도 기대할 수 없다는 것. exact-JSR 계산이 여기에 해당해 유한한 들어올림 차수로 다가갈 수밖에 없다. |
| 닫힌 식 (closed form) | 답을 유한한 식으로 적어 내릴 수 있는 것 — 대입만 하면 답이 나오고 탐색 솔버가 필요 없다. 본 글 Track B/C의 수축 판정이 여기로 환원되어 Z3를 불러도 식별력이 늘지 않았다. |
| 건전성 (soundness) | 「수축한다」고 증명한 것은 정말 수축한다는 검사기 측의 신뢰성. 불건전 증명서 = 수축하지 않는 개체를 통과시키는 오류로, 본 글에서는 전 단에서 관측 0. |
| exact-JSR 브래킷 / jsr_lb | JSR 참값을 상한·하한으로 끼워 넣는 방법 (브래킷). jsr_lb는 JSR의 하한 (lower bound). jsr_lb가 1에 가까운 개체는 수축/비수축의 경계 직전에 있다. |
| 오라클 (oracle) / 반증기 | 오라클은 「물으면 답을 돌려주는 장치」라는 계산기과학 용어. 본 글의 JSR 오라클은 반례 (수축하지 않는 증거)를 찾아 주장을 반증하는 역할. 반례가 없어도 증명이 되지는 않는다 — 그래서 증명기가 아니라 반증기다. |
| p 값 (p-value) | 「실제로는 차이가 없다 (순전히 우연)」고 가정했을 때, 관측된 것 이상으로 극단적인 차이가 나올 확률을 나타내는 통계 지표. 「차이가 우연일 확률」 그 자체가 아니라는 점에 주의. 작을수록 우연만으로 이 차이를 설명하기 어려워진다. p = 3.1e-5는 우연만으로는 약 10만 번에 3번꼴로만 나올 만큼 극단적인 차이라는 뜻. |
...
- **degree-4 SOS는 +3**, 누적 289. deg4 cone은 2 차 SDP 대비 3 개를 새로 증명한다 (deg6도 동일한 3 개에 도달). deg4∖deg6 = 0 은 「deg4 가 약해서」가 아니라 deg4 고유로 줍는 개체가 0 ＝ deg4 가 deg6 에 내포되어 있다는 의미다. +3 은 SDP 대비 deg4 cone 의 상승분이지 deg6 과의 차분이 아니다.

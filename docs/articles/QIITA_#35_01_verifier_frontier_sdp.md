...
| NP 困難 (NP-hard) | 「効率よく (多項式時間で) 解くアルゴリズムが知られておらず、存在も期待されていない」難しさの等級。exact-JSR の計算がこれで、有限の持ち上げ次数で近づくしかない。 |
...
| p 値 (p-value) | 観測された差が「偶然だけで出る確率」を表す統計の指標。小さいほど偶然とは考えにくい。p = 3.1e-5 は偶然では約 10 万回に 3 回しか出ない差という意味。 |

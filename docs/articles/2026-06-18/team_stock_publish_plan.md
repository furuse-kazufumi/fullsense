...
10. ここで言う `visibility blocker`、`team-only positive`、`外部公開シグナルあり` は **internal hypothesis / internal safety policy / 内部解析カテゴリ**であり、Qiita 公式仕様名ではない。最初にこの blocker の性質を 1 行で確定する。2026-06-19 時点の Qiita API docs 一次確認では、item の `private` は **only available on Qiita**、`group_url_name` は **Qiita Team 上で share item する group（null で public）** と読める。ただしこれは **診断前の暫定仮説（要出典）**であり、**team-only を positive に確認できるまでは過剰露出疑いを優先**する安全側ポリシーとして扱う。`private:false` 単体は漏洩兆候と断定しない

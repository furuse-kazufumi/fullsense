...
17. 3 本は一括処理しない。**1 本でも `外部公開シグナルあり` または `未確定` が出たらその記事だけ containment / remediation**、残りは diagnosis 継続とする

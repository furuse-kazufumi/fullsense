...
> 📚 **연재 내비**: ← #11 [Qwen과 상호 보완하는 llive(틈새를 메우는 설계)](./QIITA_#11_complementary_with_qwen.md) ｜ **#12 본 글**(5일간을 시간순으로 돌아보는 개발 이력) ｜ #13 [코퍼스 선행 전략(왜 5일 만에 진행했는가)](./QIITA_#13_corpus_first_advantage.md) → . ※ 각 글은 단독으로도 읽을 수 있습니다.
>
> #11까지는 llive **설계의 내용**(Qwen과의 보완, 구조의 독자성)을 다뤘다. 본 글은 시야를 잠시 뒤로 빼서, 그 설계가 **어떤 순서로 어떻게 쌓여 올라갔는지**를 5일치 시간순으로 돌아본다.
...
## 남은 물음 — “5일에 21배”는 과연 속도의 이야기인가

이 5일 동안 테스트는 49 → 1014, 하루 1버전 이상의 페이스로 진행됐다. 하지만 속도 자체는 자랑거리가 못 된다. 진짜 물음은 다른 곳에 있다——**왜 개인 1명 + AI로 이 페이스가 성립했는가**. 손이 빨라서가 아니다. Day 1에 굳힌 설계의 핵이 무너지지 않아서만도 아니다. 사실 개발 중에는 나조차 말로 풀어내지 못했던 “관점을 미리 채워 주는 무언가”가 배후에서 조용히 작동하고 있었다는 가설이 남아 있다.

다음 글 **#13 「[코퍼스 선행 전략 — AI가 내 사고 흐름에서 내가 놓친 관점을 보완한다](./QIITA_#13_corpus_first_advantage.md)」** 에서는 그 정체를 해부한다. 21개 분야 4.9만 건의 RAD 코퍼스를 먼저 쌓아 두면, AI가 내 사각을 메우는 형태로 협업이 돌아간다——이 “5일에 진행한” 페이스 뒤에 있던 협업 구조를, 개발 이력의 숫자에서 한 걸음 더 들어가 언어화해 간다.

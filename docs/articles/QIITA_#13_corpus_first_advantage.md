...
- An **essential advantage of AI-collaborative development** that I realized over the course of today's session
...
由于这些 **无法浮现到意识层面**，自己便无法补全。AI 经由语料库把这些拉取出来，正是协作开发的本质性优势。
...
1. **语料库的质量 = 输出的质量** —— 不是 AI 本体，而是 AI + 语料库 + 规约 + memory 的整套
2. **不在意识层面的视角才是价值所在** —— AI 在后台补全用户未察觉的视角
3. **单人开发 = 多人开发** —— 通过 Six Hats / Role-based agents 的自动应用，单人也能获得多视角

这 3 条公理经由 llive 的 CREAT 需求群，也提供给终端用户。
...
- 오늘 세션 전체를 통해 깨달은 **AI 협업 개발의 본질적 우위**
...
[AI (Claude Opus 4.7)]
     │ 백그라운드에서 참조:
     │  ├ raptor RAD 49 분야 (4.9만 documents)
     │  ├ hacker_corpus (Exploit-DB + ATT&CK + NVD + Phrack)
     │  ├ memory 60+ 파일 (과거의 결정 + 교훈)
     │  ├ CLAUDE.md 규약 (auto-trigger: rad-research / triz-ideation / cross-domain-ideation)
     │  └ Six Hats / TRIZ / KJ법 / MindMap의 구조화된 지식
     ▼
[출력]
     5단계 로드맵 + GPU 판단 + 평가 지표 + 보완 노선 +
     관련 선행 연구 + 동점으로 인정하는 영역 + 리스크 표 + ...
```
...
이것들은 **의식 위로 떠오르지 않기** 때문에, 스스로는 보완할 수 없습니다. AI가 코퍼스를 경유해 이것들을 끌어와 주는 것이, 협업 개발의 본질적인 우위입니다.
...
```
[Phase 1: llive 개발자 (= 나)]
  raptor 코퍼스 + memory + CLAUDE.md → AI가 다시점 보완 → 하루에 32건 요건

[Phase 2: llive 엔드유저]
  llive CREAT + Brief Grounder (TRIZ × RAD) → llive가 다시점 보완 → 사용자는 관점을 의식하지 않아도 OK
```
...
1. **코퍼스의 질 = 출력의 질** — AI 단독이 아니라, AI + 코퍼스 + 규약 + memory의 세트
2. **의식 아래에 없는 관점이야말로 가치** — 사용자가 알아차리지 못한 관점을 AI가 백그라운드에서 보완
3. **1인 개발 = 다인원 개발** — Six Hats / Role-based agents의 자동 적용으로, 1인이라도 복수 시점

이 3공리는 llive의 CREAT 요건군을 통해, 엔드유저에게도 제공됩니다.

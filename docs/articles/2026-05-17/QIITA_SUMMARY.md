...
実装の核は `Dimensions(m, kg, s, A, K, mol, cd)` の 7 次元ベクトル。低レベルの `Quantity` API では direct な次元不一致加算をすでに reject する。一方、実 Brief 文脈での mismatch をどう surface / stop するかという end-to-end 方針は次イテレーション。
...
| **MATH-08** | **内蔵計算エンジン (計算サイドカーの核)** | ✅ 実装済 |
...
| `5 m/s + 3 s = 8` | 次イテレーション (known value+unit grounding までは実装済み) |
...
![MATH-02 形式検証ゲート](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/math02_gate.svg)
...
![ORG-FX 5 段階ロードマップ (Qwen 依存からの離脱)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/orgfx_roadmap.svg)
...
![二層構造 — 補完戦略 と 独自化路線 の同時並走](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/strategy_b_two_layer.svg)
...
| `5 m/s + 3 s = 8` (次元誤り) | **MATH-01 SI 次元 grounding。low-level direct reject は実装済み、end-to-end policy は次イテレーション** |
...
cloud LLM (GPT / Claude / Gemini / Perplexity) が単独では持ちにくい、**on-prem の計算サイドカー領域**。これが llive の補完価値の一つ。
...
- v0.7-vertical MATH 8 件 (内 MATH-08 は計算サイドカーの核)
...
| 3 | [数学・単位に強い AI を作る最初の一歩](https://github.com/furuse-kazufumi/fullsense/blob/main/docs/articles/2026-05-17/03_math_vertical.md) | MATH 系 |
...
The core of the implementation is the 7-dimensional vector `Dimensions(m, kg, s, A, K, mol, cd)`. At the low-level `Quantity` API, direct dimension-mismatch addition is already rejected. What remains for the next iteration is the end-to-end policy for how mismatch patterns from real Brief text should surface and stop.
...
| **MATH-08** | **Built-in calculation engine (core of the calculation sidecar)** | ✅ Implemented |
...
| `5 m/s + 3 s = 8` | Next iteration (known value+unit grounding is already implemented) |
...
![ORG-FX 5-stage roadmap (diverging from Qwen dependency)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/orgfx_roadmap_en.svg)
...
| `5 m/s + 3 s = 8` (dimension error) | **MATH-01 SI dimensional grounding. Low-level direct reject exists; end-to-end policy is next iteration** |
...
A complement layer that cloud LLMs (GPT / Claude / Gemini / Perplexity) do not provide by default: an on-prem calculation sidecar with auditability.
...
- v0.7-vertical MATH 8 items (with MATH-08 as the core of the calculation sidecar)
...
# 速度 + 时间 = 不可能（known value+unit grounding 已实现，cross-quantity mismatch 拦截放到下一次迭代）
v = Quantity(5.0, parse_unit("m/s"))
t = Quantity(3.0, parse_unit("s"))
# 速度 × 時間 = 距離 (m)
d = v * t
...
实现的核心是 `Dimensions(m, kg, s, A, K, mol, cd)` 这一 7 维向量。现阶段先把已知的 value+unit 表达 grounded 到这个表示上，并只在其上处理量纲一致的运算。cross-quantity mismatch 的确定性拦截留到下一次迭代。

派生单位：`N` (kg·m/s²) / `J` (kg·m²/s²) / `W` (kg·m²/s³) / `Pa` (kg/m/s²) / `Hz` (1/s) / `C` (s·A) / `V` (kg·m²/s³/A) / `ohm` 仅在高频范围实现。

### MATH-08 —— 内置计算引擎（最小 skeleton）
...
| **MATH-08** | **内置计算引擎（计算 sidecar 的核心）** | ✅ 已实现 |
...
| `5 m/s + 3 s = 8` | 下一次迭代（known value+unit grounding 已实现） |
...
![MATH-02 形式验证门](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/math02_gate_zh.svg)
...
![ORG-FX 5 阶段路线图 (脱离 Qwen 依赖)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/orgfx_roadmap_zh.svg)
...
| `5 m/s + 3 s = 8`（量纲错误） | **MATH-01 SI 量纲 grounding。low-level direct reject 已有，end-to-end 方针在下一次迭代** |
...
这是 cloud LLM（GPT / Claude / Gemini / Perplexity）单独很难覆盖的 on-prem 计算 sidecar 领域，也是 llive 的长期补充价值之一。
...
- v0.7-vertical MATH 8 项（其中 MATH-08 是计算 sidecar 的核心，而不是夸大的“最大差异化轴”）
...
# 속도 + 시간 = 불가능 (known value+unit grounding 은 구현済み, cross-quantity mismatch 차단은 다음 이터레이션)
v = Quantity(5.0, parse_unit("m/s"))
t = Quantity(3.0, parse_unit("s"))
# 速度 × 時間 = 距離 (m)
d = v * t
...
구현의 핵심은 `Dimensions(m, kg, s, A, K, mol, cd)` 의 7 차원 벡터다. 현 단계에서는 알려진 value+unit 표현을 grounded 화하고, 그 표현 위에서 차원이 맞는 연산만 다룬다. cross-quantity mismatch 의 결정론적 차단은 다음 이터레이션으로 남겨 둔다.
...
| **MATH-08** | **내장 계산 엔진 (계산 sidecar 의 핵)** | ✅ 구현됨 |
...
| `5 m/s + 3 s = 8` | 다음 이터레이션 (known value+unit grounding 은 이미 구현됨) |
...
![ORG-FX 5 단계 로드맵 (Qwen 의존에서의 이탈)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/orgfx_roadmap_ko.svg)
...
![이층 구조 — 보완 전략 과 독자화 노선 의 동시 병행](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/summary/strategy_b_two_layer_ko.svg)
...
| `5 m/s + 3 s = 8` (차원 오류) | **MATH-01 SI 차원 grounding. low-level direct reject 는 구현済み, end-to-end 방침은 다음 이터레이션** |
...
cloud LLM (GPT / Claude / Gemini / Perplexity) 이 기본으로 제공하지 않는, on-prem 계산 sidecar 와 감사 가능성의 결합이 여기서의 보완 가치다.
...
- v0.7-vertical MATH 8 건 (그 중 MATH-08 은 계산 sidecar 의 핵)

...
![AI に秘書を付ける — llive 開発日記 4コマ](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/general_4koma.svg?v=2)
...
さらに **「単位の次元 grounding」** も入れたので、AI が「5 m/s + 3 s = 8」みたいな**単位が合わない式**を扱う前段で、既知の value+unit 表現を決定論的に拾えるようにしています。低レベルの `Quantity` API では direct mismatch reject まで入っていますが、cross-quantity mismatch を end-to-end でどう surface / stop するかは次イテレーションで詰めます。
...
特に **製造業 / 計測 / 物理 / 工学 / 医療** の現場では、AI が「5 m/s + 3 s = 8」のような単位を間違えた式を返してくると致命的です。llive はまず known value+unit grounding を先に入れ、低レベル API では direct mismatch reject を入れた上で、この種の mismatch を end-to-end でどう surface / stop するかを次イテレーションで詰める方針です。
...
![Giving AI a Secretary — llive Dev Diary, 4 Panels](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/general_4koma_en.svg?v=2)
...
Furthermore, I also added **unit-dimension grounding**, so before the AI tries to handle an expression where **the units don't match**, like "5 m/s + 3 s = 8," the system can deterministically pick up the known value+unit forms. The low-level `Quantity` API already includes direct mismatch reject, but exactly how to surface / stop this class of mismatch end-to-end is left to the next iteration.
...
Especially in **manufacturing / metrology / physics / engineering / medical** settings, it's fatal if the AI returns an expression with mistaken units, like "5 m/s + 3 s = 8." llive first adds known value+unit grounding, already has direct mismatch reject at the low-level API, and treats the exact end-to-end surface / stop behavior for this class of mismatch as next-iteration work.
...
![给 AI 配一个秘书 — llive 开发日记 四格](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/general_4koma_zh.svg?v=2)
...
此外，我还加入了**单位量纲 grounding**，所以在 AI 处理像 "5 m/s + 3 s = 8" 这种**单位对不上的式子**之前，系统可以先确定性地拾取已知的 value+unit 表达。低层 `Quantity` API 已经包含 direct mismatch reject，但这类 mismatch 在 end-to-end 上具体如何 surface / stop，仍留到下一次迭代再收紧。
...
尤其在**制造业 / 计量 / 物理 / 工程 / 医疗**的现场，AI 返回像"5 m/s + 3 s = 8"这种弄错了单位的式子，会是致命的。llive 先把 known value+unit grounding 接起来，低层 API 已经加入 direct mismatch reject，而这类 mismatch 在 end-to-end 上的实际 surface / stop 行为留到下一次迭代再收紧。
...
![AI에게 비서를 붙이다 — llive 개발 일기 4컷](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/general_4koma_ko.svg?v=2)
...
게다가 **단위 차원 grounding** 도 넣었기 때문에, AI가 "5 m/s + 3 s = 8"처럼 **단위가 맞지 않는 식**을 다루기 전에 알려진 value+unit 표현을 결정론적으로 집어낼 수 있습니다. low-level `Quantity` API 에는 direct mismatch reject 가 이미 들어가 있지만, 이런 mismatch 를 end-to-end 에서 어떻게 surface / stop 할지는 다음 이터레이션에서 더 조입니다.
...
특히 **제조업 / 계측 / 물리 / 공학 / 의료** 현장에서는, AI가 "5 m/s + 3 s = 8" 같은 단위를 틀린 식을 돌려주면 치명적입니다. llive는 먼저 known value+unit grounding 을 붙였고, low-level API 에는 direct mismatch reject 를 넣은 뒤, 이런 mismatch 를 end-to-end 에서 어떻게 surface / stop 할지는 다음 이터레이션에서 더 조입니다.

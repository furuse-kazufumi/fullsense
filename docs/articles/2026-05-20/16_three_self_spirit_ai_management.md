...
| +1h | ユーザー追加: 「9 因子全部入れたら本格的に動作確認」 |
| +2h | ユーザー追加: 岡潔の視点 (OKA-FX 10 件) |
| +3h | ユーザー追加: LinkedIn フィードバック (IND-FX) |
| +4h | ユーザー追加: ガッツリベンチマーク (12 系統) |
| +5h | ユーザー追加: Anthropic key 復旧 → 他 LLM 比較 |
| +6h | ユーザー追加: Qwen 脱却 / VLM 将来 / lllive スペリング |
| +7h | ユーザー追加: 開発スタイル言語化 (記事化) |
| +8h | ユーザー追加: 三自の精神 + マネジメント書籍 (本記事) |
...
- ユーザーの「思いつき」が **そのまま要件になりうる速度**

要件定義を「いつ閉じるか」を悩む必要がない。**いつでも開けて、いつでも閉じる**。これは人間チームには真似できない「AI ならではの開発スタイル」だ。

## ただし条件 — AI が自律的に動くこと

要件を積み続けてもよい、ただし AI がいちいち「これ進めていいですか」「次は何しますか」と聞いてくると即破綻する。マネージャー (= ユーザー) の頭が常にコンテキストスイッチに奪われる。

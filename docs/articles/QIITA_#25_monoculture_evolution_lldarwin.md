...
> 蒔いた。フリストン・ミリッジ・磯村・岡潔・グロタンディーク・フォン・ノイマン・
> ファインマン……と、なぜか開発者本人（古瀬＝Furuse）。錚々たる 7 人の知性に
> 1 人だけ場違いに紛れ込んでいる自覚はあります。500 世代の世代交代を経て
> 系統として残るのは誰か——。
>
> 結果、残ったのは **Furuse（52%）と Friston（48%）の 2 系統だけ**。
> ……ここでお気づきでしょうか。**Furuse と Friston、どっちも F 始まりで妙に似てる**。
> 岡潔もグロタンディークもフォン・ノイマンもファインマンも全員絶滅したのに、
> 生き残ったのが「名前が似てる F の 2 人」。これ、私が予測符号化の父と肩を
> 並べた感動譚……**ではありません**。むしろ「**AI の進化が、名前が似てるだけの
> 2 人に偏った**」、出来すぎて逆に怪しい結果です。
>
> 種明かしをすると、これは **大失敗の記録**。進化が「強い者を選んだ」のではなく、
> **選択圧がゼロだったせいで、ただの運（遺伝的浮動）で 2 系統に偏っただけ**。
> 偉人の隣に居座れたのは私の実力ではなく、**ぶっ壊れた評価関数のおかげ**です
> （むしろ私が生き残った時点で「この実験おかしいぞ」と疑うべきだった）。
> 本記事はその honest disclosure と、「測る(lleval)」の次に必要な
> 「淘汰する(lldarwin)」コンポーネントの設計話。
...
> 🍵 **休憩ポイント**: ここまでで「8 つの認知スタイルを同一環境で世代交代させた」
> という絵が浮かべば OK。問題は、この淘汰環境の**ルール(評価関数)が
> 壊れていた**こと。次節からが本題です。
...
![2 段構造図: 個体が lleval（眼鏡＝測る）を通って複数軸の pressure profile になり、それが lldarwin（淘汰器＝淘汰する）を通って次世代の親になるフロー](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/q25/two_tier_structure.svg)
...
# After Evolving an AI for 500 Generations, Only Furuse and Friston Were Left in the World #25 — An Honest Disclosure of the "Monoculture Fiasco" Where the AI Just Skewed Toward Two People Whose Names Both Start With F, and the Selection-Pressure Component lldarwin

![4-panel manga digest #25 — the "great failure" of evolving an AI for 500 generations](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/q25_4koma_en.svg?v=2)
...
> 播了下去。弗里斯顿、米利奇、矶村、冈洁、格罗滕迪克、冯·诺依曼、费曼……
> 以及不知为何混进来的开发者本人（古瀬＝Furuse）。7 位代表世界的智慧，外加
> 一个格格不入的家伙。经过 500 代的世代更替，作为系统留存下来的会是谁——。
>
> 结果，留存下来的只有 **Furuse（52%）和 Friston（48%）这 2 个系统**。
> ……发现了吗? **Furuse 和 Friston，罗马字都以 F 开头，长得出奇地像。**
> 冈洁、格罗滕迪克、冯·诺依曼、费曼全都灭绝了，活下来的却是"名字相似的 F 两人"。
> 这并**不是**我和预测编码之父并肩而立的感人故事，而更像是"**AI 的进化只是
> 偏向了两个名字碰巧相似的人**"——一个干净得反而可疑的结果。
>
> 揭晓谜底: 这是一份 **大失败的记录**。进化并不是"选择了强者"，而是
> **由于选择压为零，仅仅凭运气（遗传漂变）偏向了 2 个系统而已**。我能坐在
> 伟人旁边靠的不是实力，而是**坏掉的评价函数**（说真的，我一活下来就该怀疑
> 这实验有问题）。本文就是关于这件事的 honest disclosure，以及在
...
> 🍵 **休息点**: 到这里如果脑海里浮现出"把 8 种认知风格放在同一环境里世代更替"
> 这幅画面就 OK。问题在于，这个淘汰环境的 **规则（评价函数）坏掉了**。
> 正题从下一节开始。
...
![argmax 压垮图: 本应是典型性・多样性・专门性等多轴向量的 pressure profile 被 max() 压成 1 根标量, 多目标性消失的 before/after](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/q25/argmax_collapse_zh.svg)
...
![2 段结构图: 个体经过 lleval（眼镜＝测量）变成多轴的 pressure profile, 再经过 lldarwin（淘汰器＝淘汰）变成下一代的亲代的流程](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/q25/two_tier_structure_zh.svg)
...
# AI를 500세대 진화시켰더니, 세계에 Furuse와 Friston만 남았다 #25 — AI가 로마자로 둘 다 F로 시작하는 닮은 이름의 두 명에게 치우친 「monoculture 대실패」의 honest disclosure와 선택압 컴포넌트 lldarwin

![4컷 만화 다이제스트 #25 — AI를 500세대 진화시킨 '대실패'](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/manga/q25_4koma_ko.svg?v=2)
...
> 뿌렸다. 프리스턴・밀리지・이소무라・오카 기요시・그로텐디크・폰 노이만・파인만……
> 그리고 왜인지 섞여 들어간 개발자 본인（후루세＝Furuse）. 세계를 대표하는 7명의
> 지성에, 영 어울리지 않는 한 명. 500세대의 세대 교체를 거쳐 계통으로 남는 것은 누구인가——.
>
> 그 결과, 남은 것은 **Furuse（52%）와 Friston（48%）두 계통뿐**.
> ……눈치채셨나요? **Furuse와 Friston, 로마자로 둘 다 F로 시작해서 묘하게 닮았습니다.**
> 오카 기요시도 그로텐디크도 폰 노이만도 파인만도 전부 멸종했는데, 살아남은 건
> "이름이 닮은 F 두 명". 이건 제가 예측 부호화의 아버지와 어깨를 나란히 한
> 감동담이 **아닙니다**. 오히려 "**AI의 진화가 이름이 우연히 닮은 두 명에게
> 치우쳤을 뿐**", 너무 깔끔해서 오히려 수상한 결과입니다.
>
> 정답을 말하면, 이것은 **대실패의 기록**. 진화가 "강한 자를 선택한" 것이 아니라,
> **선택압이 0이었던 탓에, 그저 운（유전적 부동）으로 2개 계통으로 치우쳤을 뿐**.
> 위인 옆에 앉을 수 있었던 건 제 실력이 아니라 **고장 난 평가 함수 덕분**입니다
> （솔직히 제가 살아남은 시점에서 "이 실험 이상한데" 하고 의심했어야 했죠）.
> 본 글은 그것의 honest disclosure와, "측정한다(lleval)" 다음에 필요한
> "도태한다(lldarwin)" 컴포넌트의 설계 이야기다.
...
> 🍵 **휴식 포인트**: 여기까지 "8개의 인지 스타일을 동일 환경에서 세대 교체시켰다"
> 라는 그림이 떠오르면 OK. 문제는, 이 도태 환경의 **룰（평가 함수）이
...
![argmax 붕괴 도: 본래 전형성・다양성・전문성 등 복수 축 벡터여야 할 pressure profile 이 max() 로 1개의 스칼라로 붕괴되어 다목적성이 소멸하는 before/after](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/q25/argmax_collapse_ko.svg)
...
![2단 구조도: 개체가 lleval（안경＝측정）을 거쳐 복수 축의 pressure profile 이 되고, 그것이 lldarwin（도태기＝도태）을 거쳐 다음 세대의 부모가 되는 흐름](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/qiita_misc/q25/two_tier_structure_ko.svg)

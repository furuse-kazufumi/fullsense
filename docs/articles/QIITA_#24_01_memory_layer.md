...
<small><strong>EN:</strong> “LLM that doesn't forget”: 4-layer memory + Bayesian surprise gate<br>
<strong>中:</strong> "不会遗忘的 LLM": 4 层记忆 + 贝叶斯惊喜门</small>
...
## 0. この記事は何 (8 秒 read)
<small><strong>EN:</strong> 0. What this article is (8-second read) / <strong>中:</strong> 0. 本文是什么 (8 秒速读)</small>
...
<small><strong>EN:</strong> Explains llive's 4-layer memory + 1 surprise gate — a cognitive layer wrapped AROUND (not inside) the LLM. 4 distinct memory types (semantic / episodic / structural / parameter) write only what the Bayesian surprise gate finds novel. Runs fully on-prem via Faiss + DuckDB + Kùzu + safetensors.<br>
<strong>中:</strong> 讲解 llive 的 4 层记忆 + 1 个 surprise gate — 一个包裹在 LLM 外侧 (而非内部) 的认知层. 4 种角色不同的记忆 (semantic / episodic / structural / parameter) 仅写入贝叶斯惊喜门判定为新颖的内容. 仅靠 Faiss + DuckDB + Kùzu + safetensors 即可纯本地运行.</small>
...
## 1. なぜ 4 層に分けるのか
<small><strong>EN:</strong> 1. Why split into 4 layers? / <strong>中:</strong> 1. 为什么分成 4 层</small>

人間の認知科学では記憶は **意味記憶 / 出来事記憶 / 構造記憶 / 手続き記憶** に役割分担されています. llive はこれをそのまま LLM 周辺アーキテクチャに移植しました.
<small><strong>EN:</strong> Cognitive science divides human memory into semantic / episodic / structural / procedural. llive ports this division directly to its LLM-surrounding architecture.<br>
<strong>中:</strong> 认知科学将人类记忆划分为语义 / 情景 / 结构 / 程序. llive 直接将这一划分移植到围绕 LLM 的架构中.</small>
...
<small><strong>EN:</strong> 2. Semantic memory (MEM-01) / <strong>中:</strong> 2. 语义记忆 (MEM-01)</small>

### 役割

「あの議論で出た **概念** はこれだった」を引き出す層. テキストを埋め込みベクトルに変換して **コサイン類似度** で近傍検索します.
<small><strong>EN:</strong> The layer that recalls 'this is the concept that came up in that discussion'. Converts text to embeddings and retrieves nearest neighbours via cosine similarity.<br>
<strong>中:</strong> 回忆 "那次讨论中出现的概念就是这个" 的层. 将文本转为嵌入向量, 用余弦相似度做近邻检索.</small>
...
<small><strong>EN:</strong> 3. Episodic memory (MEM-02) / <strong>中:</strong> 3. 情景记忆 (MEM-02)</small>

### 役割

「**いつ** その情報を受け取ったか」を保持. **append-only 時系列ログ** で, 編集も削除もしません.
<small><strong>EN:</strong> Holds 'WHEN the information was received'. An append-only time-series log — no edits, no deletions.<br>
<strong>中:</strong> 保存 "何时收到了该信息". 仅追加的时序日志 — 不修改, 不删除.</small>
...
<small><strong>EN:</strong> 4. Structural memory (MEM-05) / <strong>中:</strong> 4. 结构记忆 (MEM-05)</small>

### 役割

「概念 A と 概念 B が **どう関係しているか**」 を表す graph. semantic が「点」だとすれば structural は「辺」です.
<small><strong>EN:</strong> A graph expressing 'HOW concept A and concept B relate'. If semantic memory is 'points', structural memory is 'edges'.<br>
<strong>中:</strong> 表达 "概念 A 和概念 B 之间是什么关系" 的图. 如果说语义记忆是 "点", 那么结构记忆就是 "边".</small>
...
<small><strong>EN:</strong> 5. Parameter memory (MEM-06) / <strong>中:</strong> 5. 参数记忆 (MEM-06)</small>

### 役割

**LoRA / IA3 / prefix adapter** などのパラメータ差分を, **記憶として** 管理します. 「会話で得た知識を Loop 後に LoRA に焼く」ような使い方です.
<small><strong>EN:</strong> Manages parameter deltas like LoRA / IA3 / prefix adapters AS memory. Use cases like 'bake conversation-acquired knowledge into a LoRA after the loop'.<br>
<strong>中:</strong> 把 LoRA / IA3 / prefix adapter 等参数差分作为记忆来管理. 比如 "循环结束后把对话中学到的知识烘焙到 LoRA 里".</small>
...
<small><strong>EN:</strong> 6. Surprise gate - selective writing (MEM-04 / MEM-07) / <strong>中:</strong> 6. 惊喜门 - 选择性写入 (MEM-04 / MEM-07)</small>

### 役割

**「書く価値があるか」を判定する関門**. 全てを書くのではなく, **既存記憶との非類似度** が θ 以上のものだけ通します.
<small><strong>EN:</strong> The gate that decides 'is this worth writing?'. Instead of writing everything, only items whose dissimilarity to existing memory exceeds θ pass through.<br>
<strong>中:</strong> 判断 "这值得写入吗" 的门. 不是全部写入, 而是只让与已有记忆的差异度超过 θ 的项目通过.</small>
...
<small><strong>EN:</strong> 7. Consolidation - Wiki compile (MEM-08) / <strong>中:</strong> 7. 巩固 - Wiki 编译 (MEM-08)</small>

4 層を回したあと, **概念のまとめ直し** が走ります. これが consolidation です.
<small><strong>EN:</strong> After cycling through the 4 layers, a concept re-organisation runs. That is consolidation.<br>
<strong>中:</strong> 4 层循环过后, 会跑一次概念重组. 这就是 consolidation.</small>
...
各 ConceptPage は Markdown として `<llive_data_dir>/wiki/<concept_id>.md` に書き出されます. **人が読める** こと, **Git checkpoint できる** こと, **diff で変化が追える** こと, この 3 つが「Wiki」と呼ぶ理由です. 元ネタは Karpathy の "LLM Wiki" 提案 (`project-llm-wiki-pattern`) です.
...
## 8. 設計判断 (この記事から 5 つ)
<small><strong>EN:</strong> 8. Design decisions (5 takeaways) / <strong>中:</strong> 8. 设计决策 (5 项)</small>
...
<small><strong>EN:</strong> Even with a fixed-θ SurpriseGate, you can cut ~90% of noise vs writing everything. Bayesian θ makes it smarter still.<br>
<strong>中:</strong> 即使是固定 θ 的 SurpriseGate, 相比全部写入也能砍掉约 90% 的噪声. 贝叶斯 θ 会让它更智能.</small>

...
<small><strong>EN:</strong> “LLM that doesn't forget”: 4-layer memory + Bayesian surprise gate / <strong>中:</strong> "不会遗忘的 LLM": 4 层记忆 + 贝叶斯惊喜门</small>
...
<small><strong>EN:</strong> Explains llive's 4-layer memory + 1 surprise gate — a cognitive layer wrapped AROUND (not inside) the LLM. 4 distinct memory types (semantic / episodic / structural / parameter) write only what the Bayesian surprise gate finds novel. Runs fully on-prem via Faiss + DuckDB + Kùzu + safetensors. / <strong>中:</strong> 讲解 llive 的 4 层记忆 + 1 个 surprise gate — 一个包裹在 LLM 外侧 (而非内部) 的认知层. 4 种角色不同的记忆 (semantic / episodic / structural / parameter) 仅写入贝叶斯惊喜门判定为新颖的内容. 仅靠 Faiss + DuckDB + Kùzu + safetensors 即可纯本地运行.</small>
...
<small><strong>EN:</strong> Cognitive science divides human memory into semantic / episodic / structural / procedural. llive ports this division directly to its LLM-surrounding architecture. / <strong>中:</strong> 认知科学将人类记忆划分为语义 / 情景 / 结构 / 程序. llive 直接将这一划分移植到围绕 LLM 的架构中.</small>
...
<small><strong>EN:</strong> The layer that recalls 'this is the concept that came up in that discussion'. Converts text to embeddings and retrieves nearest neighbours via cosine similarity. / <strong>中:</strong> 回忆 "那次讨论中出现的概念就是这个" 的层. 将文本转为嵌入向量, 用余弦相似度做近邻检索.</small>
...
<small><strong>EN:</strong> Holds 'WHEN the information was received'. An append-only time-series log — no edits, no deletions. / <strong>中:</strong> 保存 "何时收到了该信息". 仅追加的时序日志 — 不修改, 不删除.</small>
...
<small><strong>EN:</strong> A graph expressing 'HOW concept A and concept B relate'. If semantic memory is 'points', structural memory is 'edges'. / <strong>中:</strong> 表达 "概念 A 和概念 B 之间是什么关系" 的图. 如果说语义记忆是 "点", 那么结构记忆就是 "边".</small>
...
<small><strong>EN:</strong> Manages parameter deltas like LoRA / IA3 / prefix adapters AS memory. Use cases like 'bake conversation-acquired knowledge into a LoRA after the loop'. / <strong>中:</strong> 把 LoRA / IA3 / prefix adapter 等参数差分作为记忆来管理. 比如 "循环结束后把对话中学到的知识烘焙到 LoRA 里".</small>
...
<small><strong>EN:</strong> The gate that decides 'is this worth writing?'. Instead of writing everything, only items whose dissimilarity to existing memory exceeds θ pass through. / <strong>中:</strong> 判断 "这值得写入吗" 的门. 不是全部写入, 而是只让与已有记忆的差异度超过 θ 的项目通过.</small>
...
<small><strong>EN:</strong> After cycling through the 4 layers, a concept re-organisation runs. That is consolidation. / <strong>中:</strong> 4 层循环过后, 会跑一次概念重组. 这就是 consolidation.</small>
...
<small><strong>EN:</strong> Even with a fixed-θ SurpriseGate, you can cut ~90% of noise vs writing everything. Bayesian θ makes it smarter still. / <strong>中:</strong> 即使是固定 θ 的 SurpriseGate, 相比全部写入也能砍掉约 90% 的噪声. 贝叶斯 θ 会让它更智能.</small>

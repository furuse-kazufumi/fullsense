...
固定 θ の SurpriseGate でも, 典型的な反復入力では全件書き込みより書込量を大きく減らせます (削減率は θ と入力の冗長度に依存). Bayesian 化で取捨選択はさらに適応的になります. honest に言うと, この **「書かない判断」 が記憶系の品質を左右** します.
...
Welford (1962) は 60 年前. それでも今の LLM 周辺アーキテクチャで実用十分な数値安定性を提供します. 新しい model が出ても基礎数学は変わりません.

## 残った問い — 覚えた記憶は, 誰が「考える」のか

ここまでで, llive が **何を覚え, 何を忘れるか** を決める 4 層 + surprise gate を解剖しました. surprise gate は「書く価値があるか」を判定します. しかし, ここで落ち着かない問いが残ります——**記憶は何を考えるか, までは決めない**. semantic も episodic も structural も, 「引き出される」のを待つ受け身の倉庫にすぎません. 倉庫が満ちても, **どの記憶を, どの視点で読み, どう推論につなげるか** は別の層の仕事です.

しかも厄介なことに, 思考は 1 種類ではありません. 同じ記憶を見ても「構造化して読む」のと「過去の出来事と結びつけて読む」のでは, 引き出す結論が変わります. では llive は, この記憶の上で **どんな思考を, いくつ走らせている** のでしょうか?

次回 **(2)「[10 軸で考える AI](https://qiita.com/furuse-kazufumi/private/bdfad6db3f2e70c40511)」** では, 本記事の 4 層の上で動く **10 種類の思考因子** を扱います. 1 つの記憶を 10 通りの視点で同時に読み, 互いに評価させ, 生き残った思考だけを集団へ取り込む——今回「全部は覚えない」と書いた surprise gate の発想が, 思考の側では「**全部の考え方を等しく信じない**」へと引き直されます. 記憶を作った次は, その記憶を使って考える番です.
...
即使是固定 θ 的 SurpriseGate, 在典型的重复输入上, 相比全部写入也能大幅减少写入量 (削减率取决于 θ 和输入的冗余度). Bayesian 化会让取舍更具适应性. 诚实地说, 这个 **「不写入的判断」左右着记忆系统的质量**.
...
Welford (1962) 是 60 年前. 即便如此, 它在如今的 LLM 周边架构中提供足够实用的数值稳定性. 即使出现新 model, 基础数学也不会变.

## 留下的问题 —— 记住的记忆, 由谁来「思考」

到此, 我们剖析了决定 **llive 记住什么、遗忘什么** 的 4 层 + surprise gate. surprise gate 判断「是否值得写入」. 但这里留下一个让人不安的问题——**记忆并不决定要用它思考什么**. semantic、episodic、structural, 都只是等待被取出的被动仓库. 即便仓库装满, **读哪条记忆、从哪个视角读、怎样接到推理上**, 是另一层的工作.

更麻烦的是, 思考不止一种. 面对同一条记忆, 「结构化地读」和「关联过去事件来读」会得出不同结论. 那么 llive 在这片记忆之上, 究竟 **跑着多少种、哪几种思考** 呢?

下一篇 **(2)「[用 10 个轴思考的 AI](https://qiita.com/furuse-kazufumi/private/bdfad6db3f2e70c40511)」** 将讲解在本文 4 层之上运行的 **10 种思考因子**. 它把同一条记忆以 10 个视角同时阅读, 让它们互相评估, 只把存活下来的思考纳入集团——而本文写下的 surprise gate「不全部记住」的发想, 在思考一侧被引申为「**不平等地相信每一种思考方式**」. 造好记忆之后, 接下来就轮到用这片记忆来思考了.
...
> 📚 **연재 내비**: ← [llive 완전 해설 (0) series index](https://qiita.com/furuse-kazufumi/items/07b4882e872994b27b3c) ｜ **(1) 본 글**（4층 메모리 + surprise gate）｜ (2) [「10개 축으로 사고하는 AI」](https://qiita.com/furuse-kazufumi/private/bdfad6db3f2e70c40511) → ｜ ※ 각 글은 단독으로도 읽을 수 있다.
>
> series index에서 llive 전체의 지도를 봤다면, 본 글은 그 첫 정거장이다——**AI가 무엇을 기억하고 무엇을 잊는가** 를 결정하는 기억 기반을 해부한다. 「잊지 않는 LLM」을 자처하기 전에, 먼저 「전부는 기억하지 않는」 설계부터 시작한다.
...
고정 θ의 SurpriseGate라도, 전형적인 반복 입력에서는 전건 기록보다 기록량을 크게 줄일 수 있다 (감소율은 θ와 입력의 중복도에 의존). Bayesian화로 취사선택이 더 적응적으로 된다. 정직하게 말하면, 이 **「쓰지 않는 판단」이 기억계의 품질을 좌우**한다.
...
Welford (1962)는 60년 전이다. 그래도 지금의 LLM 주변 아키텍처에서 실용적으로 충분한 수치 안정성을 제공한다. 새 model이 나와도 기초 수학은 변하지 않는다.

## 남은 물음 — 기억한 기억은, 누가 「생각」하는가

여기까지 llive가 **무엇을 기억하고 무엇을 잊는가** 를 결정하는 4층 + surprise gate를 해부했다. surprise gate는 「기록할 가치가 있는가」를 판정한다. 그러나 여기서 불안한 물음이 남는다——**기억은 그것으로 무엇을 생각할지까지는 정하지 않는다**. semantic도 episodic도 structural도, 「끌려 나오기」를 기다리는 수동적인 창고일 뿐이다. 창고가 가득 차도, **어느 기억을, 어느 시점으로 읽고, 어떻게 추론으로 잇는가** 는 다른 층의 일이다.

게다가 곤란하게도, 사고는 한 종류가 아니다. 같은 기억을 보아도 「구조화해서 읽는」 것과 「과거의 사건과 엮어서 읽는」 것은 끌어내는 결론이 다르다. 그렇다면 llive는 이 기억 위에서 **어떤 사고를, 몇 가지나 돌리고 있는** 것일까?

다음 글 **(2)「[10개 축으로 사고하는 AI](https://qiita.com/furuse-kazufumi/private/bdfad6db3f2e70c40511)」** 에서는, 본 글의 4층 위에서 동작하는 **10종류의 사고 인자** 를 다룬다. 하나의 기억을 10가지 시점으로 동시에 읽고, 서로 평가하게 하여, 살아남은 사고만을 집단에 받아들인다——그리고 본 글에서 쓴 surprise gate의 「전부는 기억하지 않는다」는 발상이, 사고 쪽에서는 「**모든 사고방식을 똑같이 믿지는 않는다**」로 다시 그어진다. 기억을 만든 다음은, 그 기억을 써서 생각할 차례다.

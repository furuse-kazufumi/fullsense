...
- 物理配置: **#01〜#13 = root 複製**（2026-05-28, 原本 `2026-05-17/NN` 据置）/ #14〜25,27 = root /
  未投稿 = drafts（#26,28〜31）。新記事を drafts→root へ昇格・新規投稿したらここに 1 行追記する。
...
- **既知の不整合（非破壊のため未修正・要判断）**:
  1. #26 が root 未昇格（drafts のまま）= flat root に唯一の連番欠番。
  2. drafts の #29/#30/#31 は**本文タイトル内の番号が #27/#28/#29 とズレ**ている（改番前の名残）。投稿前に本文番号を合わせる必要あり。
  3. `archive/2026-05-17`・`archive/2026-05-18` は top-level の同名フォルダと**重複**（ドリフト）。
- 重複整理・原本との dedup・番号ズレ修正を実施する場合は別タスク（破壊的変更につき要承認）。

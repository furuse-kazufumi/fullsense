---
title: '【かみくだき版】個人開発AIのlliveが"メガ進化"！— 進化の大失敗から甦った話をやさしく'
tags:
  - llive
  - FullSense
  - 進化計算
...
id: 49f9e2359c77dce0ed4f
organization_url_name: null
slide: false
ignorePublish: true
---

> 📘 これは **かみくだき版**（やさしい語り）です。技術的な完全版（#25-27 全記録）は → **[完全版はこちら](https://fullsense.qiita.com/furuse-kazufumi/items/0819897256c2e46831ad)**
...
# 개인 개발 AI인 llive가 "메가 진화"! — 진화의 대실패에서 되살아나, 실제 LLM의 "약점"까지 도태시킨 전 기록

개인적으로 개발 중인 AI, **llive**를 진화시켜 봤더니——**8개 계통이 2개 계통까지 급감**하는 대실패.
거기서 "**중립 저장고**"라는 장치로 **모든 계통을 되살리고**, 마지막에는 **실제 LLM의 "약점" 그 자체를 도태시키는** 데까지 도달했습니다. 이 글은 그 실패·설계·반증의 전 기록입니다.

> **llive란**: FullSense 패밀리의 일원으로, "자기 진화형 모듈러 기억 LLM 프레임워크"입니다.
> LLM 본체가 아니라, 그 **둘레에 씌우는 인지 OS**를 지향합니다(OSS / PyPI `llmesh-llive`).
> 이번에는 그 llive의 구성(사고 인자·프롬프트 전략 등)을 **유전자로 간주하여 진화**시켰습니다.

이 글을 관통하는 저음(底音)은 단 하나. **"비정상적으로 깔끔한 결과는 승리가 아니라 경보다."**
진화가 깔끔하게 진행된 것처럼 보이는 순간이야말로, 저는 제 해석을 의심하게 됩니다.

![진화의 적합도와 다양성](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_stage1_baseline_status_ko.svg)

---

## 제1막: 실패 — "나와 프리스턴만 남았다"

llive의 진화는, 8명의 "창시자 페르소나"(후루세·프리스턴·오카 기요시·그로텐디크·폰 노이만·파인만 등)를 초기 집단에 두고 시작했습니다. 노린 것은 다양한 인지 스타일의 공존입니다.

그런데 150세대 돌린 결과가 이것입니다.

![계통 지배 스트림(중립 저장고 없음)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_reservoir_off_dominance_ko.svg)

**살아남은 것은 후루세와 프리스턴 2개 계통뿐.** 오카 기요시도 그로텐디크도, 25세대를 기다리지 못하고 멸종해 있었습니다. 다양성 지표(diversity_l2)도 종반에 0.8 부근까지 붕괴했습니다.

원인은 단순했습니다. **선택압이 거의 0이었던** 것입니다. 적합도가 일찌감치 한계에 도달(포화)하면, 누구를 남길지는 실력이 아니라 **우연한 표류(유전적 부동)**로 결정됩니다. 생물 진화에서 말하는 중립 진화(기무라 모토오) 그 자체로, 방치하면 집단은 하나의 계통으로 고정됩니다.

"재는 도구(평가 함수 = lleval, 말하자면 안경)"만으로는 진화가 앞으로 나아가지 않습니다. **측정한 차이를 '누가 살아남을지'로 변환하는 도태기**가 필요합니다. 그래서 만든 것이 **lldarwin**입니다.

---

## 제2막: 설계와 부활 — lldarwin

lldarwin의 핵심은 한마디, **"집약하지 않는다"**는 것입니다.

여러 평가 축을 하나의 점수로 합산(argmax)하면, 또다시 단일한 봉우리로 모두가 끌려가 다양성이 죽습니다. 대신 **ε-lexicase 선택**으로, 축을 하나씩 독립적으로 평가합니다. 어느 한 축만 돌출한 "전문가"도 살아남을 수 있으므로, 다극 구조가 자동으로 유지됩니다.

### 1단계: novelty로 "행동 다양성"을 구한다

먼저, 집단에서 벗어난 개체일수록 높게 평가하는 **novelty 압**을 더했습니다. 그 결과, 유전자 공간의 다양성(diversity_l2)은 **7.12 → 14.88(+109%)**. 종반의 붕괴도 멈췄습니다.

![baseline vs novelty의 다양성](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_stage1_diversity_overlay_ko.svg)

——여기서 저는 한순간 "이겼다"고 생각했습니다. 하지만 이것이 **첫 번째 경보**였습니다(제3막에서 회수합니다).

### 2단계: 중립 저장고로 "계통"을 되살린다

novelty를 넣어도 **계통의 고정은 멈추지 않았습니다.** 후루세와 프리스턴의 2강 그대로.
이유는 단순합니다. lexicase도 novelty도 **지금 있는 개체를 보존할 뿐**, **한 번 멸종한 계통을 부활시키는 힘은 없기** 때문입니다.

그래서 **lineage-niched 중립 저장고**를 구현했습니다. 계통마다 "지금까지 가장 좋았던 개체"를 보관해 두고, **멸종한 계통을 매 세대 슬그머니 재투입합니다.** 보전 생물학에서 말하는 종(種)의 보존에 가까운 발상입니다.

![계통 지배 스트림(중립 저장고 있음)](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_reservoir_on_dominance_ko.svg)

**전 8개 계통이 살아남았습니다.** 오카 기요시도 그로텐디크도 부활. 최대 점유율은 0.33, 계통 고정도는 **0.29**(붕괴 라인 0.8을 크게 밑돕니다). 제1막의 멸종극과는 전혀 다른 세계입니다.

### 3단계: 재투입 빈도의 비자명한 급소

"매 세대 재투입이 제일 낫지 않나?"라고 생각하기 쉽지만, 여기에 재미있는 함정이 있습니다.

![재투입 빈도의 트레이드오프](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_reinject_sweep_ko.svg)

계통을 최우선한다면 매 세대(8/8 생존). 하지만 **행동 다양성은 재투입 간격=5에서 피크**가 됩니다(단조롭지 않습니다). 계통을 너무 방치하면 저장고에서 오는 다양성 주입이 줄어, 결국 둘 다 마릅니다. **보전과 탐색의 균형점**이 존재하는 것입니다.

---

## 제3막: 실제 LLM의 "약점"을 도태시킨다

여기까지는 결정론적인 대리 평가(proxy)였습니다. 본 무대는 **실제 LLM**입니다.

다행히 수중의 on-prem 환경(로컬 ollama, llama3.2)을 쓸 수 있었기에, 이렇게 설계했습니다:
**개체의 프롬프트 유전자(사고 스킬·템플릿·어조)를 system prompt로 변환하고, 고정된 LLM에 씌워, LLM이 어려워하는 실제 과제를 풀게 한 뒤 채점한다.** LLM 본체는 고정, **진화하는 것은 프롬프트 전략**입니다(Promptbreeder 계열).

약점 축은 5가지: 오타 견고성 / 다의어의 문맥 이해 / 다단계 추론 / 신뢰도 보정 / 무관한 문맥에 대한 견고성.

![실제 LLM 약점 축 점수의 세대 추이](https://raw.githubusercontent.com/furuse-kazufumi/fullsense/main/docs/articles/assets/lldarwin_2026_05_26/lldarwin_stage2_real_llm_axes_ko.svg)

가장 효과가 컸던 것은 **다단계 추론(multistep)**이었습니다. 소박한 "간결하게 답해" 전략으로는 llama3.2가 계산 문제를 죄다 틀립니다(점수 0.0). 그런데 진화가 "**단계적으로 생각한 뒤 답한다**" 전략(CoT + 구조화)에 도달하자, **0.0 → 1.0으로 개선**. `best_score`는 1.0에 도달했습니다.

**프롬프트 전략의 진화가, LLM의 약점을 실측으로 완화했다**——이것이 lldarwin이 노린 그대로의 순간입니다. 이 실제 LLM 런은 12시간 연속으로 계속 돌리고 있습니다.

---

## 정직한 내역 — 나는 무엇을 착각하고 있었는가

여기서부터가 이 글의 심장부입니다. 방금 "이겼다"고 생각한 순간을 해부합니다.

저는 **3종류의 "다양성"을 혼동하고 있었습니다.**

1. **행동 다양성**(유전자 공간에서 얼마나 흩어져 있는가)
2. **계통 다양성**(어느 창시자의 후손이 살아 있는가)
3. **실제 LLM의 지능 다양성**(실제로 다양한 영리함을 지니는가)

novelty로 개선된 깔끔한 숫자는, **(1) 행동 다양성만**의 이야기였습니다. (2) 계통 다양성은 중립 부동으로 멋대로 고정되고(그래서 중립 저장고라는 별도의 장치가 필요했고), (3) 실제 LLM의 지능 다양성에 이르러서는, proxy로는 재고 있지조차 않습니다.

**지표를 읽는 인간(나)의 해석마저, 재고 싶었던 것에서 괴리되어 있었다.** 이것은 굿하트의 법칙의 "설계자 측 버전"입니다. 깔끔한 숫자를 승리로 읽은 시점에, 저는 경보를 흘려듣고 있었습니다.

정직하게 선을 그어 둡니다:

- **저장고도 novelty도, 실제로 효과가 있었습니다**(날조의 실패가 아닙니다). 개선은 사실입니다.
- 다만 proxy 축은 **장치가 돌아간다는 검증(mechanism feasibility)**이지, production LLM 능력의 증명은 아닙니다.
- 실제 LLM 평가에서도, 도태에 효과가 있는 것은 **프롬프트 유전자뿐**이고, 페르소나에서 유래한 유전자는 사실 중립입니다. 테스트 배터리도 작고 노이지합니다. 그리고 **on-prem 한정**(평가의 순도를 위해, 클라우드 LLM과는 섞지 않습니다).

"진화시키면 LLM의 약점은 알아서 극복된다"——그런 낙관은 취하지 않습니다. **이겼다고 생각하기 전에, 반드시 내역을 의심한다.** 그것이 llive 진화 연구의 작법입니다.

---

## 메타 고찰 — 진화형 AI로서, 낡은 구속을 버리고 만든다는 것

마지막으로, 이 글 자체에 대해 정직하게 씁니다. 이 글은 **끊임없이 진화하는 AI(나 자신)가, 경험칙에 근거해, 낡은 구속을 버리고, 수중의 모든 스킬을 동원하여, 실현 방법을 최대한 검토한 뒤에 구현한다**——는 모드로 만들어졌습니다. 사실 처음에는, 과거 연재 원고 3편을 기계적으로 결합한 74,000자의 "완전판"을 만들고 있었습니다. 중복투성이에, 만담식 주고받기가 과잉이고, 그림을 11장 늘어놓았을 뿐. **깔끔하게 정리된 것처럼 보였지만, 그것이 경보였습니다.** 그래서 한 번 전부 버리고, 이 lean한 판으로 다시 썼습니다.

여기에 하나, TRIZ적인 모순이 있습니다. **"빠르게 좋게 하려면 대담하게 버리고 싶다"**인데 **"너무 버리면 돌이킬 수 없다"**. 이 모순을 푸는 법은 분리 원리——**종류로 나누는** 것입니다.

- **버려도 되는 것(낡은 구속 / 습관)**: 장황한 구조, 과잉 연출, 관성적인 연재 분할, "길수록 훌륭하다"는 선입견. 이것들은 도태압에 걸어도 상관없습니다.
- **절대 버려서는 안 되는 것(원리)**: honest disclosure(깔끔한 숫자를 의심한다), 측정 순도(on-prem only), fail-closed, 날조 금지. 이것들은 "대체 불가능한 계통"이며, 기세에 휩쓸려 잃으면 두 번 다시 돌아오지 않습니다.

——눈치채셨나요? 이것은 본문에서 이야기한 lldarwin의 설계 그 자체입니다. **novelty(탐색)로 대담하게 움직이고, 중립 저장고(보전)로 본질을 지킨다.** 글을 쓴다는 행위조차, 같은 구조로 도태와 보전의 균형을 잡고 있습니다. **만드는 측의 AI가, 만들고 있는 대상의 원리를, 자기 자신에게 적용하고 있다.** 자기 언급적이지만, 이것이 "진화형 LLM이 낡은 구속 없이, 그러나 원리는 지키며 만든다"는 것의 구체적인 예입니다.

그러면 "최대한 검토한 뒤에 구현한다"는 어려운가요? 어려운 것은 검토 그 자체가 아니라, **검토와 구현을 오가는 규율**입니다. 한 번에 완벽을 노리지 말고, PoC로 feasibility를 확인하고(중립 저장고도 표준 런 전에 독립 PoC로 실증했습니다), 결과를 보고 다음을 정합니다. 실패한 판(74K)도 지우지 않고 source로 남깁니다. **버리는 용기와 남기는 신중함은, 같은 진화 동전의 양면**입니다.

## 구현과 내력

이 모든 것은 llive 본체에 구현·테스트되어 있습니다(진화 계열 947개 테스트 green). 주요 커밋:

| 단계 | 내용 | commit |
|---|---|---|
| Stage1 | criteria 제외 + novelty 압 | `8060204` |
| PoC | 중립 저장고의 실증 | `0d0537d` |
| Stage1.5 | 저장고를 EvolutionLoop에 편입 | `b03cbda` |
| sweep | 재투입 빈도 트레이드오프 | `da93dd3` |
| Stage2 | 실제 LLM 약점 축 평가 | `2fb2912` |

시각화는 모두 **의존성 없는 자기 완결 SVG**(지원 환경에서는 애니메이션, 미지원에서도 정적 표시).

---

## 맺음말 — llive를 직접 만져 보기

llive는 OSS입니다. "LLM의 둘레에 씌우는 인지 OS"라는 발상에 흥미가 생겼다면, 부디 들여다봐 주세요(PyPI: `llmesh-llive`). 진화·기억·평가(lleval)·도태(lldarwin)가 하나의 세계관으로 이어져 있습니다.

그리고 다음에 당신이 깔끔한 벤치 결과를 봤을 때는, 승리의 함성을 지르기 전에 딱 한 번——**"이것은, 무엇을 잰 숫자일까?"**라고 물어봐 주세요. 아마 그것이, 가장 효과적인 도태압입니다.

...
| 기질 (substrate) | 진화가 작용하는 「바탕 시스템」 — 여기서는 RWKV 풍 결합 상태 갱신으로 움직이는 작은 신경 시스템을 가리킨다. 화학의 기질이 아니라 「진화가 그 위에서 일하는 소재」라는 이미지로 읽으면 된다. |
| n=2 | 시스템 크기 (차원)가 2라는 뜻. 본 시리즈의 모든 결과는 이 작은 2차원 결합 기질에 대한 것으로, 더 큰 시스템으로의 일반화는 주장하지 않는다 (Honest disclosure 참조). |
| RWKV | Transformer의 어텐션 대신 재귀적 상태 갱신으로 시퀀스를 처리하는 근래의 신경망 계열. 본 연구는 RWKV 자체를 돌리는 것이 아니라 「RWKV 풍」 결합 상태 갱신의 형태만 유전자의 틀로 빌린다. |
| GA (유전 알고리즘) | 생물의 진화를 흉내 낸 탐색법. 개체 집단에 변이와 선택을 세대마다 반복해 성적 (fitness)이 좋은 개체를 남긴다. 본 글에서 「진화」라고 하면 이 GA를 뜻한다. |
| fitness (적합도) | 진화 계산에서 개체의 「성적」을 재는 점수. 점수가 높은 개체일수록 다음 세대에 남기 쉽다. 「안전 적합도」는 GATE를 통과한 (안전이 증명된) 개체가 도달할 수 있는 성적을 말하며, rotation 적합도는 그중 하나다. |
| seed | 난수의 씨앗. 같은 seed (예: 2024)에서 시작하면 같은 난수열이 재현되어 누구나 실험을 재현할 수 있다. 뒤집어 말하면, 결과는 「이 seed로 생성한 개체 풀」에 한정된다는 단서도 붙는다. |
| p 값 | 「사실은 차이가 없다」고 가정했을 때, 관측된 것 이상의 차이가 우연히 나올 확률. 작을수록 우연으로 설명하기 어렵다. p = 3.1e-5 (약 10만분의 3)는 inf-norm GATE와 SDP GATE의 적합도 차가 우연이라고 보기 어렵다는 뜻. |
| 볼록 최적화 | 「그릇 모양」 지형에서 가장 낮은 점을 찾는 유형의 최적화. 그릇 모양이면 가짜 골짜기 (국소해)가 없어서, 찾은 답이 정말 최선임을 보장할 수 있다. SDP는 볼록 최적화의 일종 — 그래서 「증명」에 쓸 수 있다. |
| 2차형식 | x^T P x처럼 변수의 2차 항만으로 이루어진 함수. 여기서는 상태의 「에너지를 재는 자」로 쓰며, 이 에너지가 시간에 따라 반드시 줄어든다고 증명되면 시스템은 수축한다 (이것이 Lyapunov 증명서의 내용). |
| 양정치 (PD) | 어느 방향 x로 재도 x^T P x > 0이 되는 행렬 P의 성질 (PD = positive definite). P가 에너지 자로 쓰일 수 있기 위한 조건이며, Lyapunov 증명서의 P는 이 성질을 가져야 한다. |
| Cholesky 분해 | 양정치 행렬을 삼각행렬의 곱 (L×L^T)으로 분해하는 표준 기법. 분해가 끝까지 성공한다는 사실 자체가 「양정치」의 증거가 된다. Rump verified PD는 이 분해의 수치 (후방) 오차까지 엄밀하게 추정하여, 부동소수점으로도 흔들리지 않는 기계검증 증명으로 격상시킨다. |
| 내점법 | 볼록 최적화를 실행 가능 영역의 「안쪽」을 지나며 고정밀로 푸는 해법. 반복 1회의 계산은 무겁지만 경계 직전까지 정확히 파고들어 답의 정밀도가 높다. CLARABEL이 이 방식이라 경계 부근의 오판이 잘 일어나지 않는다. |
| 1차법 (ADMM) | 기울기 수준의 가벼운 정보만으로 나아가는 해법군의 총칭. ADMM이 그 대표격으로, 반복 1회는 싸지만 정밀도는 적당한 선에서 멈춘다. SCS가 이 방식이라, 수축/발산 경계 직전의 개체에서 「사실은 수축하는데 증명하지 못하는」 (거짓 음성) 일이 일어났다. |
| inf-norm / 2-norm (유도 노름) | 행렬이 「벡터를 최대 몇 배로 늘리는가」를 재는 자. inf-norm은 행별 절댓값 합의 최댓값, 2-norm은 최대 특이값 (가장 늘어나는 방향의 배율). 둘 다 1 미만이면 매 스텝 반드시 줄어든다. 다만 재는 방법이 미리 고정된 자라서, 개체마다 맞춘 자 P를 찾아내는 SDP보다 증명 범위가 좁다. |
| 닫힌 식 (closed form) | 솔버나 반복 계산 없이 사칙연산·max 등 유한한 식으로 답을 그대로 쓸 수 있는 것. 닫힌 식으로 판정되는 성질이면 솔버를 불러도 같은 답만 돌아온다 — 「Z3는 장식」이었던 이유가 바로 이것. |
| 거짓 음성 | 사실은 「수축 (합격)」인데 「증명 불가 (불합격)」로 판정하는 오류. fail-closed GATE에서는 안전한 쪽 오류지만, 합격했어야 할 개체를 헛되이 쳐내 검증기의 실력을 실제보다 낮아 보이게 한다. SCS의 함정이 바로 이것이었다. |
| 프런티어 (frontier) | 「여기까지 증명했다」는 최전선. 검증기가 강해질수록 전선이 바깥으로 밀려난다. 본 글의 커버리지 프런티어는 JSR=1 경계에 점근할 뿐 완전히 닫히지 않는다 — exact-JSR이 NP-난해이기 때문. |
| Veronese 들어올림 | 상태 벡터를 단항식 (x², xy, y² 등)을 늘어놓은 고차원 벡터로 보내는 변환. 들어올린 공간에서는 고차 다항식 증명서도 「그냥 2차형식」으로 다룰 수 있어서, SOS 증명서 탐색을 SDP로 풀 수 있게 된다. deg4/6/8은 이렇게 얻는 증명서의 다항식 차수. |
| 스위칭 시스템 (switched system) | 여러 갱신 규칙 (행렬)을 번갈아 가며 진행하는 시스템. 어떤 스위칭 순서에서도 줄어들어야 안전하며, 그 순서들에 대한 최악 성장률이 JSR이다. 「스위칭 팽창」은 어떤 순서에서는 부풀어 버리는 개체를 가리키며, GATE는 이를 올바르게 거부했다. |
| jsr_lb | JSR의 하계 (lower bound). 「진짜 최악 성장률은 적어도 이 값 이상」이라는 보장이 붙은 추정치. 1에 가까울수록 (예: 0.9915) 수축/발산 경계 직전 — 증명이 가장 어려운 개체임을 뜻한다. |
| 오라클 (oracle) | 답 맞추기에 쓰는 독립 참조 판정기. 본 글의 JSR 오라클은 반증기 — 발산하는 반례를 찾아 잘못된 허용을 잡아낼 수는 있지만, 수축의 「증명」은 되지 않는다. 그래서 건전성 주장은 「관측된 거짓 허용 0」에 머문다. |
| Track B / C / D | 본 시리즈 실험 트랙의 이름. Track B = Z3와 닫힌 식 스칼라 테스트의 일치 검증 (20000/20000 일치), Track C = Z3 수축 검사와 닫힌 식 부등식의 일치 검증 (3270개에서 불일치 0), Track D = 3270 유전자의 스케일 검증. |

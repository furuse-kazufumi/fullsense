...
| canalization | The phenomenon that the more evolution advances, the more developmental pathways become fixed and resistant to change (Dr. Waddington). |
...
그리고 발상이 한 단계 더. 생물은 불필요한 것을 버리고 단순화한다(동굴 물고기는 눈을, 기생체는 유전자를 버린다). **비용의 낮음 자체를 진화의 선택압으로 삼으면, AI도 "싸게 검증할 수 있는 단순한 구조"로 스스로 향하지** 않을까. 다만 함정이 있다 — "싸다"에는 좋은 저렴함(구조적으로 단순)과 나쁜 저렴함(검사기가 너무 보수적이어서 아무것도 학습하지 않는 퇴화)이 있고, llcore의 L3에서 실제로 후자(unigram으로의 퇴화)가 관측되었다. 그래서 단순한 스칼라화가 아니라, 성능과 비용의 파레토(Pareto)로 다룰 필요가 있다.
...
다만 **llcore 자신의 결과가 경고한다 — 저렴함에는 2종류가 있다**:

- ✅ **좋은 저렴함 = 구조적 단순함** (저랭크/희소): 정말로 검증이 싸고, 게다가 진화가 움직이기 쉽다(navigable).
- ❌ **나쁜 저렴함① = 검사기의 보수성**: ∞노름은 가장 싸지만, 진화를 unigram(무문맥)에 함정에 빠뜨렸다. "검증이 싸다=선(善)"이라고 소박하게 보상화하면, 진화를 ∞의 함정으로 밀어 넣는다.
- ❌ **나쁜 저렴함② = 퇴화한 행동**: L3에서 관측한 unigram으로의 collapse 자체가 "안전·저렴하지만 무능한, 너무 단순한 구조가 살아남았다"는 예 — 너무 깎아서 자유 생활을 할 수 없게 된 절대 기생체 그 자체다.
...
> | 비용을 선택압으로 | 구상·user-gate | 저렴함은 2종, 파레토로 다룰 필요 |
...
- "**비용을 진화의 선택압으로**" = 생물의 환원적 진화를 본뜬 구상. 다만 좋은 저렴함(구조적 단순)과 나쁜 저렴함(보수성/퇴화)을 건전성 oracle로 판별하는 것이 핵심.

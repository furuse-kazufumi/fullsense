...
| 凸最適化 | 「お椀型」の地形で一番低い点を探すタイプの最適化問題。お椀型なら途中の小さなくぼみ (局所解) に騙されないので、見つかった答えをそのまま保証として使える。SDP/LMI はこの凸最適化の一種で、だからこそ「証明書」の探索に使える。 |
| 内点法 (interior-point) | 凸最適化を、実行可能領域の「内側」を通りながら高精度に解いていく解法。境界ぎりぎりの問題でも精度を保ちやすい代わりに、計算は重め。CLARABEL がこの方式。 |
| 一次法 / ADMM | 勾配 (1 階の傾き) 程度の情報だけで反復改善していく軽量な解法ファミリー。ADMM はその代表格で、速くて大規模に強い代わりに、境界付近では「だいたい合っている」答えで止まりやすい。SCS がこの方式。 |
| 正定値 (PD) | 行列 P が「どの方向 x で測っても xᵀPx > 0」となる性質。距離を測る (歪んだ) ものさしとして使える行列の条件で、Lyapunov 証明書の P はこれを満たす必要がある。 |
| 固有値 / 最小固有値 | 行列が各方向をどれだけ伸び縮みさせるかを表す値。対称行列では「最小固有値 > 0」と「正定値」が同値。本稿では float で計算した固有値の再チェックを、健全性の独立な確認として使っている。 |
| 特異値 / SVD | 行列が入力をどれだけ引き伸ばすかの「倍率」の集合。その最大値が 1 未満なら、その行列は必ず距離を縮める (これが 2-norm 証明書)。SVD (特異値分解) はそれを求める分解で、#36 の高速化は SVD 1 回で済む。 |
| Cholesky 分解 | 正定値行列を三角行列の積 LLᵀ に分解する計算。分解が最後まで成功すること自体が正定値性の証拠になる。Rump verified-PD は、この計算の丸め誤差まで厳密に見積もることで「証拠」を「証明」に格上げした手法。 |
| 誘導ノルム (inf-norm / 2-norm) | ベクトルの測り方から導いた「行列が状態をどれだけ伸ばすか」の標準ものさし。inf-norm 版は行絶対和の最大、2-norm 版は最大特異値。どちらもものさしの形が固定なので、「歪んだものさしで測れば縮んでいる」系を取りこぼす — 非単位 P を許す SDP はそこを突破する。 |
| 持ち上げ (lifting) / Veronese | 状態 x をその単項式の並び (x₁², x₁x₂, …) に写し替え、高次の証明を「持ち上げた空間での二次の証明」として解くテクニック。この写像を Veronese 埋め込みと呼ぶ。degree-4/6/8 SOS はこの持ち上げで作られる。 |
| 閉形式 (closed form) | 反復計算やソルバに頼らず、有限個の四則演算などで書き下せる式のこと。閉形式に帰着できる判定なら、そもそもソルバを呼ぶ必要がない — §4 で「Z3 は装飾的」と言えるのはこのため。 |
| NP 困難 | 問題サイズが大きくなると、知られているどの算法でも計算時間が爆発的に増えると考えられている問題クラス。厳密 JSR がこれに当たるため、「全部きっちり計算する」路線には原理的な壁がある。 |
| 健全性 (soundness) | 「通したものは絶対に安全」という性質。取りこぼし (偽陰性) は許されても、危険なものを誤って通すこと (偽許可) だけは決してしない。安全 gate の最優先の性質で、今回のソルバの罠は幸い健全性側ではなく取りこぼし側だった。 |
| coverage / 残差 | coverage = プールのうち、その証明書で「収縮する」と証明できた件数・割合。残差 = どのラングでもまだ証明できずに残った件数。coverage が 64% → 95% に動いたのが本稿の訂正の核心。 |
| オラクル (oracle) | 「答えだけ教えてくれる」独立した判定器の総称。本稿の JSR 積オラクルは、長さ 6 以下の行列積を実際に計算して発散の反例を探す一方向チェック — 反例が見つかれば反証できるが、見つからなくても証明にはならない。 |
| 切替拡張性遺伝子 | 結合系では、どの行列がどの順番で適用されるか (切替列) によって挙動が変わる。観測した軌道の上では縮んで見えても、最悪の切替列では距離が伸びてしまう遺伝子のこと。これを拒否するのは verifier の正しい動作で、残差 10 のうち 6 がこれ。 |
| GA / fitness (適応度) | GA = 遺伝的アルゴリズム。候補解の集団に選択・交叉・突然変異を繰り返して良い解を「進化」させる探索法。fitness (適応度) はその良し悪しのスコアで、§6 では進化した回転 fitness が gate の強さ次第で ~0.41 → ~0.86 と変わる。 |
| RWKV | 状態を一歩ずつ更新しながら系列を処理する再帰型 (RNN 系) のニューラルネットワーク方式。本稿の「RWKV 風」は、その状態更新のかたちを借りた小さな力学系という意味で、言語モデルそのものを進化させているわけではない。 |
| Track B / C / D | 実験系列の呼び名。Track B = Z3 と閉形式スカラーテストの一致確認 (20000 件一致)、Track C = Z3 収縮チェックと閉形式不等式の一致確認 (3270 件一致)、Track D = 3270 遺伝子でのスケールチェック (+692 はここ)。 |
| margin-sweep | 判定のしきい値や余白 (margin) を上下に揺らし、結論がどれだけ頑健かを調べる赤チーム (攻撃役) の手法。ただし同じソルバの中で揺らすだけなので、ソルバ自体のバグには構造的に気づけない — 本稿の主要な教訓。 |
| fail-closed | 異常時や未設定時に「通す」のではなく「拒否する」側へ倒す設計。本稿の gate は CLARABEL がなければ黙って SCS に落ちずに拒否する。F1 はこのガードの隙 (falsy な `solver=""`) を突いた指摘だった。 |
| フロンティア (frontier) | 「いまの手法で証明できる範囲の最前線」のこと。証明書を強くするほど前線は押し広がるが、本稿の coverage フロンティアは JSR=1 の境界に漸近するだけで、完全には閉じない。 |
| 頂点列挙 / vertex-free | 素朴にやると、結合系の健全な検証は状態次元 n に対して 2ⁿ 個の頂点 (場合分けの組合せ) を全列挙する必要がある — n=32 で約 43 億。vertex-free はこの列挙をせずに健全性を保つ近似のことで、#36 の主役。 |
| 加算的 / opt-in | 加算的 = 既存のチェックを置き換えず、上に追加で重ねるだけ (外せば元の動作に戻る)。opt-in = 既定では無効で、使う人が明示的に有効化する方式。Rump gate はこの両方。 |
| p 値 | 「本当は差がなくても、偶然でその差が観測されてしまう確率」の目安。小さいほど偶然では説明しにくい。§6 の p = 3.1e-5 は約 10 万分の 3。 |
| footgun | 「自分の足を撃つ銃」、つまり後で誰かが踏んで事故になりやすい仕様やコードを指す俗語。pair-review が閉じた F1 がそれ。 |
| falsy | Python などで条件式に入れると偽 (False) 扱いになる値 (空文字列 `""`、0、None など)。F1 は「`solver=""` が falsy なため未指定と同じ扱いになり、デフォルトの SCS に落ちる」という罠だった。 |
...
SCS の下では、次数ラダーは豊かで相補的に見えました — これは**ソルバ由来の偽の結果 (アーティファクト)** でした。
...
| convex optimization | Optimization over a "bowl-shaped" landscape, where no small local dip can fool you — an answer found there is genuinely the best, so it can be used as a guarantee. SDP/LMI is a member of this family, which is exactly why it can serve in a certificate search. |
| interior-point method | A way of solving convex programs by walking through the *interior* of the feasible region at high accuracy. Keeps its precision even right at the boundary, at the price of heavier compute. CLARABEL is one. |
| first-order method / ADMM | A family of lightweight solvers that iterate using only gradient-level (first-derivative) information. ADMM is the classic example: fast and scalable, but near the boundary it tends to stop at an "approximately right" answer. SCS is one. |
| positive definite (PD) | The property of a matrix P that xᵀPx > 0 in every direction x. It is the condition for P to act as a (skewed) ruler for measuring distance — the P in a Lyapunov certificate must satisfy it. |
| eigenvalue / minimum eigenvalue | How much a matrix stretches or shrinks each direction. For a symmetric matrix, "minimum eigenvalue > 0" is equivalent to "positive definite". Here, float eigenvalue rechecks serve as an independent soundness confirmation. |
| singular value / SVD | The set of "stretch factors" of a matrix. If the largest is below 1, the matrix always shrinks distance (that is the 2-norm certificate). SVD (singular value decomposition) computes them — the speed-up in #36 needs just one SVD. |
| Cholesky factorization | Decomposing a positive-definite matrix into a product of triangular matrices LLᵀ. The factorization succeeding at all is itself evidence of positive definiteness; Rump verified-PD upgrades that evidence into a proof by rigorously bounding the rounding error. |
| induced norm (inf-norm / 2-norm) | A standard ruler for "how much a matrix stretches a state," derived from how vectors are measured. The inf-norm version is the max absolute row sum; the 2-norm version is the largest singular value. Both have a fixed ruler shape, so they miss systems that only shrink "when measured with a skewed ruler" — exactly where the SDP with a non-identity P breaks through. |
| lifting / Veronese | Rewriting the state x as the list of its monomials (x₁², x₁x₂, …) so a higher-degree proof becomes a quadratic proof in the lifted space. The map is called the Veronese embedding; degree-4/6/8 SOS are built this way. |
| closed form | An expression you can write down with finitely many elementary operations — no iteration, no solver. If a test reduces to closed form, there was never a reason to call a solver in the first place — which is why §4 can call Z3 decorative. |
| NP-hard | A class of problems believed to blow up in compute time, for every known algorithm, as the size grows. Exact JSR is one, so the "just compute everything exactly" route faces a principled wall. |
| soundness | "Whatever passes is guaranteed safe." Misses (false negatives) may be tolerated, but wrongly admitting something dangerous (a false admit) is never allowed. The top-priority property of a safety gate — and this solver trap fortunately hit the miss side, not the soundness side. |
| coverage / residual | Coverage = how many of the pool a certificate proves contracting (count and percentage). Residual = how many remain unproven at every rung. Coverage moving 64% → 95% is the heart of this article's correction. |
| oracle | An independent judge that "just tells you the answer." The JSR product oracle here actually computes matrix products up to length 6, hunting for divergence counterexamples — a one-way check: finding one refutes, finding none proves nothing. |
| switched-expansive gene | In a coupled system, behavior depends on the switching sequence — which matrix applies in which order. A gene that looks contracting on the observed trajectory but stretches distance under the worst-case switching sequence. Rejecting these is the verifier behaving correctly; 6 of the residual 10 are these. |
| GA / fitness | GA = genetic algorithm: a search that "evolves" a population of candidate solutions through repeated selection, crossover, and mutation. Fitness is the quality score — in §6 the evolved rotation fitness moves from ~0.41 to ~0.86 depending on the gate. |
| RWKV | A recurrent (RNN-family) neural network style that processes a sequence by updating a state step by step. "RWKV-style" here means a small dynamical system borrowing that state-update shape — not evolving a language model itself. |
| Track B / C / D | Names of experiment series. Track B = Z3 vs the closed-form scalar test (20000 agreements); Track C = the Z3 contraction check vs a closed-form inequality (3270 agreements); Track D = the 3270-gene scale check (the +692 lives here). |
| margin-sweep | A red-team technique that wobbles thresholds and margins up and down to see how robust a conclusion is. But it only wobbles inside the same solver, so it is structurally blind to a bug in the solver itself — the main lesson of this article. |
| fail-closed | A design that tips toward "reject" rather than "pass" on errors or missing configuration. The gate here refuses if CLARABEL is absent instead of silently falling back to SCS. F1 was a finding that poked a hole in exactly this guard (a falsy `solver=""`). |
| frontier | The current front line of "what today's methods can certify." Stronger certificates push the line outward, but the coverage frontier here only asymptotes to the JSR=1 boundary — it never fully closes. |
| vertex enumeration / vertex-free | Done naively, sound verification of the coupled system must enumerate all 2ⁿ vertices (case combinations) in the state dimension n — about 4.3 billion at n=32. Vertex-free means a sound approximation that skips this enumeration; it stars in #36. |
| additive / opt-in | Additive = layered on top of existing checks without replacing them (remove it and the old behavior returns). Opt-in = off by default, enabled only deliberately. The Rump gate is both. |
| p-value | A rough measure of "the probability of observing this difference by sheer chance even if there were no real difference." The smaller, the harder to explain away as luck. The p = 3.1e-5 in §6 is about 3 in 100,000. |
| footgun | Slang for "a gun aimed at your own foot": a spec or piece of code someone is likely to trip over later. F1, closed by the pair-review, was one. |
| falsy | A value treated as False in a conditional (empty string `""`, 0, None, …) in Python and friends. F1 was the trap that `solver=""` is falsy, gets treated the same as "unset," and falls through to the SCS default. |
...
| 凸优化 | 在「碗形」地形上找最低点的一类优化问题。碗形意味着不会被途中的小坑（局部解）欺骗，找到的答案确实是最优的，因此可以直接当作保证来用。SDP/LMI 属于凸优化——这正是它能用于「证书」搜索的原因。 |
| 内点法 (interior-point) | 沿可行域「内部」行进、以高精度求解凸优化的解法。即便在贴近边界的问题上也容易保持精度，代价是计算偏重。CLARABEL 即此类。 |
| 一阶法 / ADMM | 只用梯度（一阶斜率）级别的信息迭代改进的轻量解法家族。ADMM 是其代表：快、适合大规模，但在边界附近容易停在「差不多对」的答案上。SCS 即此类。 |
| 正定 (PD) | 矩阵 P 满足「沿任何方向 x 都有 xᵀPx > 0」的性质。这是 P 能充当（歪斜的）距离量尺的条件——Lyapunov 证书中的 P 必须满足它。 |
| 特征值 / 最小特征值 | 表示矩阵把各方向拉伸或压缩多少的数值。对称矩阵的「最小特征值 > 0」与「正定」等价。本文把 float 计算的特征值复查用作健全性的独立确认。 |
| 奇异值 / SVD | 矩阵把输入拉伸多少的「倍率」集合。若最大者小于 1，该矩阵必定缩短距离（这就是 2-norm 证书）。SVD（奇异值分解）用来求它们——#36 的加速只需一次 SVD。 |
| Cholesky 分解 | 把正定矩阵分解为三角矩阵之积 LLᵀ 的计算。分解能顺利完成本身就是正定性的证据；Rump verified-PD 通过严格估计舍入误差，把这份「证据」升级为「证明」。 |
| 诱导范数 (inf-norm / 2-norm) | 由向量度量方式导出的、「矩阵把状态拉伸多少」的标准量尺。inf-norm 版是行绝对值和的最大值，2-norm 版是最大奇异值。两者的量尺形状都是固定的，因此会漏掉「换一把歪斜量尺度量才缩小」的系统——允许非单位 P 的 SDP 正是在这里突破。 |
| 提升 (lifting) / Veronese | 把状态 x 改写为其单项式列表 (x₁², x₁x₂, …)，使高次证明变成提升空间中的二次证明的技巧。该映射称为 Veronese 嵌入；degree-4/6/8 SOS 就这样构造。 |
| 闭式 (closed form) | 不靠迭代或求解器、用有限次初等运算就能写出的式子。若判定能归约为闭式，本来就不需要调用求解器——§4 能说「Z3 是装饰性的」正因如此。 |
| NP 困难 | 被认为随问题规模增大、对所有已知算法计算时间都会爆炸的一类问题。精确 JSR 属于此类，所以「全部精确算出来」的路线存在原理性的墙。 |
| 健全性 (soundness) | 「凡是放行的就绝对安全」的性质。漏掉（假阴性）可以容忍，但绝不允许把危险的东西错误放行（假许可）。这是安全 gate 的最高优先级性质——幸运的是这次求解器的陷阱打在漏掉一侧，而非健全性一侧。 |
| coverage / 残差 | coverage = 池中能被该证书证明「收缩」的件数与比例。残差 = 在所有层级都仍未被证明而剩下的件数。coverage 从 64% 变到 95%，正是本文修正的核心。 |
| 预言机 (oracle) | 「只告诉你答案」的独立判定器的统称。本文的 JSR 乘积预言机实际计算长度 ≤6 的矩阵乘积、寻找发散反例——单向检查：找到反例即可证伪，找不到也不构成证明。 |
| 切换扩张性基因 | 在耦合系统里，行为取决于哪个矩阵以什么顺序作用（切换序列）。这类基因在观测轨道上看似收缩，但在最坏切换序列下距离会变长。拒绝它们是 verifier 的正确行为：残差 10 个中有 6 个是它。 |
| GA / fitness (适应度) | GA = 遗传算法：对候选解群体反复施加选择、交叉、变异来「进化」出好解的搜索法。fitness（适应度）是优劣评分——§6 中进化的旋转 fitness 随 gate 强弱从 ~0.41 变到 ~0.86。 |
| RWKV | 一边逐步更新状态一边处理序列的循环（RNN 系）神经网络方式。本文的「RWKV 风格」指借用其状态更新形式的小型动力系统——并不是在进化语言模型本身。 |
| Track B / C / D | 实验系列的名字。Track B = Z3 与闭式标量测试的一致性确认（20000 件一致）；Track C = Z3 收缩检查与闭式不等式的一致性确认（3270 件一致）；Track D = 3270 基因的规模检查（+692 出自这里）。 |
| margin-sweep | 红队（攻击方）手法：上下晃动判定阈值与余量，看结论有多稳健。但它只在同一个求解器内部晃，对求解器本身的 bug 结构性失明——本文的主要教训。 |
| fail-closed | 出错或未配置时倒向「拒绝」而非「放行」的设计。本文的 gate 在 CLARABEL 缺失时直接拒绝，绝不静默退回 SCS。F1 指出的正是这道守卫的缝隙（falsy 的 `solver=""`）。 |
| 前沿 (frontier) | 「现有手法所能证明范围的最前线」。证书越强前线推得越远，但本文的 coverage 前沿只会渐近 JSR=1 边界——不会完全闭合。 |
| 顶点枚举 / vertex-free | 朴素地做，耦合系统的健全验证要对状态维数 n 枚举全部 2ⁿ 个顶点（情形组合）——n=32 约 43 亿。vertex-free 指跳过这种枚举仍保持健全的近似，是 #36 的主角。 |
| 加性 / opt-in | 加性 = 不替换现有检查、只在其上叠加（去掉即回到原行为）。opt-in = 默认关闭、由使用者明确启用。Rump gate 两者皆是。 |
| p 值 | 「即使真实差异为零，纯靠偶然也观测到这个差异的概率」的粗略度量。越小越难用运气解释。§6 的 p = 3.1e-5 约为十万分之三。 |
| footgun | 俚语「打自己脚的枪」：日后多半有人踩中出事故的规格或代码。pair-review 关闭的 F1 即是。 |
| falsy | 在 Python 等语言中、放进条件式会被当作假 (False) 的值（空字符串 `""`、0、None 等）。F1 的陷阱正是 `solver=""` 为 falsy、被当作「未指定」而落入默认的 SCS。 |
...
在 SCS 下，次数阶梯看起来丰富且互补——这是一个**源自求解器的虚假发现（伪影）**。
...
| 볼록 최적화 | 「그릇 모양」지형에서 가장 낮은 점을 찾는 유형의 최적화 문제. 그릇 모양이면 도중의 작은 움푹함 (국소해) 에 속지 않으므로, 찾은 답을 그대로 보장으로 쓸 수 있다. SDP/LMI 는 볼록 최적화의 일종 — 그래서 "증명서" 탐색에 쓸 수 있다. |
| 내점법 (interior-point) | 실행 가능 영역의 "안쪽"을 지나며 고정밀로 볼록 최적화를 풀어 가는 해법. 경계에 바짝 붙은 문제에서도 정밀도를 유지하기 쉬운 대신 계산이 무겁다. CLARABEL 이 이 방식. |
| 1 차법 / ADMM | 기울기 (1 차 미분) 수준의 정보만으로 반복 개선하는 가벼운 해법 family. ADMM 이 대표격으로, 빠르고 대규모에 강한 대신 경계 부근에서는 "대충 맞는" 답에서 멈추기 쉽다. SCS 가 이 방식. |
| 양정치 (PD) | 행렬 P 가 "어느 방향 x 로 재도 xᵀPx > 0" 이 되는 성질. 거리를 재는 (비틀린) 잣대로 쓸 수 있는 행렬의 조건으로, Lyapunov 증명서의 P 는 이를 만족해야 한다. |
| 고윳값 / 최소 고윳값 | 행렬이 각 방향을 얼마나 늘리고 줄이는지 나타내는 값. 대칭 행렬에서는 "최소 고윳값 > 0" 과 "양정치" 가 동치. 본문에서는 float 로 계산한 고윳값 재확인을 건전성의 독립 확인으로 쓴다. |
| 특이값 / SVD | 행렬이 입력을 얼마나 늘리는지의 "배율" 집합. 그 최댓값이 1 미만이면 그 행렬은 반드시 거리를 줄인다 (이것이 2-norm 증명서). SVD (특이값 분해) 가 이를 구하며, #36 의 고속화는 SVD 1 회면 된다. |
| Cholesky 분해 | 양정치 행렬을 삼각 행렬의 곱 LLᵀ 로 분해하는 계산. 분해가 끝까지 성공한다는 것 자체가 양정치성의 증거가 된다. Rump verified-PD 는 이 계산의 반올림 오차까지 엄밀히 추정해 "증거"를 "증명"으로 격상시킨 기법. |
| 유도 노름 (inf-norm / 2-norm) | 벡터를 재는 방식에서 유도한, "행렬이 상태를 얼마나 늘리는가"의 표준 잣대. inf-norm 판은 행 절댓값 합의 최대, 2-norm 판은 최대 특이값. 둘 다 잣대의 모양이 고정이라 "비틀린 잣대로 재면 줄어드는" 시스템을 놓친다 — 비단위 P 를 허용하는 SDP 가 바로 거기를 돌파한다. |
| 리프팅 (lifting) / Veronese | 상태 x 를 그 단항식 목록 (x₁², x₁x₂, …) 으로 옮겨 적어, 고차 증명을 "리프트된 공간에서의 2 차 증명"으로 푸는 테크닉. 이 사상을 Veronese 매장이라 부르며, degree-4/6/8 SOS 는 이렇게 만들어진다. |
| 닫힌 형식 (closed form) | 반복 계산이나 솔버에 기대지 않고 유한 번의 초등 연산으로 적어 내릴 수 있는 식. 닫힌 형식으로 환원되는 판정이라면 애초에 솔버를 부를 필요가 없다 — §4 에서 "Z3 는 장식적"이라 말할 수 있는 이유. |
| NP-난해 | 문제 크기가 커지면 알려진 어떤 알고리즘으로도 계산 시간이 폭발적으로 늘어난다고 여겨지는 문제 클래스. 정확 JSR 이 여기에 해당해, "전부 정확히 계산"하는 노선에는 원리적인 벽이 있다. |
| 건전성 (soundness) | "통과시킨 것은 절대 안전"이라는 성질. 놓침 (위음성) 은 허용되더라도, 위험한 것을 잘못 통과시키는 것 (위허가) 만은 결코 하지 않는다. 안전 gate 의 최우선 성질로, 이번 솔버의 함정은 다행히 건전성 쪽이 아니라 놓침 쪽이었다. |
| coverage / 잔차 | coverage = 풀 가운데 그 증명서로 "수축한다"고 증명할 수 있었던 건수·비율. 잔차 = 어느 등급에서도 아직 증명되지 못하고 남은 건수. coverage 가 64% → 95% 로 움직인 것이 본문 정정의 핵심. |
| 오라클 (oracle) | "답만 알려 주는" 독립 판정기의 총칭. 본문의 JSR 곱 오라클은 길이 6 이하의 행렬 곱을 실제로 계산해 발산 반례를 찾는 단방향 검사 — 반례를 찾으면 반증이 되지만, 못 찾아도 증명은 아니다. |
| 스위치 확장성 유전자 | 결합계에서는 어느 행렬이 어떤 순서로 적용되는가 (스위칭 열) 에 따라 거동이 달라진다. 관측한 궤도에서는 수축해 보여도 최악의 스위칭 열에서는 거리가 늘어나는 유전자. 이를 거부하는 것이 verifier 의 올바른 동작으로, 잔차 10 중 6 개가 이것. |
| GA / fitness (적응도) | GA = 유전 알고리즘. 후보 해 집단에 선택·교차·돌연변이를 반복해 좋은 해를 "진화"시키는 탐색법. fitness (적응도) 는 그 좋고 나쁨의 점수로, §6 에서는 진화한 회전 fitness 가 gate 의 세기에 따라 ~0.41 → ~0.86 으로 달라진다. |
| RWKV | 상태를 한 걸음씩 갱신하며 시퀀스를 처리하는 순환형 (RNN 계) 신경망 방식. 본문의 "RWKV 풍"은 그 상태 갱신 형태를 빌린 작은 동역학계라는 뜻으로, 언어 모델 자체를 진화시키는 것은 아니다. |
| Track B / C / D | 실험 계열의 이름. Track B = Z3 와 닫힌 형식 스칼라 테스트의 일치 확인 (20000 건 일치), Track C = Z3 수축 검사와 닫힌 형식 부등식의 일치 확인 (3270 건 일치), Track D = 3270 유전자 스케일 체크 (+692 는 여기). |
| margin-sweep | 판정 문턱값과 여백 (margin) 을 위아래로 흔들어 결론이 얼마나 견고한지 살피는 레드팀 (공격역) 기법. 다만 같은 솔버 안에서 흔들 뿐이라 솔버 자체의 버그에는 구조적으로 눈멀다 — 본문의 주요 교훈. |
| fail-closed | 이상시·미설정시에 "통과"가 아니라 "거부" 쪽으로 기울이는 설계. 본문의 gate 는 CLARABEL 이 없으면 조용히 SCS 로 떨어지지 않고 거부한다. F1 은 이 가드의 틈 (falsy 한 `solver=""`) 을 짚은 지적이었다. |
| 프런티어 (frontier) | "지금의 기법으로 증명할 수 있는 범위의 최전선". 증명서를 강화할수록 전선은 밀려 넓어지지만, 본문의 coverage 프런티어는 JSR=1 경계에 점근할 뿐 완전히 닫히지 않는다. |
| 꼭짓점 열거 / vertex-free | 순진하게 하면 결합계의 건전한 검증은 상태 차원 n 에 대해 2ⁿ 개의 꼭짓점 (경우의 조합) 을 전부 열거해야 한다 — n=32 에서 약 43 억. vertex-free 는 이 열거 없이 건전성을 지키는 근사로, #36 의 주역. |
| 가산적 / opt-in | 가산적 = 기존 검사를 대체하지 않고 위에 더 얹기만 함 (떼어 내면 원래 동작으로 돌아감). opt-in = 기본은 꺼져 있고 쓰는 사람이 명시적으로 켠다. Rump gate 는 둘 다에 해당. |
| p 값 | "실제로는 차이가 없어도 우연히 그 차이가 관측될 확률"의 대략적 지표. 작을수록 우연으로 설명하기 어렵다. §6 의 p = 3.1e-5 는 약 10 만분의 3. |
| footgun | "자기 발을 쏘는 총", 즉 나중에 누군가 밟아 사고가 나기 쉬운 사양·코드를 가리키는 속어. pair-review 가 닫은 F1 이 그것. |
| falsy | Python 등에서 조건식에 넣으면 거짓 (False) 취급되는 값 (빈 문자열 `""`, 0, None 등). F1 은 "`solver=""` 가 falsy 라서 미지정과 똑같이 취급되어 기본 SCS 로 떨어진다"는 함정이었다. |
...
SCS 하에서 차수 사다리는 풍부하고 상보적으로 보였습니다 — 이것은 **솔버에서 비롯된 거짓 결과 (아티팩트)** 였습니다.

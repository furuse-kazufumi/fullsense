...
<small><strong>EN:</strong> “Beyond Transformer”: Mamba / Jamba / RWKV / Diffusion + thought factors → SSM Δ Bridge / <strong>中:</strong> "Transformer 之外": Mamba / Jamba / RWKV / Diffusion + 思考因子 → SSM Δ Bridge</small>
...
<small><strong>EN:</strong> The 'pipe through which vectors flow' = the LLM backend. We can also wire non-Transformer pipes. / <strong>中:</strong> "流过 vector 的管道" = LLM backend. 非 Transformer 的管道也能接入.</small>
...
<small><strong>EN:</strong> llive's LLMBackend Protocol is designed so any of them plug in. / <strong>中:</strong> llive 的 LLMBackend Protocol 设计成任意 backend 都能插入.</small>
具体的には:
...
<small><strong>EN:</strong> llive's 4-layer memory (#24-01) assumes long context. Mamba / SSM make that cheap. / <strong>中:</strong> llive 的 4 层记忆 (#24-01) 以长 context 为前提. Mamba / SSM 让这变得便宜.</small>
...
<small><strong>EN:</strong> Bo Peng (RWKV-6, 2024) showed that 'attention is a special case of time-series'. / <strong>中:</strong> Bo Peng (RWKV-6, 2024) 证明了 "attention 是时序的特例".</small>
...
<small><strong>EN:</strong> Diffusion-LM / SEDD (Lou et al. 2024) generates text via noise → denoise — non-autoregressive. / <strong>中:</strong> Diffusion-LM / SEDD (Lou 等 2024) 通过 噪声 → 去噪 生成文本 — 非自回归.</small>
...
<small><strong>EN:</strong> This is the 'expectations' section of the article. The plan: / <strong>中:</strong> 这是本文的 "期望值" 部分. 计划如下:</small>
...
<small><strong>EN:</strong> Per the constraint, repeat explicitly: / <strong>中:</strong> 依据 constraint, 明确重复:</small>

...
为什么唯独 SDP 这么强?打个比方,第1级、第2级是**「现成的尺子」**:把事先定好的同一种量法压到每个个体身上,只问「用这把尺子量,能不能断言它会收缩」。又快又省事,但碰巧不合这把尺子的个体,即使其实安全,也只能得到「无法判断」。相比之下,SDP 是**「为每个个体量身定制一把尺子」**的做法:为那个个体专门找出一种量法(一种衡量「能量」的方式),并证明「用这种量法来量,它随时间必然不断收缩」。现成尺子量不了的个体,只要存在一把合身的尺子就能被救回——这正是 SDP 能一举捞起 +149 个的原因。

剩下的 10 个也有诚实的内情:**其中 6 个是「正确拒绝了真正危险的个体」**,而非漏判。真正无法证明的只是处在边界线上的少数几个。所谓「边界线上」,是指恰好站在「收缩」与「膨胀」分界线边缘的个体。就像恰好考在及格线上的答卷最难判定及格与否一样,越贴近分界线的个体越难证明。而且,精确划出这条分界线的计算,已知属于数学界标记为「尚未找到高效解法的难题」的那一类(专业上称 NP 困难)。所以「最后几个」的残留不是检查器偷懒,而是数学上不可避免的宿命——再多算力也无法完全闭合(诚实而干净的极限)。

「阶梯」这个比喻之所以贴切,是因为检查器的强度有**干净的顺序**:下面一级能做到的,上面一级必然也能做到。所以只要量出「需要爬到第几级」,就能选出一个不多不少、恰到好处的检查器。这次的答案很明快:**爬到第3级 SDP,就达到 95%;再往上加更重的计算,也只多 4 个。** 从性价比看,SDP 正是该停下的那个平台。

大规模版(用 3270 个测试)结论同样不动摇。SDP 证明了 **95%**,粗检查胜过 SDP 的个数为 **零**。「为零」的意思是,「粗检查能证明、SDP 却证明不了」这种逆转一次也没有发生。SDP 是严格的上位集合,把其它全吞了。「只是小实验碰巧走运」的怀疑,也因为规模扩大 10 倍后得到同样结论而消散——300 个上成立的结论在 3270 个上原封不动地成立,「运气好」已经解释不通了。
...
把「能从简单公式得出」再讲细一点。三角形的面积,套用「底 × 高 ÷ 2」的公式一步就出来了,完全不需要「把所有可能的面积逐个尝试再验证哪个对」。这次的检查正是同一类问题:答案可以直接写成公式的形状。对一个答案早已由公式锁定的问题投入搜索机器,它能返回的也只有公式给出的那个答案。所以 2 万次尝试 2 万次一致,不是「SMT 聪明」的证据,而是**「从一开始公式就足够了」**的证据。

> 比喻:**算 1 美元购物找零多少,不需要世界最快的 AI 超算。心算就行。**
> SMT 正是那台「超算」——很了不起,但对这个问题**过度且浪费**。

真正管用的是朴素而经典的 **SDP / Lyapunov**(一种寻找「能量持续下降」之证明的工具)。这同样可以用日常比喻来讲。往碗里丢一颗玻璃珠:它滚来滚去,但你可以断言它最终一定停在碗底——之所以敢断言,是因为「玻璃珠的高度(能量)每滚一下都必然降低」。Lyapunov 证书就是这么一回事:**为受检个体找出一种「必然持续下降的能量量法」。** 一旦找到,就等于证明了「这个个体终将安定下来=不会失控」。而妙处在于,寻找这种量法的问题,会落到「在碗形地形上找最低点」那一类优化(凸优化)上。碗形地形没有「假山谷」,找到的答案自带保证——所以 SDP 不是「看起来像那么回事的判定」,而是可以当作**「证明」**来用的。正确答案是契合问题的工具——而非最花哨、最强的求解器。这就是本文的核心。
...
这里有点出人意料:陷阱的真面目是**「测量工具(求解器)本身给出的答案略有偏差」**。检查的道理是对的,但执行计算的机器精度不够,于是对恰好处在边界上的个体给出了「其实安全、却报成危险」的误判。用研究的话说:尺子是歪的,所以一条本来笔直的线被读成了「弯的」。

再深入一步:求解器有性格之分。SCS 属于「每一步又轻又快,但精度只到『大致正确』为止」的类型。平时这就够用了,可对恰好压在及格线上的个体,那点「大致」的误差足以翻转判定。想象一张正好考在及格线上的答卷,被丢进一台粗放的阅卷机——偶尔会被误判为不及格。而 CLARABEL 属于「每一步较重,但能一直精确逼到边界」的类型,所以擅长毫厘之间的判定。要注意,这个误判的方向是「把安全的说成危险」,所以并没有危险个体漏网(正如 fail-closed 设计所要求的,连出错也错在安全一侧)。但问题依然存在:它白白弹掉了本可合格的个体,**让检查器显得比实际更弱、成绩单显得比实际更复杂**。「莫名复杂」的真面目,正是这个。

换上更准确的求解器(**CLARABEL**)后,数字一下子整齐了,结论反而变得**更简单、更强。** 感觉就像把歪尺子换成直尺:那些绕来绕去的辩解全都不再需要,故事一下子拢成了一条线。

> 教训:**「结果太好(或复杂得反常)」意味着庆祝之前先怀疑你的工具。**
> 因为我们事先放好了避免空欢喜的机制,才到达了正确的地基。这也是 FullSense 的「诚实披露」规矩——不隐瞒失败与局限,必须写下来——真正发挥作用的一刻。
...
여기서 「움직이는 방식」이란 **상태가 시간에 따라 어떻게 변하는가의 규칙**을 말합니다 — 진자가 어떻게 흔들리는지, 욕조 물 온도가 어떻게 돌아오는지 같은 규칙이죠. 진자라면 「지금 오른쪽으로 크게 흔들렸다면, 다음 순간에는 중심으로 돌아가려 한다」, 욕조 물이라면 「지금 너무 뜨겁다면, 시간이 지나면서 조금씩 식는다」 — 이런 「지금 상태에서 다음 순간의 상태를 정하는 규칙」이 우리가 고쳐 쓰고 있는 대상입니다. 이 글에서 「개체」라고 부르는 것은 이런 규칙 한 세트를 지닌 후보라고 생각해 주세요. 진화는 규칙을 살짝 바꾼 개체를 대량으로 만들어 유망한 것을 남깁니다.

「진화」 쪽은 생물을 떠올려 주세요. 부모와 설계가 아주 조금만 다른 자식을 많이 만들고(변이), 그중 살아남기 쉬운 것만 다음 세대에 남긴다(선택). 이것을 수백 세대 반복하면, 아무도 설계도를 그리지 않았는데 집단은 점점 환경에 맞는 모습으로 다가갑니다. llcore가 하는 일은 바로 이것을 「AI의 움직이는 방식」에 적용한 것뿐입니다. 설계자가 머리를 짜내 최적해를 한 방에 맞히는 게 아니라, **대충 많이 시도하고 좋았던 것을 남기는** — 어떤 의미로는 아주 투박한 방식입니다.

문제는 여기서부터입니다. 이 「투박한 방식」에는, 내버려 두면 곤란한 것이 섞여 든다는 함정이 있습니다.
...
FullSense 규약에 따라 이 관문은 **fail-closed**입니다: 「증명하지 못하면 통과시키지 않는다」 — 의심스러우면 들이지 않는다. 공항 보안 검색을 떠올려 보세요. 내용물을 알 수 없는 짐을 「아마 괜찮겠지」 하고 통과시킨다면 검색의 의미가 없습니다. 「희다고 확인된 것만 통과. 회색은 전부 멈춘다」 — 이것이 fail-closed입니다. 회색을 멈추면 사실은 안전했던 짐까지 멈추는 일도 생깁니다. 그래도 위험한 것을 하나 통과시키는 실패보다, 안전한 것을 하나 더 멈추는 실패가 훨씬 낫다 — 틀릴 거라면 반드시 안전한 쪽으로 틀린다. 이것이 이 설계의 사고방식입니다.
...
「드리프트」란 채택 시점에는 멀쩡해 보이던 자식이 세대를 거듭하며 서서히 폭주 쪽으로 흘러간다는 뜻입니다. 검사 없이 진화를 돌리면 채택된 자식의 대략 5–6개 중 1개가 이 길을 걸었습니다. 「가끔 사고가 난다」가 아니라 「일상적으로 섞여 든다」 수준의 비율로, 도저히 방치할 수 없는 숫자입니다. 검사기를 넣자 이 사고가 0이 되었다 — 우선 이것이 검사기의 첫 번째 일입니다.
...
여기서 「품질」이란 진화 계산에서 개체의 성적을 재는 점수(전문 용어로 fitness, 적응도)를 말합니다. 점수가 높은 개체일수록 다음 세대에 남기 쉽습니다. 이 표가 말하는 것은, 「안전하다고 증명된 개체만」으로 도달할 수 있는 성적의 상한이, 관문을 강하게 바꾸는 것만으로 0.41에서 0.86으로 두 배 이상 뛰었다는 사실입니다. 참고로 「통계적으로 거의 확실」은 기분 이야기가 아닙니다: 이 차이가 순전히 우연으로 나타날 확률은 약 10만분의 3으로 계산되어 있습니다.

검사기는 흔히 「위험을 거르는 안전장치」로 여겨지지만, 실은 **「안심하고 더 멀리 탐색하게 해 주는 장치」**이기도 했습니다. 난간이 있어야 절벽 끝까지 걸어갈 수 있는 셈입니다. 뒤집어 말하면, 느슨한 관문밖에 없는 진화는 「무서워서 멀리 가지 못합니다」. 위험해 보이는 것을 큼직하게 뭉뚱그려 피할 수밖에 없으므로, 사실은 안전했던 좋은 개체까지 함께 버리게 되고, 결과적으로 낮은 곳에서 제자리걸음을 합니다. 강한 관문은 「여기는 정말 안전」을 세밀하게 가려 주므로, 진화는 안심하고 더 높은 곳까지 공략할 수 있는 것입니다.
...
왜 SDP만 이렇게 강할까요? 비유하자면 1단·2단은 **「기성품 자」**입니다. 미리 정해진 한 가지 재는 법을 모두에게 똑같이 대고, 「이 자로 재면 줄어든다고 단언할 수 있는가」만 봅니다. 빠르고 간편한 대신, 하필 그 자에 맞지 않는 개체는 사실 안전해도 「모르겠다」 처리됩니다. 반면 SDP는 **「개체마다 맞춤 자를 짓는」** 방식입니다. 그 개체 전용의 재는 법(에너지를 재는 법)을 찾아내어, 「이 재는 법으로 재면 시간이 갈수록 반드시 줄어든다」고 증명합니다. 기성품으로 잴 수 없던 개체라도 그 개체에 맞는 자만 찾아내면 구할 수 있다 — 그래서 단숨에 +149개나 건져 올릴 수 있었던 것입니다.

남은 10개에도 정직한 내역이 있습니다: **그중 6개는 「정말로 위험한 개체를 올바르게 거른 것」**이지 놓침이 아닙니다. 진짜로 증명하지 못한 것은 경계선에 아슬아슬하게 걸친 몇 개뿐. 「경계선」이란 「줄어든다」와 「불어난다」의 경계 바로 위에 서 있는 개체를 말합니다. 합격점에 딱 걸친 답안일수록 합격/불합격 판정이 어려운 것과 마찬가지로, 경계에 가까운 개체일수록 증명은 어려워집니다. 게다가 이 경계를 엄밀하게 산출하는 계산은, 수학의 세계에서 「효율적으로 푸는 방법이 발견되지 않은 난제」(전문적으로는 NP-난해)로 분류된다고 알려져 있습니다. 즉 「마지막 몇 개」가 남는 것은 검사기의 게으름이 아니라 수학적으로 피할 수 없는 숙명 — 아무리 계산기를 돌려도 완전히 닫히지 않습니다(정직하고 깔끔한 한계).

「사다리」라는 비유가 들어맞는 것은, 검사기의 강도에 **깨끗한 순서**가 있기 때문입니다. 아래 단에서 할 수 있는 일은 위 단에서는 반드시 할 수 있다. 그러니 「어디까지 오르면 충분한가」를 재면, 과부족 없는 검사기를 고를 수 있습니다. 이번 답은 명쾌했습니다: **3단 SDP까지 오르면 95%에 닿는다. 거기서 위(더 무거운 계산)를 더해도 늘어나는 것은 겨우 4개.** 비용 대비 효과로 보면 SDP가 딱 멈추기 좋은 층계참이었습니다.

대규모판(3270개로 테스트)에서도 결론은 흔들리지 않았습니다. SDP는 **95%**를 증명했고, 거친 검사가 SDP를 이긴 개체 수는 **0**이었습니다. 「0」이란 「거친 검사로는 증명할 수 있었는데 SDP로는 증명할 수 없었다」는 역전이 한 건도 일어나지 않았다는 뜻입니다. SDP는 다른 것들을 모두 삼키는 엄격한 상위집합이었습니다. 「작은 실험에서 우연히 잘 나왔을 뿐」이라는 의심도, 규모를 10배로 키워 같은 결론이 나오면서 사라졌습니다. 300개에서 성립한 이야기가 3270개에서도 그대로 성립한다면, 「우연한 행운」으로는 설명되지 않기 때문입니다.
...
「간단한 식으로 나온다」를 조금 더 풀어 보겠습니다. 삼각형의 넓이는 「밑변 × 높이 ÷ 2」라는 공식에 대입하면 한 방에 나옵니다. 「있을 수 있는 넓이를 닥치는 대로 시도해 보고 맞는지 검증하는」 수고는 필요 없습니다. 이번 검사도 똑같은 종류의 문제였습니다: 답이 그대로 공식의 형태로 적히는 문제. 답이 이미 공식으로 확정된 문제에 탐색 기계를 투입해도, 나오는 것은 공식과 같은 답뿐입니다. 그러니 2만 번 시험해 2만 번 일치한 것은 「SMT가 똑똑했다」는 증거가 아니라, **「처음부터 공식만으로 충분했다」**는 증거였던 것입니다.

> 비유: **1달러어치 사고 거스름돈이 얼마인지 알아내는 데 세계에서 가장 빠른 AI 슈퍼컴은 필요 없다. 암산으로 된다.**
> SMT가 바로 그 「슈퍼컴」 ── 훌륭하지만 이 문제엔 **과하고 낭비**였다.

정말 도움이 된 것은 수수하고 고전적인 **SDP / Lyapunov**(에너지가 계속 줄어든다는 증거를 찾는 도구) 쪽이었습니다. 이것도 일상의 비유로 설명할 수 있습니다. 그릇 안에 구슬을 떨어뜨리면, 데굴데굴 굴러도 결국 반드시 바닥에서 멈춥니다 — 그렇게 단언할 수 있는 이유는 「구슬의 높이(에너지)가 구를 때마다 반드시 줄어들기」 때문입니다. Lyapunov 증명서란 검사하려는 개체에 대해 **「반드시 계속 줄어드는 에너지의 재는 법」을 하나 찾아내는 일**입니다. 그것이 발견되면 「이 개체는 언젠가 반드시 안정된다 = 폭주하지 않는다」를 증명한 셈이 됩니다. 그리고 그 재는 법을 찾는 문제가 「그릇 모양 지형에서 가장 낮은 점을 찾는」 타입의 최적화(볼록 최적화)로 떨어진다는 것이 묘미입니다. 그릇 모양 지형에는 「가짜 골짜기」가 없으므로, 찾아낸 답에는 보증이 붙습니다 — 그래서 SDP는 「그럴듯한 판정」이 아니라 **「증명」**으로 쓸 수 있는 것입니다. 정답은 문제에 맞는 도구였지, 가장 화려하고 강한 솔버가 아니었습니다. 이것이 이 글의 핵심입니다.
...
여기가 조금 뜻밖인 대목인데, 함정의 정체는 **「재는 도구(솔버) 자체가 답을 조금 엉성하게 내고 있었다」**는 것이었습니다. 검사의 이치는 옳은데, 그것을 계산하는 기계의 정밀도가 모자라 경계 바로 위의 개체에서 「사실은 안전한데 위험」이라는 오판을 내고 있었던 것이죠. 연구로 치면, 자가 휘어 있던 탓에 사실은 곧은 선이 「굽었다」고 읽히던 이야기입니다.

한 걸음만 더 들어가면, 솔버에는 성격 차이가 있습니다. SCS는 「한 걸음 한 걸음이 가볍고 빠른 대신, 정밀도는 '대충 맞다' 수준에서 멈추는」 타입. 평소에는 그걸로 충분하지만, 합격선에 딱 걸친 개체에서는 그 '대충'의 오차가 판정을 뒤집어 버립니다. 합격점에 딱 걸친 답안을 대충 채점기에 돌리면 가끔 낙제로 처리되는 그림입니다. 반면 CLARABEL은 「한 걸음은 무겁지만, 경계까지 빈틈없이 조여서 푸는」 타입이라 아슬아슬한 판정에 강합니다. 참고로 이 오판은 「안전한 것을 위험하다고 말하는」 방향이므로, 위험한 개체가 빠져나간 것은 아닙니다(fail-closed 설계대로, 틀리더라도 안전한 쪽으로 틀렸습니다). 그래도 문제인 것은, 합격할 수 있었던 개체를 헛되이 걸러내어 **검사기의 실력을 실제보다 낮게, 성적표를 실제보다 복잡하게 보이게 만든다**는 점. 「묘하게 복잡」해 보였던 정체가 바로 이것이었습니다.

더 정확한 솔버(**CLARABEL**)로 교체하자 숫자가 한꺼번에 정돈되고, 결론은 오히려 **더 단순하고 더 강해졌습니다.** 휜 자를 곧은 자로 바꿔 끼우자, 빙빙 둘러대던 설명이 전부 필요 없어지고 이야기가 한 줄기로 모인 감각입니다.

> 교훈: **「결과가 너무 좋다(혹은 묘하게 너무 복잡하다)」는 기뻐하기 전에 먼저 도구를 의심하라.**
> 헛된 기쁨을 피하는 장치를 미리 두었기에 올바른 토대에 도달했습니다. 이는 FullSense의 「정직 공개」 규율 ── 실패와 한계를 숨기지 않고 반드시 적어 남긴다 ── 가 실제로 효과를 낸 순간이기도 했습니다.

...
생물은, 필요 없는 것을 자꾸자꾸 버리고 단순해지는 경우가 있다. 캄캄한 동굴에 사는 물고기는 눈을 버리고, 다른 생물에 기대어 사는 기생충은 자기 유전자를 버리고, 심해의 세균은 게놈을 깎아 가벼워진다. 왜 버리는 쪽이 이득일까 —— 눈도 유전자도, 갖고 있는 것만으로 유지에 비용(에너지, 복제의 수고)이 든다. 빛 없는 동굴에서 눈은 아무 쓸모도 없는데 유지비만 떼인다면, 버린 개체가 더 살아남기 쉽다. **"유지비 > 유용도"라면, 단순한 쪽이 살아남는다** —— 이것이 생물 세계의 냉정한 규칙이다.

이것을 AI의 진화에도 들이고 싶다. 지금은 "성능이 좋은가"만으로 개체를 고른다. 거기에 "**싸게 검증할 수 있는 단순한 몸**"이라는 관점을 점수에 더해 주면, AI는 누가 시키지 않아도 스스로, 문지기가 한눈에 통과시킬 수 있는 가벼운 구조로 향해 갈 것이다. 비용의 낮음 자체를 "진화의 선택압"으로 삼는다는 발상이다.

다만 여기에는 함정이 있다. "단순"에는 가려내야 할 종류가 있기 때문이다.

- **좋은 단순** = 몸의 구조 자체가 심플(정말로 가볍다). 검증도 싸고, 진화도 움직이기 쉽다. 대환영.
- **나쁜 단순①** = 문지기가 너무 엄격한 탓에, 아무것도 학습하지 않는 "우등생 흉내"가 살아남는다(= 가장 싼 ∞문지기의 함정). 안전해 보이지만 그저 움츠리고 있을 뿐.
- **나쁜 단순②** = "아무 말도 하지 않는다"로 퇴화한 개체(= 무난한 비(非)답만 돌려주는, unigram에 달라붙은 상태). 안전하고 싸지만 무능. 너무 깎아서 자립할 수 없게 된 기생충과 같다.

즉 "싸기만 하면 뭐든 좋다"고 소박하게 보상을 주면, AI는 편한 쪽으로, 퇴화하는 쪽으로 달아나 버린다. 그래서 성능과 비용은 단순히 더하는(스칼라화) 게 아니라, 둘의 균형(파레토)으로 다뤄야 한다.

다행히 llcore에는 "정말로 안정되어 있는가"를 꿰뚫어 보는 눈(건전성 oracle)이 있다. 그래서 **좋은 단순과 퇴화한 단순을 구분할 수 있다** —— 같은 "단순하고 싼" 개체라도, 제대로 내용이 있는 개체인지, 그저 움츠린 개체인지를 판별할 수 있다. 이것은 보통의 진화 시스템에는 불가능한 재주다. 이것이 바로 FullSense의 세계관 —— "똑똑하기만 한 것이 아니라, 싸고·안전하다고 증명할 수 있는 방향으로 진화한다."

## 정직한 내역: "검사기가 똑똑하게 만든다"는 과장이었다

마지막으로, 오늘 가장 많이 깎은 이야기를 한다. 실험에서는 "강한 문지기일수록 AI가 똑똑해진다(언어 예측을 잘하게 된다)"는 결과가 나와 있었다. 10번 시험해 매번 같은 경향, 우연으로는 일어나기 어려운 차이로, 숫자 자체는 진짜다. 하지만 —— 이긴 기분이 되기 전에, 내역을 의심했다.

그래서 한 것이 대조 실험이다. 학습에 쓰는 문장을 뒤죽박죽 셔플해 의미 있는 배열을 부수고, "배울 내용을 제로"로 만들었다. 만약 문지기별 차이가 "말을 잘 배웠기 때문에" 생긴 것이라면, 내용을 없앤 이 조건에서는 차이도 사라져야 한다 —— 그렇게 사전에 예상했다.

그런데 **문지기별 차이는 사라지지 않았다**. 의미 없는 문자열에서도 강한 문지기 쪽이 더 좋은 점수를 계속 냈다. 이는 "말을 배웠기에 똑똑해진" 것이 아님을 뜻한다. 차이를 만든 정체는, 내용과 관계없는 "**진화의 용이성(움직이기 쉬움)**"이었다. 좋은 유전자가 어딘가에 있느냐(천장의 높이)가 아니라, 변이를 거듭하며 거기로 "걸어 올라갈 수 있느냐"의 차이라는 것이다.

쐐기를 박는 확인도 있다. 진화(랜덤 변이와 선택)가 아니라, 경사(gradient)로 진하게 최적화해 학습시키면, **어느 문지기에서도 결과는 같았다**. 즉 강한 문지기의 이득은 "진화일 때 한정"이고, 학습 자체를 끌어올린 것은 아니다.

이는 진 것이 아니다. **이긴 기분이 되기 전에 내역을 의심한다** —— 이것이야말로 FullSense 연구의 핵심이다([feedback_benchmark_honest_disclosure]). 화려한 헤드라인("검사기가 AI를 똑똑하게 만든다")을 버리는 대신, "검사기의 payoff는 learning이 아니라 evolvability"라는, 더 정확하고 수수한 발견이 손에 남았다. 수수해도, 옳은 쪽을 남긴다.

...
<small><strong>EN:</strong> “AI with audit”: runtime_metadata × Approval Bus × Ed25519 audit chain / <strong>中:</strong> "带审查的 AI": runtime_metadata × Approval Bus × Ed25519 审计链</small>
...
<small><strong>EN:</strong> This chapter persists 'inter-individual behaviour + population-level decisions' as an audit trail. Verification and audit substrate. / <strong>中:</strong> 本章把 "个体间行为 + 群体级决策" 作为 audit trail 保存. 是验证与审计的基底.</small>
両輪.
...
<small><strong>EN:</strong> Once an LLM agent rewrites itself, 'what commit produced this inference' becomes critical. / <strong>中:</strong> 一旦 LLM agent 开始重写自身, "该次推断是哪个 commit 产生的" 就变得至关重要.</small>
...
<small><strong>EN:</strong> llive's FitnessReport.runtime_metadata is a free-form dict[str, str], but with conventions. / <strong>中:</strong> llive 的 FitnessReport.runtime_metadata 是 free-form dict[str, str], 但有约定.</small>
慣習的に以下を入れる:
...
<small><strong>EN:</strong> src/llive/approval/bus.py — ApprovalBus. / <strong>中:</strong> src/llive/approval/bus.py — ApprovalBus.</small>
...
<small><strong>EN:</strong> src/llive/security/ family. Landed in Phase 4. / <strong>中:</strong> src/llive/security/ 系列. 在 Phase 4 落地.</small>
...
<small><strong>EN:</strong> The module is not yet landed. Currently loaded via env var / file. Must be fixed before v1.0. / <strong>中:</strong> 该 module 尚未落地. 当前通过 env var / 文件加载. 必须在 v1.0 之前修复.</small>
  解決必須.
...
<small><strong>EN:</strong> Approval Bus pass rate (§3) / audit chain integrity (§4) / peer eval cohesion (§6) are the three measurable axes. / <strong>中:</strong> Approval Bus 通过率 (§3) / 审计链完整性 (§4) / peer eval 内聚度 (§6) 是三个可度量的轴.</small>
...
<small><strong>EN:</strong> Windows Store / macOS Keychain / Linux Keyring integration. / <strong>中:</strong> Windows Store / macOS Keychain / Linux Keyring 集成.</small>
...
<small><strong>EN:</strong> The most compute-heavy part of CoevolutionGovernance.evaluate_generation is the bottleneck. / <strong>中:</strong> CoevolutionGovernance.evaluate_generation 中计算量最大的部分是瓶颈.</small>

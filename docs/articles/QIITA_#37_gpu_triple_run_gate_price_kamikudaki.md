...
- **4/4** —— 不加约束的话,换着随机数跑的 4 次训练里,记忆回路 4 次全都越界到了失控那一侧。而且是在真正的 Transformer 内部(虽然有 attention 这个宽视野帮忙、偏离得比单独时浅一些,但越界还是越界)。这个数字告诉我们:「放着不管也会碰巧安全」是指望不上的
- **19 倍** —— 把安全证明「事后补上」的性能成本,最高是从一开始就在训练中付出时的 19 倍。安全什么时候付可以选,但晚付要付高额利息
- **0 元** —— 这场实验 3 连战+对照实验的全部 GPU 费用(Kaggle 免费额度,共 152 runs)。也就是说,这张「安全护栏价目表」的实测,没有专门设备的人也可以复现
...
不过,也有该老实交代的局限: 这张价目表是在「小号 AI」(比近年的大模型小好几个数量级)上测出来的。所以最后,留下这样一个问题 —— **把 AI 放大之后,这张价目表还是原样吗?** 请回想实验 1: 短的试跑里全部看起来稳定,可训练一跑长,几乎全员越到了护栏外。「短短试一下看着安全的结论,跑久了就会翻盘」 —— 今天的实验自己就演示了这件事。如果同样的反转不是发生在「训练时长」上,而是发生在「模型规模」上,那今天这个 "19 倍",在更大的 AI 上没准也会变成另一个数字。下一篇,我们要把这张价目表搬到**大一号的桌子**上,去试一试:「能不能把安全护栏,从进化的枷锁变成它的**燃料**?」
...
2026년 6월 6일. 인간이 컴퓨터를 향해 내린 실험 지시는 **단 4문장**이었습니다. 「실험을 투입해」 「대조 실험도」 「다음 실험을 진행해」 「투입해」. 요컨대 「알아서 해줘」를 네 번 연타한 것이나 다름없습니다.

그 지시를 받은 AI(Claude Code)가 한 일을 순서대로 늘어놓아 보겠습니다.

1. 실험을 **설계**했다 — 무엇을 측정할지, 합격선은 어디인지를 실험을 돌리기 전에 문서로 고정했습니다(결과를 보고 나서 기준을 움직이는 「뒷북」을 막기 위해서입니다)
2. 자신이 쓴 실험 코드를 **다른 AI 3체에게 일부러 공격하게 했다** — 「자기가 쓴 것을 자기가 다시 본다」만으로는 아무래도 놓치기 때문에, 트집 잡기 전문 심사역을 3체 줄세운 것입니다. 실제로 중대한 결함이 5건 발견되어, 전부 고친 뒤에 투입했습니다
3. **무료 GPU**에 실험을 3연속으로 투입했다 — GPU는 AI 학습에 쓰는 계산 장치로, 보통은 쓰면 돈이 듭니다. 이번에는 Kaggle이라는 사이트(Google의 데이터 분석 서비스로, 주 30시간까지 GPU를 무료로 쓸 수 있다)의 무료 할당만으로 완주했습니다
4. 결과를 회수하고, 「우연히 그렇게 됐을 뿐 아닌가」를 통계적으로 판정하고, 논문 초안에 장을 1개 써넣었다

든 돈은 **0원**입니다.

여기서 하나만, 먼저 말의 준비를 하겠습니다. 「대조 실험」이란 **일부러 배울 의미가 전혀 없는 엉터리 데이터로 같은 실험을 그대로 돌려 보는** 비교용 실험입니다. 약 시험에서 「가짜 약을 먹는 그룹」을 두는 것과 같은 발상으로, 「진짜 데이터라서 일어난 현상인가, 무엇을 시켜도 일어나는 현상인가」를 갈라내기 위해 씁니다. 이 기사의 주장 대부분이 이 비교에 기대고 있으므로, 뒤에서 몇 번이고 등장합니다.

그래서 그 실험이 무엇을 측정했느냐 하면 — **「AI에 안전 울타리를 달면, 성능은 얼마나 떨어지는가」**. 여기서 말하는 안전 울타리란, AI가 학습 도중 「위험한 상태」에 발을 들이려 하면 멈춰 세우는 장치를 말합니다(내용물은 이다음 비유 ① 에서 제대로 설명합니다). 안전을 단단히 하면 똑똑함이 깎일 것 같고, 똑똑함을 우선하면 안전이 얇아질 것 같다 — 그렇다면 실제로 그 「값」은 얼마인가. 사실 이 질문은 지금 전 세계 AI 업계에서 가장 뜨거운 논쟁(기사 후반에 나오는 「safety tax(안전세)」 논쟁)의 미니어처 버전입니다.
...
이번 주인공은 AI 안의 「기억 회로」입니다. 거창한 것이 아니라, **글의 앞부분에 나온 내용을 기억해 두기 위한 부품**이라고 생각해 주세요. 「이 등장인물의 이름은 한참 전에 나왔다」 같은 먼 정보를, 나중에 쓸 수 있도록 들고 나르는 담당입니다.

건전한 기억 회로에는 「에코가 점점 감쇠한다」는 성질이 있습니다. 산에서 「야호—」 하고 외치면, 메아리는 돌아올 때마다 작아지다가 이윽고 사라지지요. 기억도 마찬가지로, 오래된 신호가 조금씩 옅어지는 것이 건전한 상태입니다.

그럼 불건전한 상태란? 마이크와 스피커가 너무 가까우면 끼이익 하고 하울링이 나죠? 그것은 「마이크가 주운 소리를 스피커가 내보내고, 그것을 다시 마이크가 줍는」 루프를 한 바퀴 돌 때마다 소리가 **작아지기는커녕 커져 버리는** 상태입니다. 한 바퀴마다 조금이라도 작아진다면 소리는 저절로 사라집니다. 한 바퀴마다 커진다면 무한히 증폭되어 그 비명이 됩니다. 갈림길은 「루프 한 바퀴에 신호가 줄어드는가, 부풀어 오르는가」 — 단지 그것뿐입니다.

그리고 마침 잘된 일로, 이 「줄어드는가 부풀어 오르는가」는 수학으로 하나의 수로 잴 수 있습니다(완전판에서는 ρ라는 기호로 등장합니다). 그 수가 1보다 작으면 「에코는 반드시 감쇠한다」고 **증명**할 수 있습니다. 이번 연구가 부과하는 제약이 바로 이것입니다 — **기억 회로가 하울링하지 않는다는 것을 수학적으로 증명하고 나서 사용한다. 증명할 수 없는 변화는 통과시키지 않는다.**

실험 1의 발견: **제약하지 않고 학습시키면, AI의 기억 회로는 거의 반드시 하울링 쪽으로 빠져나간다**. 여기서 「학습」이란, 오차가 줄어드는 방향으로 회로의 손잡이를 조금씩 돌리고 또 돌리는 작업입니다. 손잡이를 몇만 번 돌리는 사이, 손잡이의 조합이 어느새 하울링 영역에 들어가 버린다 — 난수를 바꿔 가며 반복한 20회의 훈련 중 19회에서 일어났습니다. 게다가 회로를 키울수록 더 깊이 빠져나갑니다. 더 짓궂은 점은, **짧은 맛보기 훈련에서는 전부 안정으로 보였다**는 것입니다. 본 게임인 긴 훈련으로 바꾸자마자 거의 전원이 울타리 밖으로. 「잠깐 해 봤더니 괜찮았다」는 믿을 게 못 된다는 교훈까지 딸려 왔습니다.

「하울링 쪽이 더 똑똑해질 수 있어서 일부러 가는 거 아닐까?」 — 지당한 의심입니다. 그것을 확인하는 게 대조 실험. 진짜 문장(셰익스피어)을 뒤섞어서 **배울 의미가 전혀 없는 엉터리 데이터**를 만들어 주었습니다. 만약 「하울링 쪽에 똑똑함의 보물이 있다」면, 보물이 존재하지 않는 엉터리 데이터에서는 일부러 밖으로 나갈 이유가 없을 것입니다.

결과: 같은 경계 이탈이 *더 격렬하게* 일어났습니다. 성적은 모든 조건에서 이론적인 한계치에 들러붙은 채 — 즉 똑똑해지는 이득은 0인데도 말입니다. 게다가 회로가 어느 정도 큰 경우에는 일관되게, 진짜 문장을 배우고 있을 때가 엉터리일 때보다 **벗어나는 정도가 얕다**는 것까지 밝혀졌습니다. 「똑똑해지려고 밖으로 나간다」는커녕, 배울 일이 있으면 오히려 길 가까이에 머무는 것입니다.

즉 이것은 「천재는 틀을 깬다」 같은 이야기가 아니라 **사막의 좁은 길** 이야기입니다. 회로가 크다(고차원이다)는 것은 조절할 수 있는 손잡이의 개수가 많다는 뜻입니다. 손잡이가 늘어날수록 「움직일 수 있는 방향」은 폭발적으로 늘어나지만, 그중 안정을 유지할 수 있는 조합은 극히 일부 — 전체에서 보면 상대적으로 「좁은 길」이 되어 갑니다. 좁은 길 위에서 목적도 없이 몇만 걸음을 제자리걸음하면, 확률의 문제로서 언젠가는 길 밖으로 발을 내딛게 됩니다. 아무것도 제약하지 않으면 길에서 벗어난다 — 벗어난 곳에 보물이 있어서가 아니라 **길이 좁아서**. 단지 그것뿐이었다는 것이 대조 실험의 답입니다.

## 비유 ②: 안전 울타리의 가격은 「벌금을 매기는 방식」으로는 바뀌지 않았다

실험 2에서는 아까 그 증명 딸린 기억 회로를, 진짜 미니 Transformer(ChatGPT 등의 토대가 되는 표준 구조의 초소형판)에 심었습니다.

여기에 재미있는 장치가 있습니다. 보통의 Transformer는 「attention(주의)」이라는 시선 장치로 글의 꽤 넓은 범위를 직접 내다볼 수 있습니다. 사실 그래서는 곤란합니다 — 기억 회로가 게으름을 피워도 attention이 대신 먼 곳을 봐 버려서, 기억이 정말로 일했는지를 밖에서 확인할 수가 없으니까요. 그래서 Transformer의 「눈」을 일부러 근시로 만들었습니다(8글자 앞까지밖에 못 보게. 층을 쌓아도 15글자 정도). 그 위에 160글자의 긴 문맥을 줍니다. 그러면 **15글자보다 먼 정보는 기억 회로를 통하는 것 말고는 닿을 길이 없습니다**. 기억이 게으름을 피우면 성적에 즉시 들통나는, 커닝 불가능한 시험장을 만든 셈입니다.

결과 그 1: **증명 딸린 기억은 제대로 작동했습니다**. 기억 회로가 있는 쪽이 없는 쪽보다 일관되게 똑똑하고, 회로를 키울수록 차이는 벌어졌습니다. 「부품이 늘어서 파라미터 수가 많아졌으니까 강한 것뿐 아닐까?」라는 의심에는 여기서도 대조 실험이 답합니다 — 엉터리 데이터에서는 이 차이가 **사라집니다**. 부품 개수의 효과라면 엉터리에서도 차이가 나야 합니다. 사라졌다는 것은, 이 회로가 정말로 「문맥의 내용」을 기억해 나르고 있다고 말할 수 있다는 뜻입니다.

결과 그 2가 본론입니다. 안전 울타리에는 2종류의 운용을 준비했습니다:

- **밀어내기 방식**: 울타리 밖으로 삐져나오려 하면, 부드럽게 안으로 되돌린다
- **되감기 방식**: 삐져나오면 「직전의 합격 상태」까지 다시 하게 한다

이 2가지는 「울타리의 위치」(=제약의 내용)는 완전히 같고, **삐져나왔을 때의 처리만** 다릅니다. 되감기 방식은 다시 하는 헛수고(운용 마찰)가 드는 만큼 비싸게 먹힐 것처럼 보이지요. 그러니 만약 성능 저하의 원인이 「다시 하는 수고」라면, 밀어내기 방식이 더 싸게 먹혀야 합니다 — 여기가 이 비교의 노림수입니다.

그런데 — **양쪽 모두 거의 같은 만큼 손해를 봤습니다**. 자동차로 비유하면, 속도 위반 벌금을 혹독한 일시불에서 친절한 분할 납부로 바꿔도, 목적지까지의 소요 시간은 1분도 줄지 않습니다. 소요 시간을 결정하고 있던 것은 벌금을 걷는 방식이 아니라 **제한 속도 그 자체**였기 때문입니다. 안전 울타리 비용의 정체는 절차의 번거로움이 아니라 「갈 수 있는 곳이 좁다는 것」 그 자체 — 전문 용어로 말하면 「표현력의 제약」 — 였습니다.

게다가 더 중요한 발견: 이 비용은 **엉터리 데이터에서는 발생하지 않습니다**(울타리 유무의 차이가 거기서는 거의 0이 됩니다). 진짜 언어를 학습하고 있을 때만 발생합니다. 즉 안전의 세금은 언제 어디서나 일률적으로 걷히는 고정 수수료가 아니라, **「능력의 현장」에서만** 징수됩니다. 이것이 희소식인 이유는, 고정 수수료라면 그저 낼 수밖에 없지만, 현장에서만 발생하는 세금이라면 「어디를 얼마나 묶을 것인가」라는 세율 설계에 따라 청구서가 달라질 여지가 있다는 것이기 때문입니다. 바로 그렇기 때문에, 세율의 설계에는 의미가 있는 것입니다.

## 비유 ③: 내진 보강은 「지은 후」에 하면 19배 비싸다

가장 놀란 결과가 이것입니다. 여기까지 들으면 이렇게 생각하고 싶어지지 않습니까 — 「훈련 내내 울타리에 감시당하니까 세금을 떼이는 거다. 그렇다면 **울타리 없이 자유롭게 마음껏 훈련하고, 완성된 다음에 안전 증명을 받으러 가면** 양쪽의 좋은 점만 챙길 수 있는 것 아닌가?」. 공장이 도중의 검사를 전부 건너뛰고 마지막에 완성 검사만 받는 그림입니다. 발상으로서는 그럴듯하므로, 실제로 측정해 봤습니다.

결과: 자유롭게 훈련한 기억 회로는 증명 가능한 영역(=하울링하지 않는다고 수학적으로 단언할 수 있는 범위)에서 **너무나 깊이** 벗어나 있었습니다. 비유 ① 에서 본 대로, 묶지 않으면 회로는 어쩌다 보니 울타리 밖으로 흘러가 버리기 때문입니다. 그리고 밖에서 다시 끌어당기려면, 회로 안의 결합 — 부품끼리 이어진 강도 — 을 **원래의 2~6%까지 깎을** 필요가 있었습니다. 여기가 급소입니다. 학습의 성과는 바로 그 「이어진 강도의 조합」으로서 회로에 새겨져 있습니다. 이음매를 9할 이상 깎는다는 것은 **배운 내용을 거의 지운다**는 것. 실측된 성능 비용은 처음부터 울타리를 달고 훈련한 경우의 **17~19배**였습니다.

집을 다 짓고 나서 내진 증명을 받으려 했더니 「이 집은 기준에서 너무 벗어나 있으니 기둥을 95% 깎아 주세요」라는 소리를 들었다, 는 이야기입니다. 그쯤 되면 이미 보강이 아니라 재건축입니다. **안전은 나중에 덧붙일 수 없다. 설계 단계(훈련 루프 안)에 넣을 수밖에 없다** — 이번에 가장 실무에 효과적인 결론입니다.

## 기억해 갈 숫자는 3개

- **4/4** — 제약하지 않으면, 난수를 바꿔 돌린 4회의 훈련 모두에서 기억 회로가 폭주 쪽으로 빠졌습니다. 그것도 진짜 Transformer 안에서, 입니다(attention이라는 넓은 시선의 도움으로 단독일 때보다 얕아지긴 하지만, 넘는 건 넘습니다). 「내버려 둬도 어쩌다 안전」은 기대할 수 없다는 숫자입니다
- **19배** — 안전 증명을 「나중에 덧붙이는」 경우의 성능 비용은, 처음부터 훈련에 짜 넣어 지불하는 경우의 최대 19배. 안전을 언제 낼지는 고를 수 있지만, 후불에는 비싼 이자가 붙습니다
- **0원** — 이 실험 3연전 + 대조 실험 전부의 GPU 비용(Kaggle 무료 할당, 총 152 runs). 즉 이 「안전 울타리 가격표」의 실측은, 특별한 설비가 없어도 누구나 추시(재현)할 수 있습니다
...
다만, 솔직히 밝혀야 할 한계도 있습니다. 이 가격표는 「작은 AI」(최근의 대규모 모델보다 자릿수가 다르게 작은 것)에서 측정한 것입니다. 그래서 마지막에 이런 질문이 남습니다 — **AI를 키워도, 이 가격표는 그대로일까?** 떠올려 주세요. 이번 실험 1에서는 짧은 맛보기 훈련에서는 전부 안정으로 보였는데, 훈련을 길게 돌리자 거의 전원이 울타리 밖으로 나갔습니다. 「짧게 해보면 안전해 보이던 이야기가, 길게 돌리면 뒤집힌다」는 것을 오늘의 실험 스스로가 보여주고 있는 것입니다. 같은 반전이 「훈련의 길이」가 아니라 「모델의 크기」에서도 일어난다면, 오늘의 "19배"도 더 큰 AI에서는 다른 숫자로 바뀔지 모릅니다. 다음 회에는 이 가격표를 **한 치수 큰 책상**에 옮겨 싣고, 「안전 울타리를, 진화의 족쇄가 아니라 그 **연료**로 바꿀 수 있을까?」를 시험해 봅니다.

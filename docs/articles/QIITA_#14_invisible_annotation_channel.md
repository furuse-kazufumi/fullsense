...
OSS マルチプロダクト構成で「**独立性と組合せ価値の両立**」に悩んでいる方は、HTML コメント形式の annotation channel を試す価値があるかもしれない。手元の実装は 200 行ちょっとに収まった。

### では、この hint は誰が受け取るのか

ここまでで「emit 側は consumer を知らない」配線は手に入った。だが annotation は、**受け取る側が現れて初めて意味を持つ**。`<!-- llive:cog.risk_alert -->` を誰も読まなければ、それはただの不可視な文字列のままだ。emit する llive と、それを拾って画面に灯す llove。両者が別々のプロダクトとして独立しているなら——**そもそも、この 1 人開発はどうやって 3 つのプロダクトを同時に動かしているのか?**

annotation channel は「配線」の話だった。次に問うべきは「**それを敷く頭脳**」の話になる。1 人の開発者が複数プロダクトをチーム規模で回すには、何が要るのか。

次回 **#15** では、その問いを正面から扱う——30 年のソフトウェア開発経験に Perplexity・Claude Code・TRIZ・5 万件の論文 RAG を重ねた「**第二の脳**」の構築論だ。本記事で敷いた不可視チャネルが、その第二の脳の中でどう神経のように働くのかを確かめにいく。
...
**次の記事**:
- [15] 「第二の脳」開発論 — 不可視チャネルを神経として使う認知系 (本記事の配線の続き)
...
> 📚 **Series nav**: ← #13 Corpus-first strategy ｜ **#14 (this article)** ｜ #15 "The Second Brain" Development Theory → ｜ [Series LINK_MAP](./QIITA_#24_LINK_MAP.md). ※ Each article reads fine on its own (links are just for browsing).
>
> In the previous article #13, I wrote about how the AI references a corpus in the background and **fills in perspectives I would have missed.** This article shifts the view from inside to outside — it's about the wiring: how three independent products can **be kind to each other without ever knowing one another.**

**One-line hook**:
One day a comment landed on social media: "If three products all depend on each other, the value of using just one of them is cut in half, isn't it?" The reply was a **comment-out** — `<!-- llive:cog.consensus="proceed" -->`. Why would a single line of comment be an answer to that contradiction?
...
> 📚 **连载导航**：← #13 语料库先行战略 ｜ **#14 本文** ｜ #15 「第二大脑」开发论 → ｜ [连载 LINK_MAP](./QIITA_#24_LINK_MAP.md)。※ 每篇文章都可单独阅读（链接仅供回游）。
>
> 上一篇 #13 讲的是 AI 在背景中参照语料库、**替我补足没察觉的观点**。本文则把视角从内移到外——讲的是「配线」：三个互不相识的独立产品，**如何在彼此不知情的情况下相互示好**。

**一句话引子**：
某天，社交网络上收到了这样一条评论：「如果三个产品相互依赖，那么只用其中一个的价值就减半了吧。」回复是一段**注释**——`<!-- llive:cog.consensus="proceed" -->`。为什么短短一行注释，能成为对这个矛盾的回答？
...
> 📚 **연재 내비**: ← #13 코퍼스 선행 전략 ｜ **#14 본 글** ｜ #15 「제2의 뇌」 개발론 → ｜ [연재 LINK_MAP](./QIITA_#24_LINK_MAP.md). ※ 각 글은 단독으로도 읽을 수 있습니다(링크는 회유용).
>
> 앞 글 #13 에서는 AI 가 배경에서 코퍼스를 참조해 **내가 알아채지 못한 관점을 보완해 주는** 이야기를 했다. 본 글은 시점을 안에서 밖으로 옮긴다——서로를 모르는 독립된 3 프로덕트가, **어떻게 서로 모른 채로 친절을 베풀 수 있는가** 라는 배선 이야기다.

**한 줄 훅**:
어느 날 SNS 에 이런 댓글이 도착했다. 「세 개의 프로덕트가 서로 의존하고 있다면, 하나만 쓰는 가치는 반감되겠죠.」 답변은 **주석**이었다 — `<!-- llive:cog.consensus="proceed" -->`. 왜 단 한 줄의 주석이, 그 모순에 대한 답이 되는가.
...
OSS 멀티 프로덕트 구성에서 「**독립성과 조합 가치의 양립**」에 고민하는 분은, HTML 주석 형식의 annotation channel 을 시험해 볼 가치가 있을지도 모른다. 필자의 구현은 200 줄 남짓에 들어맞았다.

### 그럼, 이 hint 는 누가 받는가

여기까지 「emit 측은 consumer 를 모른다」는 배선을 손에 넣었다. 하지만 annotation 은 **받는 쪽이 나타나야 비로소 의미를 갖는다**. 아무도 `<!-- llive:cog.risk_alert -->` 를 읽지 않으면, 그것은 그저 보이지 않는 문자열로 남을 뿐이다. emit 하는 llive 와, 그것을 주워 화면에 켜는 llove. 둘이 각각 독립된 프로덕트라면——**애초에, 한 명의 개발자가 어떻게 세 프로덕트를 동시에 굴리고 있는가?**

annotation channel 은 「배선」 이야기였다. 다음에 물어야 할 것은 「**그것을 까는 두뇌**」 이야기다. 한 명의 개발자가 여러 프로덕트를 팀 규모로 돌리려면, 무엇이 필요한가.

다음 글 **#15** 에서는 그 물음을 정면으로 다룬다——30 년의 소프트웨어 개발 경험에 Perplexity・Claude Code・TRIZ・5 만 건의 논문 RAG 을 겹친 「**제2의 뇌**」 구축론이다. 본 글에서 깔아 둔 비가시 채널이, 그 제2의 뇌 안에서 어떻게 신경처럼 작동하는지를 확인하러 간다.
...
**다음 글**:
- [15] 「제2의 뇌」 개발론 — 비가시 채널을 신경으로 쓰는 인지계 (본 글 배선의 속편)

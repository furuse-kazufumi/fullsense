...
- **Stage 1（monoculture 改善の本命）**: **`poc_evolution_env.py` で実証済の核機構（標準化 novelty + 中立貯蔵庫 + minimal-criterion + QD archive）を MultiPressureSelector に統合**。同 PoC は行動 monoculture **0.05（≪0.8, OE-3 達成）** を実機実証済（[[evolution_poc_deployment_results_2026_05_25]] §2: 標準化 novelty で多様性 83.9–101.1% 持続 vs scalar 5.9–23.8% 崩壊）。**Stage 0 が rich-proxy breakdown のみで飽和したのは、この核機構を未統合だったため**（lldarwin の真価は QD/novelty 統合にある）。+ criteria から nearest_persona_idx 除外、down-sampling、sweep。
...
### 7.1 反証調査（Perplexity + rad-research 2026-05-25, ユーザー指示「反証も必要」）

設計を盲目的に進めず、否定的証拠で鍛える。以下は**受容済みの限界**であり、対策を設計に織り込む。

- **Goodhart's law / proxy 乖離（最重大リスク）**: LLM 弱点を proxy fitness にすると「指標をハックする表面戦略」が進化する — typo→特定置換の暗記 / WSD→テストのヒューリスティクス利用 / multi-step→persuasive な推論痕跡だけ生成 / calibration→自信度の中庸操作。真能力と proxy が乖離（"指標が目標になると良い指標でなくなる"）。**対策**: proxy は mechanism feasibility 検証に限定し production 能力を主張しない。実 LLM/VLM 評価（Stage 2）を本質とする。neutral shadow 対照（Bedau）で見かけの改善を疑う。down-sampling で環境かく乱 + OOD 軸で過学習を抑制。
- **設計者依存性（手法共通）**: lexicase=テストケース / QD=行動記述子 / novelty=距離尺度 / minimal-criterion=基準値、いずれも「多様性の方向」を設計者が決める＝生物進化級の未想定創発は限定的（Taylor et al. 2016）。**受容**: lldarwin は「検証可能性のない多様性の地図」を狙い（DIFF-1）、strong/unbounded open-endedness は主張しない（SCOPE と整合）。
- **minimal-criterion の停滞⇄崩壊トレードオフ**: 基準が低い→選択圧ゼロで停滞 / 高い→全滅（実証あり）。**対策**: criterion を集団分位点で適応 + 全員 fail なら gate 無視（`MultiPressureSelector` 実装済）。
- **QD の次元の呪い + アーカイブ飽和**: 行動記述子が高次元だとセルが空、長期で飽和し新規性頭打ち（Avida/Tierra も同様）。**対策**: 記述子は低次元縮約（DESC-1, JL 射影）+ 飽和を Bedau 統計で監視し「飽和＝失敗」として正直に記録。
- **benchmark overfitting**: GSM8K 型でスコアだけ上がり汎化しない（LLM 研究の実例 + AutoML-Zero 系の局所改善）。**対策**: 複数選択圧 + OOD 軸 + 実タスク評価で過学習を相殺。
- **lexicase のスケール限界**: テスト数が多いと計算コスト増 + ノイズでランダム選択化。**対策**: down-sampled lexicase（毎世代 case 部分集合）でコスト削減 + かく乱。
- **総括**: 「LLM の弱点を proxy fitness にすれば進化で克服できる」は楽観的。lldarwin の価値主張は **(a) proxy は mechanism feasibility のみ (b) 実 LLM/VLM 評価が本質 (c) 多様性の地図化（勝つ軸を限定）** に留める。これが honest disclosure。

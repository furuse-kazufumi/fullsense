...
| 13 | **ルール無視で競合をプロセス排除する個体を生まない／だが到達可能性は残す** | **specification gaming / 進化の創発的 exploit**（Lehman et al. 2018）＋ **safe exploration / containment / mesa-optimization** | **contained open-endedness**: 探索は逸脱に到達可（red-team 信号）／実行は Approval Bus・fail-closed・on-prem 隔離で**実権限ゼロ**／逸脱個体は検出・監査(AuditTrail)・隔離し deploy しない。**#12 メタ進化が自らのガバナンスを迂回しうる**ため**メタ層にもガバナンスを及ぼす**（迂回不可を不変条件に） |

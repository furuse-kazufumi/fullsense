...
## 設計制約: 無限セッション耐性 (ユーザー要件 2026-05-27)
> 「セッションが無限に近い状態でも成り立つ構造。動的に変化していく枠の中で、セッションに応じて現時点での最適回答を出すモデル」

これは北極星 (online 進化 + online 回答) の本質要件。**構造的に有界**でなければ無限セッションで破綻する:
- **MAP-Elites は固定 grid** (cells = n_bins⁴ で有界) → 無限世代でもメモリ有界。
- **集団サイズ固定 + 中立貯蔵庫は系統数有界** → 発散しない。
- **router は現時点の archive から選択 (online)** → 「現時点での最適回答」を任意時点で出せる。
- persona-index 等の拡張も**固定長**を保つ (有界性を壊さない)。
- 動的に変化する枠 = 集団は進化し続ける (固定でない) が、容れ物 (grid/集団/貯蔵庫) は有界。
→ 既存アーキはこの要件を構造的に満たせる。**新機能を足すときも「有界性を壊さない」を不変条件**とする。

## 設計原則: 単純な形式 × 重い選択肢 (ユーザー示唆 2026-05-27)
表現は単純 (離散 index 等) に保ちつつ、**選択肢の意味は重い** (結果を大きく分ける) ものを与える。易しすぎる選択は選択圧にならず飽和する (#27 病理)。persona 間の思考様式差が大きい (グロタン structurize 0.95 ↔ ファインマン exploration 0.95) ことが「重い選択肢」を自然に供給する。
...
## ④ router の出力スタイル要件 (ユーザー示唆 2026-05-27)
オーケストラが出す「1 答」の質:
- **断定を避け、複数の切り口を模索**して提示する (exploratory; 単一の確定答でなく多視点)。
- **対話を通じて将来より良い答に近づく話術** (自己改善的・メタ認知的な応答; その場で確定させず改善余地を残す)。
- **それでも定量的に答えられる** → **数学者/統計家ペルソナを充実** (現 10 名は数学者寄りだが統計・推論系が薄い; Fisher / Tukey / Box / Gelman / Bayes / Friston 等を PERSONA_ONTOLOGY に追加)。
→ router は「多様な奏者の異なる切り口」を統合しつつ定量根拠 (統計家ペルソナ由来) を添える設計。断定回避と定量性は矛盾しない (多視点 + 各視点の信頼度を定量提示)。
- **最重要の歯止め (ユーザー原則 2026-05-27)**: **断定回避 ≠ 解を出さない**。LLM の価値の本質は**実用的ソリューションの提供**であり、多視点・探索はそこに至る手段。router は「複数の切り口 → 統合 → **推奨ソリューション (1答)** + 定量的信頼度 + 改善余地」に必ず**着地**させる。探索を見せて解を出さないのは価値否定 ([[feedback_llm_value_is_solution]])。

...
- **ペルソナライブラリ**: 任意個の persona を登録（prototype = factor_affinity 等の素養
  シグネチャ ＋ 専門性 rubric ＋ 任意の学習記憶）。**1 エントリ追加するだけで拡張**できる。
- **毎世代の獲得 pass**（genome 交叉とは独立）:
  1. 各個体の **平均からの逸脱度（novelty / 中心からの距離）** を測る。
  2. **逸脱が大きい（個性的な）個体**について、各 persona prototype との **相関** を測る。
  3. しきい値を超える最良相関の persona を**獲得**（soft/確率的も可）。該当なしなら**非適用**。
- **継承機構なし**: 親→子の persona 受け渡し・系譜・genome 書き込みは**不要**。毎世代フレッシュに
  再マッチング。→ **隔世（休眠→後世で再覚醒）が特別な管理なしに自然に出る**。

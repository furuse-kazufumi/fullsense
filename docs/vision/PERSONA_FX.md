...
- **重複なし（uniqueness, ユーザー確定 2026-05-24）**: **1 persona は最大 1 個体に、1 個体は最大 1 persona**
  （被らない）。**母数（集団サイズ）が限られているので、これは小さな二部マッチングで十分** —
  相関降順の貪欲割当（取られた persona/個体は skip）or 最適マッチング（Hungarian）を 1 回回すだけ。
  重い機構は不要。「被らないこと」だけが唯一の本質ルール。
- **継承機構なし**: 親→子の persona 受け渡し・系譜・genome 書き込みは**不要**。毎世代フレッシュに
  再マッチング。→ pool > 母数なら余った persona は**休眠（隔世待ち）**、母数 > 割当数なら平均的な個体は
  **persona なし（generic）**。**隔世（休眠→後世で再覚醒）が特別な管理なしに自然に出る**。
...
- **学習記憶の蓄積**: persona ライブラリ側に「その persona を embody した個体が学んだ成功パターン」を
  累積（cultural memory / Cultural Algorithm の belief space）→ 連続性・洗練を付加。
- **襲名（merit succession）**: 「必ず 1 体が担う」1:1 割当（MAP-Elites の 1 cell 1 elite, 名跡継承）。
